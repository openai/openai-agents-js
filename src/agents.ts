...
export async function runSupplementCopilot(
  property: { roof: string; age: number; sqft: number; mesh: number; damageProb: number; cond: string },
  signal?: AbortSignal,
...
const ScoutSchema = z.object({
  rankings: z.array(z.object({ id: z.string(), reason: z.string() })),
});
...
const scoutAgent = new Agent({
  ...
  model: 'gpt-4.1-mini',
  instructions:
    'You are a storm damage canvass expert. Given properties ranked by expected value, write one concise sentence explaining why each is a priority target. Focus on the specific combination of hail size, roof vulnerability, age, and close probability. Be direct and specific.',
  outputType: ScoutSchema,
});

export async function runAIScout(
  properties: Array<{ id: string; address: string; mesh: number; roof: string; age: number; cond: string; damageProb: number; closeProb: number; expectedValue: number }>,
  signal?: AbortSignal,
): Promise<ScoutRanking[]> {
  const list = properties.slice(0, 20).map((p, i) =>
  ...
  const result = await run(scoutAgent, `Rank and explain these targets:\n${list}`, { signal });
...
const conciergeAgent = new Agent({
  ...
  model: 'gpt-4.1-mini',
  instructions:
    "You draft personalized storm damage outreach messages for a roofing contractor. Match the homeowner's contact preference exactly. SMS: under 160 characters. Email: 2-3 short paragraphs. Phone: 3-4 talking points only. Be warm and specific about the storm event.",
  outputType: ConciergeSchema,
});

export async function runConcierge(
  property: { address: string; homeowner: string; contactPref: string; mesh: number; damageProb: number },
  signal?: AbortSignal,
