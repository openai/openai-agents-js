...
1. **Hosted OpenAI tools** – run alongside the model on OpenAI servers. _(web search, file search, code interpreter, image generation, tool search, Programmatic Tool Calling)_
...
| Programmatic Tool Calling | `'programmatic_tool_calling'` | Let the model coordinate eligible tools from isolated JavaScript. |
...
| `programmaticToolCallingTool()` | Enables generated JavaScript to coordinate tools that explicitly include `allowedCallers: ['programmatic']`. |

These helpers map JavaScript/TypeScript-friendly option names to the underlying OpenAI Responses API tool payloads. Refer to the official [OpenAI tools guide](https://developers.openai.com/api/docs/guides/tools) for the full tool schemas and advanced options like ranking options or semantic filters, the official [Tool search guide](https://developers.openai.com/api/docs/guides/tools-tool-search) for the current built-in tool-search flow, and the official [Programmatic Tool Calling guide](https://developers.openai.com/api/docs/guides/tools-programmatic-tool-calling) for runtime details and model availability.

#### Programmatic Tool Calling

Programmatic Tool Calling lets a supported Responses API model generate JavaScript that calls eligible tools, including parallel calls and predictable dependent workflows. Add `programmaticToolCallingTool()` and opt each eligible tool in with `allowedCallers`. Omitted `allowedCallers` keeps the existing direct-only default.
...
Use `outputSchema` on function tools that return predictable JSON values. It describes the JSON encoded in the function output so generated code can use the fields reliably. The SDK preserves `program`, nested tool calls, their caller linkage, and `program_output` in run history and stateless replay.

Keep writes and approval-sensitive actions direct-only unless the application has an explicit authorization boundary. A function can support both routes with `allowedCallers: ['direct', 'programmatic']`. Eligible SDK surfaces are function tools, hosted MCP, shell, apply patch, and code interpreter. Tool search itself remains a top-level tool: deferred tools must be loaded before a later program can call them.
...
| `allowedCallers` | No | Responses API only. Use `['programmatic']` or `['direct', 'programmatic']` to opt the function into Programmatic Tool Calling. Omitted means direct-only. |
| `outputSchema` | No | Responses API only. JSON Schema for the JSON value encoded in the function output. |
