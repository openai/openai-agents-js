...
| --- | --- |
| `webSearchTool(options?)` | JS-friendly options such as `searchContextSize`, `userLocation`, `filters.allowedDomains`, `searchContentTypes`, and `imageSettings`. |
...
These helpers map JavaScript/TypeScript-friendly option names to the underlying OpenAI Responses API tool payloads. To request image search results, include `'image'` in `webSearchTool({ searchContentTypes })`. Use `imageSettings.maxResults` to request a positive number of images and `imageSettings.caption` to request captions when available. The SDK requests raw web search results whenever `searchContentTypes` includes `'image'` and exposes the returned image URLs and metadata on the web search call item's `providerData.results`.

Refer to the official [OpenAI tools guide](https://developers.openai.com/api/docs/guides/tools) for the full tool schemas and advanced options like ranking options or semantic filters, the official [Web search guide](https://developers.openai.com/api/docs/guides/tools-web-search#image-search-results) for image result fields, the official [Tool search guide](https://developers.openai.com/api/docs/guides/tools-tool-search) for the current built-in tool-search flow and model availability, and the official [Programmatic Tool Calling guide](https://developers.openai.com/api/docs/guides/tools-programmatic-tool-calling) for model support and wire-level behavior.
...
- Tool discovery belongs to the Agent that performed the search and does not transfer through a handoff. The SDK preserves discovery for the same logical Agent across history or Session replay when that Agent's name is unique in the run graph. Missing or ambiguous ownership requires a new search, so give Agents unique names when persisted discovery must survive reconstruction.
...
Programmatic Tool Calling is Responses-only. Chat Completions, Voice agents, and the AI SDK model adapter reject these options. When an eligible tool is deferred, tool search must load it before a later generated program can call it.
