...
| Test a `RealtimeAgent` function tool | Emit `function_call` and expect `sendFunctionCallOutput` | [Test a Voice agent tool workflow](#test-a-realtime-agent-tool-workflow) |
...
## Voice agent and session recipes

### Test a Voice agent tool workflow
...
<Code
  ...
  title="Test a Voice agent function tool"
/>
...
- [Voice Agents](/openai-agents-js/guides/voice-agents) introduces `RealtimeSession`.
