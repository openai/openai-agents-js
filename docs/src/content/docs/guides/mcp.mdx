...
| `allowedTools`    | `string[] \| { toolNames?: string[] }`                                                         | Allowlist of tool names exposed to the model.                                                            |
| `requireApproval` | `'never' \| 'always' \| { always?: { toolNames: string[] }, never?: { toolNames: string[] } }` | Approval policy for hosted MCP tool calls. If omitted, defaults to `'never'`.                            |
...
When you work with multiple MCP servers, you can use `connectMcpServers` to connect them together, track failures, and close them in one place.
...
If your runtime supports `Symbol.asyncDispose`, `MCPServers` also supports the `await using` pattern.
