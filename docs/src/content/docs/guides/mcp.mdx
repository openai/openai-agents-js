...
## MCP SDK v2 and v1 server compatibility

The Agents SDK uses the MCP TypeScript SDK v2 client internally. This does not require your MCP server to migrate to the v2 server package. For stdio and Streamable HTTP connections, the Agents SDK negotiates the protocol automatically and remains compatible with existing servers implemented with the MCP TypeScript SDK v1 when they support a compatible protocol version.

If your application only uses the Agents SDK's `MCPServerStdio`, `MCPServerStreamableHttp`, or `MCPServerSSE` classes, you do not need to install or bridge MCP v1 and v2 packages for this compatibility. Migrate application-owned MCP SDK objects separately when you want to adopt the v2 MCP SDK APIs; see the MCP TypeScript SDK's [v1 to v2 migration guide](https://github.com/modelcontextprotocol/typescript-sdk/blob/main/docs/migration/upgrade-to-v2.md).
...
The `requireApproval` object form accepts optional `always` and `never` entries. Each entry can select tools with `toolNames` or by their read-only annotation. The optional `onApproval` callback receives the current run context and approval item, then returns a promise containing `approve` and an optional `reason`.
...
<Code
  ...
  title="Configure local MCP tools for an agent"
/>
...
- The SDK removes URL credentials, query parameters, and fragments from URL-derived server names and transport errors before exposing them through logs, traces, and error messages. Set an explicit, unique, non-secret `name` when you need a stable server label.
...
| `toolInputGuardrails` | `ToolInputGuardrailDefinition[]` | Run these guardrails before every tool call converted from this server. |
| `toolOutputGuardrails` | `ToolOutputGuardrailDefinition[]` | Run these guardrails after every tool call converted from this server. |
| `useStructuredContent` | `boolean` | Use JSON-serialized MCP `structuredContent` as the model-visible output when available and the result is not an error. Defaults to `false`. |
...
`toolInputGuardrails` and `toolOutputGuardrails` use the SDK's local function-tool guardrail pipeline. They apply to every tool converted from this server in streaming and non-streaming runs. They do not apply to hosted MCP tools, which the Responses API executes remotely. See [Tool guardrails](/openai-agents-js/guides/guardrails#tool-guardrails) for rejection behavior and approval ordering.
...
Before reconnecting, `MCPServers` closes the selected servers. It reconnects only servers that closed successfully; cleanup failures remain available through `failed` and `errors`.
...
<Code
  ...
  title="Close connected MCP servers with async disposal"
/>
...
<Code
  ...
  title="Prefix local MCP tool names with server names"
/>
...
Generated names are ASCII-safe, stay within the name-length limit for `FunctionTool` instances, and do not collide with names of local `FunctionTool` instances or enabled handoffs on the same agent. The setting affects all local MCP transports, including Streamable HTTP, SSE, and stdio; hosted MCP tools keep their hosted server labels and tool metadata.
