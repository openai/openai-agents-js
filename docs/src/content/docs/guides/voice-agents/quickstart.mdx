...
1. **Create a project**
   ...
   In this quickstart we will create a realtime agent you can use in the browser. If you want to scaffold a new project, you can start with [`Next.js`](https://nextjs.org/docs/getting-started/installation) or [`Vite`](https://vite.dev/guide/installation.html).
...
2. **Create a session**
   ...
   Unlike a regular agent, a realtime agent is continuously running inside a `RealtimeSession` that handles the conversation and connection to the model over time. This session also manages audio processing, interruptions, and the broader conversation lifecycle that you will configure later on.
...
<Aside type="note">
  This quickstart uses the shortest path to get a browser voice agent running:
  `RealtimeSession` runs in the browser, and the built-in WebRTC transport uses
  a data channel for Realtime events. If the browser should carry only audio
  while an application server owns Realtime events, tools, and business logic,
  see [Browser audio with server-side
  controls](/openai-agents-js/guides/voice-agents/transport#browser-audio-with-server-side-controls).
  For lower-level browser control, see the [`/raw-client` example in
...
From here you can start designing and building your own realtime agent:
