...
- `OpenAIRealtimeWebRTC` is the simplest browser path and handles audio input/output for you. Its built-in connection also uses a data channel for Realtime events. If the browser should carry only audio while an application server owns Realtime events, tools, and business logic, see [Browser audio with server-side controls](/openai-agents-js/guides/voice-agents/transport#browser-audio-with-server-side-controls).
...
For input transcription, `audio.input.transcription` accepts the GA transcription models and their context options. Use `gpt-live-transcribe` for low-latency streaming transcription; its `delay` setting trades faster partial transcripts against final transcript quality. Both `gpt-live-transcribe` and `gpt-transcribe` accept a free-form `prompt`, literal `keywords`, and a `languages` array. These models use `languages`; older transcription models that accept one language hint continue to use the singular `language` field. See the official [transcription context guidance](https://developers.openai.com/api/docs/guides/transcription/#improve-transcription-quality) and [Realtime transcription guide](https://developers.openai.com/api/docs/guides/realtime-transcription/) for model-specific workflows.
...
| `callId` | `string` | Attach the WebSocket transport as a sideband connection to an existing Realtime call, including calls established with WebRTC or SIP. |
...
Unlike regular agents, handoffs behave slightly differently for Voice Agents. When a handoff is performed, the ongoing session is updated with the new agent configuration in place. Because of this, the new agent automatically has access to the ongoing conversation history and input filters are currently not applied.
...
Just like regular agents, Voice Agents can call tools to perform actions. Realtime supports **function tools** (executed locally) and **hosted MCP tools** (executed remotely by the Realtime API). You can define a function tool using the same `tool()` helper you would use for a regular agent.

Responses-only tool options do not carry over to Voice agents. Function tool `outputSchema` and `allowedCallers` values that include `'programmatic'` are rejected, as are programmatic callers on hosted MCP tools. Use directly callable function or hosted MCP tools in a Realtime session, or delegate work that needs Programmatic Tool Calling to a Responses agent.
