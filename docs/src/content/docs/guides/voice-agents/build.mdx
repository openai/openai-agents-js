...
5. You cannot add new `mcp_call`, `mcp_tool_call`, or `mcp_approval_request` items, or update existing ones, through `updateHistory()`. `resetHistory()` rejects those changes with a `UserError` before sending any events, because the current history shape does not retain all of the fields the Realtime API needs to recreate an MCP item once its conversation entry is deleted. Leaving an MCP item unchanged, or removing it from history entirely, is still supported. Full MCP history replay is being considered separately.
