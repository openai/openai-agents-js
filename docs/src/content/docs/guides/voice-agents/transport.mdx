...
The built-in transports target OpenAI's GA Realtime wire format. Azure OpenAI Realtime endpoints can expose a different session schema or lifecycle, especially when the session is created while negotiating WebRTC. Before switching providers, verify that your Azure deployment accepts the fields sent by the SDK, including `session.type` and `output_modalities`.

If the endpoint implements an older or provider-specific protocol, use the SDK release supported by that endpoint or provide a custom `RealtimeTransportLayer` that transforms the outgoing session configuration. Keep the application server responsible for any provider-specific URL, model, and authentication details; do not put a standard OpenAI API key in browser code.
...
For the full setup, read [Voice Agents on Cloudflare](/openai-agents-js/extensions/cloudflare).
...
For the full setup, read [Voice Agents on Twilio](/openai-agents-js/extensions/twilio).
