...
## React Native / Custom Environment Bindings

When using React Native, you can inject your own WebSocket and WebRTC bindings (e.g., from `react-native-webrtc`) before creating a session:
...
// Must be called once before creating any WebRTC connections
registerGlobals();
...
// Then create the session as usual
``` 
