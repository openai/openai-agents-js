...
- Each `run()` or `Runner.run()` invocation is wrapped in a task span.
- Each time an agent runs, it is wrapped in `AgentSpan`
- Each model/tool loop iteration is wrapped in a turn span.
...
Task and turn spans add a high-level `task -> agent -> turn` hierarchy and aggregate token usage for each scope. To omit only these runner-created spans while keeping agent, model, tool, guardrail, and handoff spans, set `includeTaskAndTurnSpans` to `false`:
...
You can also pass the same `tracing` option to an individual `run()` call. This setting does not affect task or turn spans that you create manually with `createTaskSpan()` or `createTurnSpan()`.
