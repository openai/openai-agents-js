...
Use `VercelCloudBucketMountStrategy` with an `s3Mount()` entry to expose an S3 bucket inside a Vercel sandbox. The client ensures that Mountpoint for Amazon S3 1.21 or later is available in the sandbox. Mounts are read-only by default; set `readOnly: false` only when the agent must overwrite or delete bucket objects.

This example passes short-lived AWS credentials on the mount entry. Because those credentials are forwarded to a command in the remote sandbox, the client requires the explicit `allowS3CredentialExposure: true` acknowledgement. You can instead supply the standard `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and optional `AWS_SESSION_TOKEN` variables through the client's `env` option or the manifest environment.

<Code
  lang="typescript"
  code={vercelS3MountExample}
  title="Mount an S3 bucket in a Vercel sandbox"
/>

Vercel S3 mounts have a deliberately narrow lifecycle:

- Declare every mount in the manifest used to create the sandbox. After a mount is active, `materializeEntry()` and `applyManifest()` are not supported.
- Reuse the live session when you need the same mounted sandbox across runs. Serialized session state that contains mounts cannot be resumed.
- Mounted bucket contents are excluded from workspace persistence. Tar persistence and hydration temporarily detach and restore the mounts, while native Vercel snapshot hydration is not supported with active mounts.
- Closing the session unmounts the buckets before stopping the sandbox.

Mount paths must remain inside the Vercel workspace, cannot be the workspace root, and cannot overlap another mount path. The client also rejects hydrate archives that would overwrite a declared mount path.
