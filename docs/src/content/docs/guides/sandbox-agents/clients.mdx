...
| Hosted providers | Provider-specific strategies live with provider implementations and should be documented with those provider branches or extension packages. |
...
Hosted provider implementations live in `@openai/agents-extensions` provider subpaths. Provider-specific docs and examples should add exact environment variable notes, runnable examples, port behavior, PTY support, snapshot behavior, and cleanup behavior alongside the provider code.

The TypeScript hosted clients are expected to live in `@openai/agents-extensions` provider subpaths, matching the provider branches. Install `@openai/agents-extensions` and satisfy its package-level peers; each provider may also require a provider SDK package or backend setup:
...
Hosted provider support varies by provider. Keep provider-specific docs close to the provider implementation so users can see exact options, environment variables, port behavior, PTY support, snapshot behavior, and cleanup behavior.
