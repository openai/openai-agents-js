...
| `VercelSandboxClient` | Supports create-time S3 mounts with `VercelCloudBucketMountStrategy`. Inline credentials require the trusted exact-path manifest acknowledgement described above; reuse the live mounted session because serialized mount state cannot be resumed directly. |
