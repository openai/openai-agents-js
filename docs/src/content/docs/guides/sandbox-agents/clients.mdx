...
| Goal | Start with | Why |
| --- | --- | --- |
| Trusted local development on macOS or Linux | `UnixLocalSandboxClient` | Runs host processes with a managed workspace; adds no OS-level confinement. |
| Basic container isolation | `DockerSandboxClient` | Runs work inside Docker with a specific image. |
| Hosted execution | A hosted sandbox client | Uses a provider-managed environment; isolation depends on the provider and configuration. |
...
| --- | --- | --- |
| `UnixLocalSandboxClient` | none | Trusted local development on macOS or Linux, or execution within an externally isolated environment. |
| `DockerSandboxClient` | Docker CLI available locally | You want container isolation or a specific image for local parity. |

`UnixLocalSandboxClient` runs shell commands as host processes on both macOS and Linux, including commands that use a PTY. The client does not add OS-level filesystem or network isolation. Commands run with the permissions of the selected host user, subject to any externally enforced isolation. The workspace directory, command working directory, and `HOME` environment variable do not confine commands to the workspace.

Each new Unix-local session receives a separate temporary workspace. `Manifest.root` is a logical path, and `workspaceBaseDir` selects the parent directory for temporary workspaces. Using the same value for either setting does not make newly created sessions share a workspace. Reusing a live session shares that session's workspace. Separate Unix-local sessions and workspaces do not establish an OS isolation boundary.

Unix-local path grants restrict supported host source access and SDK file APIs. A `readOnly` grant prevents SDK file API writes to the granted path; the grant does not restrict arbitrary shell commands. Commands can access host paths allowed by the process's permissions and external isolation even without a path grant. See [manifest path grants](/openai-agents-js/guides/sandbox-agents/concepts#manifest).

For untrusted commands, including commands influenced by untrusted inputs, choose an appropriately configured `DockerSandboxClient` or hosted backend, or provide external isolation. Select the backend and configuration for the filesystem, network, and process isolation your application requires.
...
Set `labels: Record<string, string>` in the `DockerSandboxClient` constructor or per-run `sandbox.options` to apply user-defined labels to the created container. A per-run `labels` record replaces the constructor record instead of merging with it. The SDK rejects non-string values and reserves `openai-agents-sandbox`, `openai-agents-sandbox.session-identity`, and `openai-agents-sandbox.mount-authority-fingerprint` for its own ownership checks.

The SDK copies user-defined labels into serialized Docker session state. When you explicitly resume that state, caller-provided labels must exactly match the persisted label record; start a fresh sandbox session to change them. Live-container reuse also verifies that every configured label still has its required value. Unrelated labels added outside the SDK do not prevent reuse.
