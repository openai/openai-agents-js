...
| `VercelSandboxClient` | Supports create-time S3 mounts with `VercelCloudBucketMountStrategy`. Inline credentials require `allowS3CredentialExposure: true`; reuse the live mounted session because serialized mount state cannot be resumed. |
