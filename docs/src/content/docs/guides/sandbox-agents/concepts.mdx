...
Throughout this page, "sandbox session" means the live execution environment managed by a sandbox client. The exact boundary depends on the client and configuration: Unix-local sessions run host processes without adding OS-level confinement on macOS or Linux, while Docker and hosted clients use container or provider-managed execution environments. This is different from the SDK's conversational `Session` interfaces described in [Sessions](/openai-agents-js/guides/sessions).

The outer runtime still owns approvals, tracing, handoffs, and resume bookkeeping. The sandbox session manages commands and file changes. The backend and configuration determine the isolation provided by the execution environment.
...
If shell access is only one occasional tool, start with hosted shell in the [Tools guide](/openai-agents-js/guides/tools). Reach for sandbox agents when workspace management, sandbox client choice, or sandbox-session resume behavior are part of the design.
...
Use `UnixLocalSandboxClient` for trusted local development on macOS or Linux, or within an externally isolated environment. Unix-local workspaces, working directories, and the `HOME` environment variable do not provide OS-level confinement. For untrusted commands, including commands influenced by untrusted inputs, choose an appropriately configured `DockerSandboxClient` or hosted provider, or supply external isolation. On Windows, use Docker or a hosted provider. See [Unix-local execution limits](/openai-agents-js/guides/sandbox-agents/clients#local-clients).
...
For secrets that must be looked up again when a manifest is materialized or resumed, subclass `EnvValueReference` and register it with `registerEnvValueReference()`. A reference persists only non-secret lookup metadata and resolves the current runtime value through `resolve()`. Each subclass must declare its own stable, non-empty static `type`, and `serialize()` must not include the reserved `value` field. Register the parser in every process before reconstructing manifests, and have the parser validate and allowlist lookup keys. Resuming persisted `RunState` environment references requires a current trusted manifest with corresponding reference entries; otherwise the SDK rejects the resume before restoring the session.
...
Each path grant has a sandbox path and, when needed, a separate host path:

- `path` is the absolute POSIX path visible inside the sandbox.
- `hostPath` is an optional absolute native host path used when the host and sandbox paths differ. On Windows it must be drive-qualified; UNC and device paths are not supported.

`UnixLocalSandboxClient` requires the host and sandbox paths to be the same, so omit `hostPath` there. `DockerSandboxClient` supports a separate `hostPath` when the grant is present in the manifest used to create the container. It cannot add or change that mapping on an already-running container. A Docker command can use a create-time granted sandbox path as its `workdir`; paths outside the workspace and configured grants remain rejected.

For Unix-local sessions, these grants govern access to local sources through the SDK and paths accessed by SDK file APIs. A `readOnly: true` grant prevents SDK file API writes to the granted path. The grants do not constrain arbitrary shell commands on macOS or Linux: commands can access host paths allowed by the process's permissions and external isolation, even when those paths have no grant.
...
Use local snapshots for local durable snapshots and remote snapshots when your app provides a remote snapshot client. Mounted and ephemeral paths are not copied into snapshots as durable workspace contents. During hydration, the SDK rejects archive members that overlap any protected mounted or ephemeral path before writing the archive into the workspace. If the workspace root itself is ephemeral, only an empty archive can be hydrated.
...
Use `archiveLimits` to cap archive input bytes, extracted bytes, and member count during workspace hydration and resume. Passing an empty object enables the defaults: 1 GiB of archive input, 4 GiB extracted, and 100,000 members. Set an individual field to `null` to disable only that limit. Omitting `archiveLimits`, or setting the whole option to `null`, disables archive limits entirely.
...
Tool-agents can either get their own sandbox session or reuse a live sandbox from the parent run. Reuse is useful for an explorer agent that only needs to inspect the parent's workspace without paying to create, hydrate, or snapshot another sandbox. Sharing a session does not itself enforce read-only access.

Give a tool-agent its own `runConfig` through `sandboxAgent.asTool(...)` when the tool-agent needs a separate workspace, backend, or image. For untrusted commands, choose a backend and configuration that provide the required isolation. A separate Unix-local session alone does not provide OS-level confinement.
...
- [Agents as tools](/openai-agents-js/guides/tools#4-agents-as-tools): expose multiple sandbox agents as tools, usually by passing a sandbox run config on each `asTool(...)` call so each tool gets its own session. The backend and configuration determine the isolation provided by each session.
