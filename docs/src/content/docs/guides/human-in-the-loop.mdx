...
If you serialize `RunState`, use unique `name` values for every agent in the handoff and `Agent.asTool()` graph. Agent names are used to resolve serialized agents during resume, and duplicate names raise an error when the state is serialized or deserialized.
