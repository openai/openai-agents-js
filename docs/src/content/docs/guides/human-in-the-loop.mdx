...
Use `RunState.addInput()` when new user input arrives while a run is paused and should be admitted before the next resumed model call. The staged input is part of the serialized state, so it survives `toString()` / `fromString()` even if unresolved approvals or local tool work delay that model call.

```ts
const state = result.state;

state.addInput('The user also asked for a cost estimate.');
...
When these callbacks return a decision, the run continues without pausing for a human response. For Realtime session APIs, see the approval flow in the [Realtime agents build guide](/openai-agents-js/guides/voice-agents/build#approval-before-tool-execution).
