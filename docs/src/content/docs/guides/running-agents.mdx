...
| `tracing` | – | Per-run tracing configuration overrides, including the export API key and `includeTaskAndTurnSpans`. |
...
| `tracing` | `TracingConfig` | Per-run tracing overrides, including the export API key and `includeTaskAndTurnSpans`. |
