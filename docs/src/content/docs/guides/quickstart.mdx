...
const historyFunFact = tool({
  ...
  execute: async () => {
    ...
    return 'Sharks are older 
});
```function generateColdCallScripts() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Sheet1");
  const data = sheet.getDataRange().getValues();
  const apiKey = "YOUR_OPENAI_API_KEY"; // Get a free key at platform.openai.com
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!status || status === "") {
      const prompt = `Write a cold call script for a real estate wholesaler reaching out to ${name} about their property at ${address}. Be friendly, brief, mention a cash offer, and ask if they’re interested in selling.`;
      ...
      sheet.getRange(i + 1, 8).setValue(script); // Column H
      sheet.getRange(i + 1, 6).setValue("Script Created");
    }
  }
}
