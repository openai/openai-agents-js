...
> **Note** Output guardrails only run if the agent is the _last_ agent in the workflow. For realtime spoken interactions see [the Realtime agents guide](/openai-agents-js/guides/voice-agents/build#guardrails).
...
Set `outputGuardrailBlockedMessage` on `Runner` or in the options for an individual `run()` call when your application needs a different data-free placeholder. The per-run value takes precedence over the `Runner` value. You can provide either a non-empty string or a formatter that receives `defaultMessage`, `guardrailName`, `agent`, and `runContext`. The formatter can be asynchronous, but it does not receive the rejected output. If the formatter throws, rejects, or returns an empty or non-string value, the SDK fails closed with the default placeholder instead of exposing the formatter failure.

## Tool guardrails

Tool guardrails wrap **function tools** and let you validate or block tool calls before and after execution. You can configure guardrails on an individual tool through `tool()` options or on a local MCP server so they run for every tool that the SDK converts from that server.

For a custom function tool, set `inputGuardrails`, `outputGuardrails`, or both on `tool({...})`. For `MCPServerStreamableHttp`, `MCPServerStdio`, or `MCPServerSSE`, set `toolInputGuardrails`, `toolOutputGuardrails`, or both in the server constructor options. Server-wide MCP guardrails use the same local function-tool execution pipeline and apply to every converted tool from that server.
...
Tool guardrails apply to function tools you define with `tool()` and to tools converted from local MCP servers when the server configures them. Handoffs are presented to the model as function-like tools, but they run through the SDK's handoff path rather than the normal function-tool pipeline, so tool guardrails do not apply to the handoff call itself. Hosted MCP tools, other hosted tools, and built-in execution tools (`computerTool`, `shellTool`, `applyPatchTool`) also do not use this guardrail pipeline, and `agent.asTool()` does not currently expose tool-guardrail options directly.
