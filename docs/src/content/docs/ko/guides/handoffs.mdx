...
핸드오프는 한 에이전트가 대화의 일부를 다른 에이전트에 위임하도록 합니다. 이는 서로 다른 에이전트가 특정 영역을 전문적으로 다룰 때 유용합니다. 예를 들어 고객 지원 앱에서는 예약, 환불, FAQ를 담당하는 에이전트를 둘 수 있습니다.
...
프롬프트에 핸드오프를 언급하면 LLM의 반응이 더 안정적입니다. SDK는 `RECOMMENDED_PROMPT_PREFIX`를 통해 권장 접두사를 제공합니다.
