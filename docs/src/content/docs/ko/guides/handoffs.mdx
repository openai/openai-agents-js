...
핸드오프는 LLM 에게 도구로 표현됩니다. `Refund Agent`라는 에이전트로 핸드오프할 경우 도구 이름은 `transfer_to_refund_agent`가 됩니다.
...
프롬프트에 핸드오프를 언급하면 LLM 의 응답 신뢰도가 높아집니다. SDK 는 `RECOMMENDED_PROMPT_PREFIX`를 통해 권장 접두사를 제공합니다.
