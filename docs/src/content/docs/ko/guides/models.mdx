...
`Agent`를 초기화할 때 모델을 지정하지 않으면 기본 모델이 사용됩니다. 현재 기본값은 [`gpt-4.1`](https://platform.openai.com/docs/models/gpt-4.1)이며, 에이전트형 워크플로의 예측 가능성과 낮은 지연 시간 간의 뛰어난 균형을 제공합니다.

[`gpt-5`](https://platform.openai.com/docs/models/gpt-5) 같은 다른 모델로 전환하려면 두 가지 방식으로 에이전트를 구성할 수 있습니다.
...
이 방식으로 GPT-5의 추론 모델([`gpt-5`](https://platform.openai.com/docs/models/gpt-5), [`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini), [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano))을 사용할 때, SDK는 합리적인 기본 `modelSettings`를 적용합니다. 구체적으로 `reasoning.effort`와 `verbosity`를 모두 `"low"`로 설정합니다. 기본 모델의 추론 강도를 조정하려면, 원하는 `modelSettings`를 전달하세요:
...
더 낮은 지연 시간을 위해 [`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini) 또는 [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano)에 `reasoning.effort="minimal"`을 사용하면 기본 설정보다 더 빠르게 응답을 받는 경우가 많습니다. 다만 Responses API의 일부 내장 도구(예: 파일 검색과 이미지 생성)는 `"minimal"` 추론 강도를 지원하지 않으므로, 이 Agents SDK의 기본값은 `"low"`입니다.
...
맞춤 네트워킹 설정이 필요하다면 `setDefaultOpenAIClient(client)`를 통해 직접 만든 `OpenAI` 클라이언트를 연결할 수도 있습니다.
...
| `store`             | `boolean`                                  | 응답을 저장하여 검색 / RAG 워크플로에 사용                                 |
| `reasoning.effort`  | `'minimal' \| 'low' \| 'medium' \| 'high'` | gpt-5 등의 추론 강도                                                       |
| `text.verbosity`    | `'low' \| 'medium' \| 'high'`              | gpt-5 등의 텍스트 장황도                                                   |

설정은 어느 수준에든 부착할 수 있습니다:

<Code lang="typescript" code={modelSettingsExample} title="모델 설정" />

`Runner` 수준 설정은 충돌하는 에이전트별 설정을 무시합니다.
...
도구나 instructions 같은 추가적인 에이전트 구성은 저장된 프롬프트에 구성해 둔 값을 재정의합니다.

---

## 사용자 지정 모델 프로바이더

자체 프로바이더 구현은 간단합니다 – `ModelProvider`와 `Model`을 구현하고, 그 프로바이더를 `Runner` 생성자에 전달하세요:
