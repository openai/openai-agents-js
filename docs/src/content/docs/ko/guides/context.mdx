...
Context는 과부하된 용어입니다. 신경 써야 할 컨텍스트에는 두 가지 주요 클래스가 있습니다:

1. 실행 중 코드가 액세스할 수 있는 **로컬 컨텍스트**: 도구에 필요한 의존성이나 데이터, `onHandoff` 같은 콜백, 라이프사이클 훅
...
LLM이 호출될 때 볼 수 있는 데이터는 대화 기록에서만 옵니다. 추가 정보를 제공하려면 다음과 같은 선택지가 있습니다:

1. 에이전트 `instructions`에 추가 – 시스템 또는 개발자 메시지라고도 합니다. 컨텍스트를 받아 문자열을 반환하는 함수이거나 정적 문자열일 수 있습니다.
...
4. 검색이나 웹 검색 도구를 사용해 파일, 데이터베이스 또는 웹의 관련 데이터로 응답을 근거 있게 만듭니다.
