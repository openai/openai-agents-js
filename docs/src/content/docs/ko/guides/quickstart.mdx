...
에이전트를 실행하려면 `run` 메서드를 사용합니다. 시작할 에이전트와 전달할 입력을 함께 넘기면 실행이 시작됩니다.
...
추가 에이전트를 유사하게 정의하여 문제를 더 작은 부분으로 나누고, 에이전트가 현재 작업에 더욱 집중하도록 할 수 있습니다. 또한 에이전트에 모델을 정의하여 문제에 따라 다른 모델을 사용할 수 있습니다.
...
여러 에이전트를 오케스트레이션하기 위해 에이전트에 `handoffs` 를 정의할 수 있습니다. 이렇게 하면 에이전트가 대화를 다음 에이전트로 넘길 수 있습니다. 이는 실행 과정에서 자동으로 일어납니다.
...
Runner 는 개별 에이전트의 실행, 발생 가능한 핸드오프 및 도구 실행을 처리합니다.
...
이제 모두 하나의 전체 예제로 묶어 보겠습니다. 이를 `index.js` 파일에 넣고 실행하세요.

<Code lang="typescript" code={quickstartExample} title="빠른 시작" />

## 트레이스 보기

Agents SDK 는 트레이스를 자동으로 생성합니다. 이를 통해 에이전트의 동작, 호출한 도구, 핸드오프 대상 에이전트를 검토할 수 있습니다.

에이전트 실행 동안 무슨 일이 일어났는지 확인하려면
