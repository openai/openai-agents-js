...
3. OpenAI API 키를 설정합니다. 키가 없다면 OpenAI API 키를 생성하려면 [이 안내서](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따르세요.
...
에이전트는 instructions 와 name 으로 정의됩니다.
...
이는 해당 실행 동안 수행된 모든 액션과 최종 출력을 담은 결과를 반환합니다.
...
추가 에이전트를 비슷하게 정의하여 문제를 더 작은 부분으로 나누고, 현재 작업에 더 집중하도록 할 수 있습니다. 또한 에이전트에 모델을 정의해 문제마다 다른 모델을 사용할 수도 있습니다.
...
여러 에이전트를 오케스트레이션하려면 에이전트에 `handoffs` 를 정의할 수 있습니다. 이렇게 하면 에이전트가 대화를 다음 에이전트로 넘길 수 있습니다. 이는 실행 과정에서 자동으로 이루어집니다.
...
Runner 는 개별 에이전트의 실행, 잠재적 핸드오프, 도구 실행을 처리합니다.
