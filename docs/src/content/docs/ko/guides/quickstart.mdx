...
1. 프로젝트를 생성하고 npm을 초기화하세요. 한 번만 하면 됩니다.
...
3. OpenAI API 키를 설정하세요. 없다면 OpenAI API 키를 만들기 위해 [이 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따르세요.
   ...
   자세한 내용은 [SDK 설정](/openai-agents-js/ko/guides/config) 가이드를 참조하세요.
...
에이전트에 정보를 검색하거나 작업을 수행하기 위한 도구를 제공할 수 있습니다.
...
문제를 더 작은 부분으로 나누고 현재 작업에 더 집중하도록 추가 에이전트를 유사하게 정의할 수 있습니다. 에이전트에 모델을 정의하여 문제별로 다른 모델을 사용할 수도 있습니다.
...
여러 에이전트를 오케스트레이션하기 위해 에이전트의 `handoffs` 를 정의할 수 있습니다. 그러면 에이전트가 다음 에이전트로 대화를 넘길 수 있으며, 실행 과정에서 자동으로 이루어집니다.
...
Runner 는 개별 에이전트 실행, 잠재적 핸드오프, 도구 실행을 처리합니다.
...
이제 모두 합쳐 하나의 전체 예제로 만들어 봅시다. 이를 `index.js` 파일에 넣고 실행하세요.

<Code lang="typescript" code={quickstartExample} title="Quickstart" />

## 트레이스 보기

Agents SDK 는 자동으로 트레이스를 생성합니다. 이를 통해 에이전트의 동작, 호출한 도구, 핸드오프 대상 등을 검토할 수 있습니다.

에이전트 실행 중에 무슨 일이 있었는지 검토하려면
