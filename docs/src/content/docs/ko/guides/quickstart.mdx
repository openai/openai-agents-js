...
1. 프로젝트를 생성하고 npm 을 초기화하세요. 한 번만 수행하면 됩니다.
...
3. OpenAI API 키를 설정하세요. 없다면 OpenAI API 키를 생성하려면 [이 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따르세요.
...
이 호출은 해당 실행 동안 수행된 최종 출력과 모든 작업이 담긴 결과를 반환합니다.
...
에이전트에 정보를 조회하거나 작업을 수행하기 위한 도구를 제공할 수 있습니다.
...
추가 에이전트를 유사하게 정의해 문제를 더 작은 부분으로 나누고, 현재 작업에 더 집중하도록 만들 수 있습니다. 또한 에이전트에 모델을 정의하여 각 문제에 다른 모델을 사용할 수 있습니다.
...
여러 에이전트를 오케스트레이션하려면 에이전트에 `handoffs` 를 정의하세요. 이렇게 하면 에이전트가 대화를 다음 에이전트로 넘길 수 있습니다. 이는 실행 과정에서 자동으로 발생합니다.
...
실행 후 결과의 `finalAgent` 속성을 보면 최종 응답을 생성한 에이전트를 확인할 수 있습니다.

## 에이전트 오케스트레이션 실행

Runner 는 개별 에이전트 실행, 잠재적인 핸드오프, 도구 실행을 처리합니다.
...
Agents SDK 는 자동으로 트레이스를 생성합니다. 이를 통해 에이전트가 어떻게 동작했는지, 어떤 도구를 호출했는지, 어느 에이전트로 핸드오프했는지 검토할 수 있습니다.

에이전트 실행 중에 무슨 일이 있었는지 확인하려면
[OpenAI Dashboard 의 Trace viewer](https://platform.openai.com/traces)로 이동하세요.

## 다음 단계

더 복잡한 에이전트 플로우를 구축하는 방법을 알아보세요:
