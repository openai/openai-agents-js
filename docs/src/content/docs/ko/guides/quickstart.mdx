...
1. 프로젝트를 만들고 npm을 초기화하세요. 한 번만 수행하면 됩니다.
...
3. OpenAI API 키를 설정하세요. 키가 없다면 [이 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따라 OpenAI API 키를 생성하세요.
   ...
   자세한 내용은 [구성 가이드](/openai-agents-js/ko/guides/config)를 참고하세요.
...
추가 에이전트를 유사하게 정의하여 문제를 더 작은 부분으로 분해하고, 각 에이전트가 맡은 작업에 더 집중하도록 할 수 있습니다. 또한 에이전트에 모델을 정의하여 문제마다 다른 모델을 사용할 수 있습니다.
...
여러 에이전트를 오케스트레이션하기 위해, 에이전트에 `handoffs`를 정의할 수 있습니다. 이를 통해 에이전트가 다음 에이전트로 대화를 넘길 수 있습니다. 이는 실행 과정에서 자동으로 수행됩니다.
...
Agents SDK는 자동으로 트레이스를 생성합니다. 이를 통해 에이전트가 어떻게 동작했는지, 어떤 도구를 호출했는지, 어느 에이전트로 핸드오프했는지를 검토할 수 있습니다.
