...
- `stream: true`로 `run`을 호출한 경우 [`StreamedRunResult`](/openai-agents-js/openai/agents/classes/streamedrunresult). 스트리밍에 대한 자세한 내용은 [스트리밍](/openai-agents-js/ko/guides/streaming)도 참고하세요
...
- `unknown` — 에이전트의 출력 타입으로 JSON 스키마가 정의된 경우. 이 경우 JSON은 파싱되었지만 타입은 수동으로 검증해야 합니다
- `z.infer<outputType>` — 에이전트의 출력 타입으로 Zod 스키마가 정의된 경우. 출력은 이 스키마에 대해 자동으로 파싱됩니다
...
이렇게 하면 SDK가 가능한 모든 핸드오프 전반에 걸친 출력 타입을 추론하고 `finalOutput` 속성에 대해 유니온 타입을 제공합니다.
...
`lastAgent` 속성은 마지막으로 실행된 에이전트를 포함합니다. 애플리케이션에 따라 다음에 user가 무언가를 입력할 때 유용한 경우가 많습니다. 예를 들어, 프런트라인 분류 에이전트가 언어별 에이전트로 핸드오프하는 경우, 마지막 에이전트를 저장해 두었다가 다음에 user가 에이전트에 메시지를 보낼 때 재사용할 수 있습니다.

스트리밍 모드에서는 현재 실행 중인 에이전트에 매핑되는 `currentAgent` 속성에 접근하는 것도 유용할 수 있습니다.

## 새 항목

`newItems` 속성은 실행 중에 생성된 새 항목을 포함합니다. 항목은 [`RunItem`](/openai-agents-js/openai/agents/type-aliases/runitem)입니다. 실행 항목은 LLM이 생성한 원문 항목을 래핑합니다. 이를 사용하면 LLM의 출력뿐만 아니라 이러한 이벤트가 어떤 에이전트와 연관되었는지도 확인할 수 있습니다.
...
`state` 속성은 실행의 상태를 포함합니다. `result`에 첨부된 대부분은 `state`에서 파생되지만, `state`는 직렬화/역직렬화가 가능하며, [에러에서 복구](/openai-agents-js/ko/guides/running-agents#exceptions)가 필요하거나 [`interruption`](#interruptions)을 처리해야 하는 경우 이후의 `run` 호출에 대한 입력으로도 사용할 수 있습니다.
...
`rawResponses` 속성은 에이전트 실행 중 모델이 생성한 원문의 LLM 응답을 포함합니다.

### 마지막 응답 ID

`lastResponseId` 속성은 에이전트 실행 중 모델이 생성한 마지막 응답의 ID를 포함합니다.

### 가드레일 결과

`inputGuardrailResults`와 `outputGuardrailResults` 속성은 가드레일 결과(있는 경우)를 포함합니다. 가드레일 결과에는 로그를 남기거나 저장하고 싶은 유용한 정보가 포함될 수 있으므로, 이를 제공해 드립니다.

도구 가드레일 결과는 `toolInputGuardrailResults` 및 `toolOutputGuardrailResults`를 통해 별도로 노출됩니다.

### 원본 입력

`input` 속성은 run 메서드에 제공한 원본 입력을 포함합니다. 대부분의 경우 필요하지 않지만 필요한 경우를 대비해 제공됩니다.
