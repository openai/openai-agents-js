...
이렇게 하면 SDK 가 가능한 모든 핸드오프 전반에서 출력 타입을 추론하고 `finalOutput` 속성에 대해 유니온 타입을 제공합니다.
...
- `result.history` — 입력과 에이전트들의 출력 사본을 모두 포함
- `result.output` — 전체 에이전트 실행의 출력을 포함

`history`는 채팅형 유스케이스에서 전체 히스토리를 유지하기에 편리합니다:
...
`newItems` 속성에는 실행 중에 생성된 새 항목이 포함됩니다. 항목은 [`RunItem`](/openai-agents-js/openai/agents/type-aliases/runitem)s 입니다. 실행 항목은 LLM 이 생성한 원문 항목을 래핑합니다. 이를 사용하면 LLM 의 출력과 더불어 이러한 이벤트가 어느 에이전트에 연관되었는지에 접근할 수 있습니다.
...
`state` 속성에는 실행의 상태가 포함됩니다. `result`에 첨부된 대부분은 `state`에서 파생되지만, `state`는 직렬화/역직렬화 가능하며 [에이전트 실행](/openai-agents-js/ko/guides/running-agents#exceptions)로 다시 호출할 때 입력으로도 사용할 수 있습니다. 이는 오류에서 복구하거나 [`interruption`](#interruptions)을 처리해야 하는 경우에 유용합니다.

## 인터럽션(중단 처리)

에이전트에서 `needsApproval`을 사용 중인 경우, 계속하기 전에 처리해야 하는 `interruptions`가 트리거될 수 있습니다. 이때 `interruptions`는 인터럽션을 유발한 `ToolApprovalItem`들의 배열이 됩니다. 인터럽션을 처리하는 방법에 대한 자세한 내용은 [휴먼 인 더 루프 (HITL)](/openai-agents-js/ko/guides/human-in-the-loop) 가이드를 확인하세요.
...
`inputGuardrailResults`와 `outputGuardrailResults` 속성에는 가드레일의 결과(있는 경우)가 포함됩니다. 가드레일 결과에는 로그하거나 저장하고 싶은 유용한 정보가 포함되는 경우가 있으므로 이를 제공합니다.

도구 가드레일 결과는 `toolInputGuardrailResults` 및 `toolOutputGuardrailResults`를 통해 별도로 제공됩니다.

### 원본 입력

`input` 속성에는 run 메서드에 제공한 원본 입력이 포함됩니다. 대부분의 경우 필요하지 않겠지만, 필요한 경우 사용할 수 있도록 제공됩니다.
