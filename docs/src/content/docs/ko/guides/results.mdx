...
- `unknown` — 에이전트가 출력 타입으로 JSON 스키마를 정의한 경우. 이때 JSON은 파싱되었지만 타입은 직접 검증해야 합니다
...
- `result.history` — 입력과 에이전트 출력의 복사본을 모두 포함
- `result.output` — 전체 에이전트 실행의 출력을 포함

채팅과 유사한 사용 사례에서는 전체 이력을 유지하기 위해 `history`가 편리합니다:

<Code lang="typescript" code={historyLoop} title="히스토리 루프" />

## 마지막 에이전트

`lastAgent` 속성에는 마지막으로 실행된 에이전트가 포함됩니다. 애플리케이션에 따라 다음 번에 사용자가 입력할 때 유용할 수 있습니다. 예를 들어, 1차 분류 에이전트가 언어별 에이전트로 핸드오프하는 경우 마지막 에이전트를 저장해 두었다가 사용자가 다음에 메시지를 보낼 때 재사용할 수 있습니다.

스트리밍 모드에서는 현재 실행 중인 에이전트를 가리키는 `currentAgent` 속성에 접근하는 것도 유용합니다.

## 새 항목

`newItems` 속성에는 실행 중 생성된 새 항목이 포함됩니다. 항목은 [`RunItem`](/openai-agents-js/openai/agents/type-aliases/runitem)s 입니다. 실행 항목은 LLM이 생성한 원문 항목을 래핑합니다. 이를 사용해 LLM 출력 외에도 해당 이벤트가 어떤 에이전트와 연관되었는지 접근할 수 있습니다.
...
- [`RunReasoningItem`](/openai-agents-js/openai/agents/classes/runreasoningitem)은 LLM의 reasoning 항목을 나타냅니다. 원문 항목은 생성된 reasoning 입니다
...
`state` 속성에는 실행의 상태가 포함됩니다. 대부분의 `result`에 첨부된 정보는 `state`에서 파생되지만, `state`는 직렬화/역직렬화가 가능하며, [오류 복구](/openai-agents-js/ko/guides/running-agents#exceptions)나 [`interruption`](#interruptions) 처리가 필요한 경우 이후 `run` 호출의 입력으로도 사용할 수 있습니다.
...
`lastResponseId` 속성에는 에이전트 실행 중 모델이 마지막으로 생성한 응답의 ID가 포함됩니다.

### 가드레일 결과

`inputGuardrailResults` 및 `outputGuardrailResults` 속성에는 가드레일 결과(있는 경우)가 포함됩니다. 가드레일 결과에는 로깅하거나 저장하고 싶은 유용한 정보가 포함될 수 있어 이를 제공합니다.
