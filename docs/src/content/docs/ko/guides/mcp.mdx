...
> 참고: SDK 에는 레거시 Server‑Sent Events 전송을 위한 `MCPServerSSE` 도 포함되어 있지만, SSE 는 MCP 프로젝트에서 더 이상 권장되지 않습니다. 새로운 통합에는 Streamable HTTP 또는 stdio 를 사용하세요.
...
호스티드 툴은 전체 라운드트립을 모델 내부에서 처리합니다. 애플리케이션 코드가 MCP 서버를 호출하는 대신 OpenAI Responses API 가 원격 도구 엔드포인트를 호출하고 결과를 모델로 스트리밍합니다.
...
도구 호출의 안전성을 프로그램적으로 판단할 수 있다면 [`onApproval` 콜백](https://github.com/openai/openai-agents-js/blob/main/examples/mcp/hosted-mcp-on-approval.ts)을 사용해 승인 또는 거부할 수 있습니다. 사람의 승인이 필요하다면 로컬 함수 도구에서와 동일하게 `interruptions` 를 사용하는 [human-in-the-loop (HITL) approach](/openai-agents-js/guides/human-in-the-loop/)를 적용할 수 있습니다.

<Code
  lang="typescript"
  code={hostedHITLExample}
  title="호스티드 MCP 도구와 함께하는 휴먼인더루프 (HITL)"
/>
...
이 예시에서 `GOOGLE_CALENDAR_AUTHORIZATION` 환경 변수는 Google OAuth Playground 에서 얻은 OAuth 토큰을 포함하며, 커넥터 기반 서버가 Calendar API 를 호출하도록 승인합니다. 스트리밍도 함께 시연하는 실행 가능한 샘플은 [`examples/connectors`](https://github.com/openai/openai-agents-js/tree/main/examples/connectors)를 참고하세요.
...
<Code
  ...
  title="Streamable HTTP MCP 서버와 함께 실행"
/>
...
표준 I/O 만 노출하는 서버의 경우 `fullCommand` 로 `MCPServerStdio` 를 인스턴스화하세요:

<Code
  lang="typescript"
  code={stdioExample}
  title="Stdio MCP 서버와 함께 실행"
/>

## 알아두면 좋은 사항

**Streamable HTTP** 및 **Stdio** 서버의 경우, `Agent` 가 실행될 때마다 사용 가능한 도구를 확인하기 위해 `list_tools()` 를 호출할 수 있습니다. 이 라운드트립은 지연을 유발할 수 있으므로(특히 원격 서버), `MCPServerStdio` 또는 `MCPServerStreamableHttp` 에 `cacheToolsList: true` 를 전달하여 메모리에서 결과를 캐시할 수 있습니다.
