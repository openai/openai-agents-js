...
도구는 에이전트가 **행동을 수행**하도록 합니다. 데이터를 가져오거나, 외부 API를 호출하거나, 코드를 실행하거나, 심지어 컴퓨터를 사용할 수 있습니다. JavaScript/TypeScript SDK는 네 가지 카테고리를 지원합니다:

1. **호스티드 툴** – OpenAI 서버에서 모델과 함께 실행됩니다. _(웹 검색, 파일 검색, 컴퓨터 사용, Code Interpreter, 이미지 생성)_
2. **함수 도구** – 어느 로컬 함수든 JSON 스키마로 래핑해 LLM이 호출할 수 있게 합니다.
...
| Image generation        | `'image_generation'` | 텍스트를 기반으로 이미지 생성          |

<Code lang="typescript" code={toolsHostedToolsExample} title="Hosted tools" />

정확한 매개변수 세트는 OpenAI Responses API와 일치합니다. `rankingOptions`나 의미 기반 필터와 같은 고급 옵션은 공식 문서를 참고하세요.

---

## 2. 함수 도구

`tool()` 헬퍼로 **어떤** 함수든 도구로 전환할 수 있습니다.
...
| `strict`        | No       | `true`(기본값)일 때, 인수가 유효성 검사를 통과하지 않으면 SDK가 모델 오류를 반환합니다. 퍼지 매칭을 원하면 `false`로 설정하세요.                                                                              |
...
유효하지 않거나 부분적인 입력을 모델이 추론하도록 해야 한다면 원문 JSON 스키마를 사용할 때 strict 모드를 비활성화하세요:
...
에이전트를 도구로 실행하면, Agents SDK는 기본 설정으로 러너를 만들고 함수 실행 내에서 해당 에이전트를 실행합니다. `runConfig` 또는 `runOptions`의 속성을 제공하려면 `asTool()` 메서드에 전달하여 러너 동작을 사용자 지정할 수 있습니다.
...
완전한 예시는 [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts)를 참고하세요. 또한 MCP 서버 도구 통합에 대한 종합적인 가이드를 찾는다면 [모델 컨텍스트 프로토콜 (MCP)](/openai-agents-js/ko/guides/mcp)를 참조하세요.
...
- **입력 검증** – 가능하다면 Zod 스키마로 엄격한 JSON 검증을 사용하세요
...
- [`tool()`](/openai-agents-js/openai/agents/functions/tool)과 다양한 호스티드 툴 타입에 대한 TypeDoc 레퍼런스를 살펴보세요
