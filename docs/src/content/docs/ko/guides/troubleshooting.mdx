...
- **Cloudflare Workers**: Agents SDK는 Cloudflare Workers에서 사용할 수 있지만, 현재 다음과 같은 제한이 있습니다:
  ...
  - 요청 종료 시 트레이스를 수동으로 플러시해야 합니다. 자세한 내용은 [트레이싱 가이드](/openai-agents-js/ko/guides/tracing#export-loop-lifecycle)를 참고하세요
  - Cloudflare Workers의 `AsyncLocalStorage` 제한적 지원으로 인해 일부 트레이스가 정확하지 않을 수 있습니다
  - 출력 WebSocket 연결은 fetch 기반 업그레이드를 사용해야 합니다(전역 `WebSocket` 생성자 사용 불가). Realtime의 경우 `@openai/agents-extensions`의 Cloudflare 전송(`CloudflareRealtimeTransportLayer`)을 사용하세요
- **브라우저**:
  - 브라우저에서는 현재 트레이싱이 지원되지 않습니다
- **v8 아이솔레이트**:
  - 적절한 브라우저 폴리필을 갖춘 번들러를 사용하면 v8 아이솔레이트용으로 SDK를 번들링할 수 있지만, 트레이싱은 작동하지 않습니다
  - v8 아이솔레이트는 광범위하게 테스트되지 않았습니다
...
또는 SDK의 특정 부분에만 디버깅 범위를 지정할 수 있습니다:
