...
- **Cloudflare Workers**: Agents SDK는 Cloudflare Workers에서 사용할 수 있지만, 현재 다음과 같은 제한이 있습니다:
  ...
  - 트레이스는 요청이 끝날 때 수동으로 flush 해야 함. 자세한 내용은 [트레이싱](/openai-agents-js/ko/guides/tracing#export-loop-lifecycle)을 참조하세요
...
- **v8 isolates**:
  - 적절한 브라우저 폴리필을 갖춘 번들러를 사용하면 v8 isolates용으로 SDK를 번들링할 수 있지만, 트레이싱은 동작하지 않음
  - v8 isolates는 광범위하게 테스트되지 않음

## 디버그 로깅

SDK 사용 중 문제가 발생한다면 디버그 로깅을 활성화하여 동작에 대한 더 많은 정보를 확인할 수 있습니다.
...
또는 SDK의 특정 부분에만 디버깅을 범위 제한할 수 있습니다:
