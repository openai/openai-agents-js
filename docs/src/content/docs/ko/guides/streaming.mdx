...
Agents SDK 는 모델 및 기타 실행 단계를 점진적으로 출력할 수 있습니다. 스트리밍은 UI 를 반응적으로 유지하고, 전체 최종 결과를 기다리지 않고 사용자에게 업데이트할 수 있게 합니다.
...
스트리밍이 활성화되면 반환된 `stream` 은 `AsyncIterable` 인터페이스를 구현합니다. 각 발생 이벤트는 실행 중에 무슨 일이 일어났는지 설명하는 객체입니다. 스트림은 에이전트 실행의 서로 다른 부분을 설명하는 세 가지 이벤트 유형 중 하나를 내보냅니다. 대부분의 애플리케이션은 모델의 텍스트만 원하므로 스트림은 이를 위한 헬퍼를 제공합니다.

### 텍스트 출력 가져오기

발생하는 텍스트의 스트림을 얻으려면 `stream.toTextStream()` 을 호출하세요. `compatibleWithNodeStreams` 가 `true` 이면 반환값은 일반 Node.js `Readable` 입니다. 이를 `process.stdout` 또는 다른 대상으로 직접 파이프할 수 있습니다.
...
프라미스 `stream.completed` 는 실행과 모든 보류 중인 콜백이 완료되면 resolve 됩니다. 더 이상 출력이 없음을 보장하려면 항상 이를 await 하세요.

### 모든 이벤트 수신

각 이벤트가 도착할 때 검사하려면 `for await` 루프를 사용할 수 있습니다. 유용한 정보에는 저수준 모델 이벤트, 에이전트 전환, 그리고 SDK 고유의 실행 정보가 포함됩니다:
...
완전한 스크립트 예시는 [the streamed example](https://github.com/openai/openai-agents-js/tree/main/examples/agent-patterns/streamed.ts) 를 참고하세요. 순수 텍스트 스트림과 원문 이벤트 스트림을 모두 출력합니다.

## 이벤트 유형

스트림은 세 가지 서로 다른 이벤트 유형을 내보냅니다:
...
스트리밍은 실행을 일시 중지하는 핸드오프와 호환됩니다(예: 도구가 승인을 요구하는 경우). 스트림 객체의 `interruption` 필드는 인터럽션(중단 처리) 을 노출하며, 각 항목에 대해 `state.approve()` 또는 `state.reject()` 를 호출하여 실행을 계속할 수 있습니다. `{ stream: true }` 로 다시 실행하면 스트리밍 출력이 재개됩니다.
