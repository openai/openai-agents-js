...
Agents SDK 는 모델과 기타 실행 단계를 점진적으로 출력할 수 있습니다. 스트리밍은 UI 를 반응성 있게 유지하고 전체 최종 결과를 기다리지 않고 사용자에게 업데이트할 수 있게 합니다.
...
스트리밍이 활성화되면 반환된 `stream` 은 `AsyncIterable` 인터페이스를 구현합니다. 각 발생 이벤트는 실행 중에 무슨 일이 일어났는지 설명하는 객체입니다. 스트림은 에이전트 실행의 서로 다른 부분을 설명하는 세 가지 이벤트 유형 중 하나를 방출합니다. 대부분의 애플리케이션은 모델의 텍스트만 원하므로 스트림은 이를 위한 도우미를 제공합니다.

### 텍스트 출력 가져오기

방출된 텍스트의 스트림을 얻으려면 `stream.toTextStream()` 을 호출합니다. `compatibleWithNodeStreams` 가 `true` 이면 반환값은 일반 Node.js `Readable` 입니다. 이를 바로 `process.stdout` 또는 다른 대상으로 파이프할 수 있습니다.
...
실행과 보류 중인 모든 콜백이 완료되면 `stream.completed` 프로미스가 resolve 됩니다. 더 이상 출력이 없음을 보장하려면 항상 이를 기다리세요.

### 모든 이벤트 수신

각 이벤트가 도착할 때 검사하려면 `for await` 루프를 사용할 수 있습니다. 유용한 정보에는 저수준 모델 이벤트, 모든 에이전트 전환, 그리고 SDK 고유 실행 정보가 포함됩니다:
...
스트리밍은 실행을 일시 중지하는 핸드오프(예: 도구 승인 필요)와 호환됩니다. 스트림 객체의 `interruption(중단 처리)` 필드가 인터럽션(중단 처리)을 노출하며, 각 인터럽션에 대해 `state.approve()` 또는 `state.reject()` 를 호출해 실행을 계속할 수 있습니다. `{ stream: true }` 로 다시 실행하면 스트리밍 출력이 재개됩니다.
...
- 애플리케이션이 텍스트 결과만 중요하다면 개별 이벤트 객체를 처리하지 않도록 `toTextStream()` 을 사용하는 것을 권장합니다
