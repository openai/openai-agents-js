...
3. **첫 번째 에이전트 만들기**
   ...
   새로운 [`RealtimeAgent`](/openai-agents-js/openai/agents-realtime/classes/realtimeagent/) 만들기는 일반 [`Agent`](/openai-agents-js/ko/guides/agents) 만들기와 매우 유사합니다.
...
4. **세션 생성**
   ...
   일반 에이전트와 달리 Voice Agent는 `RealtimeSession` 내에서 지속적으로 실행되고 청취하며, 시간이 지남에 따라 대화 및 모델과의 연결을 처리합니다. 이 세션은 오디오 처리, 인터럽션(중단 처리), 그리고 나중에 다룰 기타 수명 주기 기능도 처리합니다.
...
5. **세션에 연결**
   ...
   세션에 연결하려면 이전에 생성한 클라이언트 에페메럴 토큰을 전달해야 합니다.

   ```typescript
   await session.connect({ apiKey: 'ek_...(put your own key here)' });
   ```

   이는 브라우저에서 WebRTC를 사용해 Realtime API에 연결하고 오디오 입력 및 출력을 위해 마이크와 스피커를 자동으로 구성합니다. `RealtimeSession`을 백엔드 서버(예: Node.js)에서 실행하는 경우 SDK는 자동으로 WebSocket을 연결 방식으로 사용합니다. 다양한 전송 계층에 대해서는 [전송 방식](/openai-agents-js/ko/guides/voice-agents/transport) 가이드에서 더 알아볼 수 있습니다.
...
7. **엔진을 가동하고 대화를 시작하기**
   ...
   웹 서버를 시작하고 새 Realtime Agent 코드가 포함된 페이지로 이동하세요. 마이크 액세스 요청이 표시됩니다. 접근을 허용하면 에이전트와 대화를 시작할 수 있습니다.
