...
1. **프로젝트 생성**
   ...
   이 빠른 시작에서는 브라우저에서 사용할 수 있는 음성 에이전트를 만듭니다. 새 프로젝트를 시도해 보고 싶다면 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 또는 [`Vite`](https://vite.dev/guide/installation.html)를 사용해 보세요.

   ```bash
   npm create vite@latest my-project -- --template vanilla-ts
   ```

2. **Agents SDK 설치** (Zod v4 필요)

   ```bash
   npm install @openai/agents zod
   ```

   또는 독립형 브라우저 패키지로 `@openai/agents-realtime`을 설치할 수 있습니다.

3. **클라이언트 임시 토큰 생성**

   이 애플리케이션은 사용자의 브라우저에서 실행되므로 Realtime API 를 통해 모델에 안전하게 연결할 방법이 필요합니다. 이를 위해 백엔드 서버에서 생성해야 하는 [임시 클라이언트 키](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)를 사용할 수 있습니다. 테스트 목적이라면 `curl`과 일반 OpenAI API 키를 사용해 키를 생성할 수도 있습니다.
   ...
   응답에는 최상위에 "value" 문자열이 포함되며, "ek\_" 접두사로 시작합니다. 이 임시 키를 사용해 이후 WebRTC 연결을 설정할 수 있습니다. 이 키는 유효 기간이 짧으므로 재생성이 필요합니다.
...
2. **세션 생성**
   ...
   일반 에이전트와 달리 Voice Agent 는 대화를 지속적으로 처리하고 시간에 따라 모델과의 연결을 유지하는 `RealtimeSession` 내부에서 계속 실행되고 청취합니다. 이 세션은 오디오 처리, 인터럽션(중단 처리), 그 외 이후에 다룰 다양한 라이프사이클 기능도 처리합니다.
   ...
   `RealtimeSession` 생성자는 첫 번째 인수로 `agent`를 받습니다. 이 에이전트가 사용자가 처음으로 상호작용할 수 있는 에이전트가 됩니다.
...
3. **세션에 연결**
   ...
   이는 브라우저에서 WebRTC 를 사용해 Realtime API 에 연결하고, 오디오 입력과 출력을 위해 마이크와 스피커를 자동으로 구성합니다. `RealtimeSession`을 백엔드 서버(예: Node.js)에서 실행하는 경우 SDK 는 자동으로 WebSocket 을 연결 방식으로 사용합니다. 다양한 전송 계층에 대해서는 [전송 방식](/openai-agents-js/ko/guides/voice-agents/transport) 가이드에서 더 알아볼 수 있습니다.
...
2. **실행하고 대화 시작**
   ...
   웹 서버를 시작하고 새로운 Realtime Agent 코드가 포함된 페이지로 이동하세요. 마이크 접근 권한 요청이 표시됩니다. 권한을 허용하면 에이전트와 대화를 시작할 수 있습니다.
...
여기서부터는 자신만의 음성 에이전트를 설계하고 구축할 수 있습니다. 음성 에이전트는 일반 에이전트와 동일한 기능이 많지만, 고유한 기능도 일부 포함합니다.
...
- 음성 에이전트에 다음을 제공하는 방법 알아보기:
  ...
  - [오디오 인터럽션(중단 처리) 처리](/openai-agents-js/ko/guides/voice-agents/build#interruptions)
  - [세션 기록 관리](/openai-agents-js/ko/guides/voice-agents/build#conversation-history-management)
...
- 다양한 전송 계층 더 알아보기
  ...
  - [사용자 지정 전송 메커니즘 구축](/openai-agents-js/ko/guides/voice-agents/transport#building-your-own-transport-mechanism)
