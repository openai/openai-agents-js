...
0. **프로젝트 생성**
   ...
   이 빠른 시작에서는 브라우저에서 사용할 수 있는 음성 에이전트를 만듭니다. 새 프로젝트를 시작해 보고 싶다면 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 또는 [`Vite`](https://vite.dev/guide/installation.html)를 사용해 볼 수 있습니다.
...
2. **클라이언트 일회용 토큰 생성**
   ...
   이 애플리케이션은 사용자의 브라우저에서 실행되므로 Realtime API를 통해 모델에 안전하게 연결해야 합니다. 이를 위해 백엔드 서버에서 생성해야 하는 [ephemeral client key](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)를 사용할 수 있습니다. 테스트 목적으로는 `curl`과 일반 OpenAI API 키를 사용해 키를 생성할 수도 있습니다.
   ...
   응답에는 최상위에 "value" 문자열이 포함되며, "ek\_" 접두사로 시작합니다. 이 일회용 키를 사용해 나중에 WebRTC 연결을 설정할 수 있습니다. 이 키는 유효 기간이 짧으므로 다시 생성해야 합니다.
...
4. **세션 생성**
   ...
   일반 에이전트와 달리, 음성 에이전트는 대화와 시간 경과에 따른 모델과의 연결을 처리하는 `RealtimeSession` 내부에서 지속적으로 실행되고 청취합니다. 이 세션은 오디오 처리, 인터럽션(중단 처리), 기타 여러 수명주기 기능도 처리합니다.
   ...
   `RealtimeSession` 생성자는 첫 번째 인수로 `agent`를 받습니다. 이 에이전트는 사용자가 처음 상호작용할 수 있는 에이전트가 됩니다.

5. **세션에 연결**

   세션에 연결하려면 앞에서 생성한 클라이언트 일회용 토큰을 전달해야 합니다.

   ```typescript
   await session.connect({ apiKey: 'ek_...(put your own key here)' });
   ```

   이는 브라우저에서 WebRTC를 사용해 Realtime API에 연결하고 마이크와 스피커를 자동으로 구성하여 오디오 입출력을 처리합니다. `RealtimeSession`을 백엔드 서버(예: Node.js)에서 실행하는 경우 SDK는 자동으로 WebSocket을 연결 방식으로 사용합니다. 다양한 전송 계층에 대해 더 알아보려면 [전송 방식](/openai-agents-js/ko/guides/voice-agents/transport) 가이드를 참고하세요.
...
7. **엔진 가동 및 대화 시작**
   ...
   웹 서버를 시작하고 새로운 Realtime Agent 코드가 포함된 페이지로 이동하세요. 마이크 액세스 요청이 표시됩니다. 액세스를 허용하면 에이전트와 대화를 시작할 수 있습니다.
...
여기에서 자신만의 음성 에이전트를 설계하고 구축할 수 있습니다. 음성 에이전트는 일반 에이전트와 많은 기능을 공유하지만, 고유한 기능도 일부 포함합니다.
