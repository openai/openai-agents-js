...
0. **프로젝트 생성**
   ...
   이 빠른 시작에서는 브라우저에서 사용할 수 있는 음성 에이전트를 만듭니다. 새로운 프로젝트를 시작하려면 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 또는 [`Vite`](https://vite.dev/guide/installation.html)를 사용해 볼 수 있습니다.

   ```bash
   npm create vite@latest my-project -- --template vanilla-ts
   ```

1. **Agents SDK 설치** (Zod v4 필요)
...
4. **세션 생성**
   ...
   일반 에이전트와 달리, 음성 에이전트는 대화와 모델과의 지속적인 연결을 처리하는 `RealtimeSession` 내에서 계속 실행되고 청취합니다. 이 세션은 오디오 처리, 인터럽션(중단 처리), 그리고 이후 다룰 다양한 라이프사이클 기능도 처리합니다.
...
5. **세션 연결**
   ...
   이렇게 하면 브라우저에서 WebRTC를 사용해 Realtime API에 연결하고 마이크와 스피커를 자동으로 오디오 입력과 출력에 맞게 구성합니다. `RealtimeSession`을 백엔드 서버(예: Node.js)에서 실행하는 경우 SDK는 자동으로 WebSocket을 연결로 사용합니다. 다양한 전송 계층에 대해서는 [Realtime 전송 계층](/openai-agents-js/ko/guides/voice-agents/transport) 가이드에서 자세히 알아볼 수 있습니다.
...
7. **엔진 가동 및 대화 시작**
   ...
   웹 서버를 시작하고 새 Realtime Agent 코드가 포함된 페이지로 이동하세요. 마이크 접근 권한 요청이 표시됩니다. 허용하면 에이전트와 대화를 시작할 수 있습니다.
...
여기서부터 자신만의 음성 에이전트를 설계하고 구축할 수 있습니다. 음성 에이전트는 일반 에이전트와 많은 기능을 공유하지만 고유한 기능도 일부 포함합니다.
