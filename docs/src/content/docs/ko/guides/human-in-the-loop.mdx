...
이 가이드는 SDK 에 내장된 휴먼인더루프 (HITL) 지원을 사용해 사람의 개입에 따라 에이전트 실행을 일시 중지하고 재개하는 방법을 보여줍니다.

현재 주요 사용 사례는 민감한 도구 실행에 대한 승인을 요청하는 것입니다.

## 승인 요청

`needsApproval` 옵션을 `true` 또는 boolean 을 반환하는 async 함수로 설정해 승인이 필요한 도구를 정의할 수 있습니다.
...
2. 승인이 필요한 경우, 에이전트는 승인이 이미 허용 또는 거부되었는지 확인합니다
...
아래는 터미널에서 승인을 요청하고 상태를 파일에 임시로 저장하는 휴먼인더루프 흐름의 좀 더 완전한 예시입니다.
...
휴먼인더루프 흐름은 서버를 계속 실행하지 않고도 오랜 시간 동안 인터럽트될 수 있도록 설계되었습니다. 요청을 종료하고 나중에 계속해야 하는 경우 상태를 직렬화해 두었다가 나중에 재개할 수 있습니다.
...
승인 요청에 오랜 시간이 걸리고, 에이전트 정의를 의미 있게 버전 관리하거나 Agents SDK 버전을 올릴 계획이라면, 패키지 별칭을 사용해 두 버전의 Agents SDK 를 병렬로 설치하고 자체 분기 로직을 구현하는 것을 권장합니다.

실무적으로는 코드에 자체 버전 번호를 부여하고 이를 직렬화된 상태와 함께 저장하며, 역직렬화가 코드의 올바른 버전으로 향하도록 안내하는 것을 의미합니다.
