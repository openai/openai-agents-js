...
이 가이드는 SDK의 기본 제공 휴먼인더루프 (HITL) 지원을 사용하여 사람의 개입에 따라 에이전트 실행을 일시 중지하고 재개하는 방법을 보여줍니다.
...
2. 승인이 필요한 경우, 에이전트는 이미 승인 또는 거절되었는지 확인합니다
   - 승인이 부여되거나 거절되지 않은 경우, 도구 호출을 실행할 수 없다는 정적 메시지를 에이전트에 반환합니다
   - 승인/거절이 없는 경우 도구 승인 요청을 트리거합니다
...
휴먼인더루프 (HITL) 흐름은 서버를 계속 가동하지 않고도 장기간 인터럽션이 가능하도록 설계되었습니다. 요청을 종료하고 나중에 계속해야 하는 경우 상태를 직렬화하여 이후에 재개할 수 있습니다.
...
승인 요청에 시간이 오래 걸리고, 에이전트 정의에 의미 있는 방식으로 버전을 매기거나 Agents SDK 버전을 올릴 예정이라면, 패키지 별칭을 사용해 두 버전의 Agents SDK를 병렬로 설치하여 자체 브랜칭 로직을 구현하는 것을 권장합니다.

실제로는 코드에 자체 버전 번호를 부여하고 이를 직렬화된 상태와 함께 저장하며, 역직렬화가 코드의 올바른 버전으로 향하도록 안내하는 것을 의미합니다.
