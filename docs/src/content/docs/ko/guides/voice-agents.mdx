...
Voice Agents는 OpenAI의 speech-to-speech 모델을 사용해 실시간 음성 채팅을 제공합니다. 이들 모델은 스트리밍 오디오, 텍스트, 도구 호출을 지원하며, 음성/전화 고객 지원, 모바일 앱 경험, 음성 채팅 같은 애플리케이션에 적합합니다.
...
<LinkCard
  ...
  description="OpenAI Agents SDK로 단 몇 분 만에 첫 실시간 음성 어시스턴트를 구축하세요."
/>
...
- 스트리밍된 이벤트에 대한 콜백
- 텍스트 및 음성 에이전트 모두에 동일한 구성 요소 재사용

speech-to-speech 모델을 사용하면, 모델이 동작한 후 텍스트를 다시 오디오로 변환하기 위해 전사할 필요 없이 모델의 오디오 처리 능력을 실시간으로 활용할 수 있습니다.
