...
<LinkCard
  ...
  description="OpenAI Agents SDK를 사용해 몇 분 만에 첫 실시간 음성 어시스턴트를 구축하세요."
/>
...
- 세션 설정, 도구, 가드레일, 인터럽션, 히스토리 관리를 위한 [음성 에이전트 구축](/openai-agents-js/ko/guides/voice-agents/build)
- WebRTC/WebSocket/SIP 전송 선택과 커스텀 전송을 위한 [전송 방식](/openai-agents-js/ko/guides/voice-agents/transport)

## 음성-대-음성 사용 이유

음성-대-음성 모델을 사용하면, 모델이 동작한 후 텍스트로 전사하고 다시 오디오로 재변환할 필요 없이 오디오를 실시간으로 처리하는 모델의 능력을 활용할 수 있습니다.

![음성-대-음성 모델](https://cdn.openai.com/API/docs/images/diagram-chained-agent.png)
