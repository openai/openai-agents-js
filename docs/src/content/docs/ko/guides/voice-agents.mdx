...
음성 에이전트는 OpenAI 음성-음성 모델을 사용해 실시간 음성 채팅을 제공합니다. 이 모델은 오디오, 텍스트, 도구 호출의 스트리밍을 지원하며 음성/전화 고객 지원, 모바일 앱 경험, 음성 채팅 같은 애플리케이션에 적합합니다.
...
<LinkCard
  ...
  description="OpenAI Agents SDK를 사용해 단 몇 분 만에 첫 실시간 음성 비서를 만드세요."
/>
...
- 스트리밍 이벤트에 대한 콜백
- 텍스트 에이전트와 음성 에이전트 모두에 동일한 컴포넌트 재사용
