...
기본적으로 SDK는 처음 import될 때 `OPENAI_API_KEY` 환경 변수를 읽습니다. 해당 변수를 설정할 수 없다면 `setDefaultOpenAIKey()`를 수동으로 호출할 수 있습니다.
...
직접 생성한 `OpenAI` 클라이언트 인스턴스를 전달할 수도 있습니다. 그렇지 않으면 SDK가 기본 키를 사용하여 자동으로 생성합니다.
...
`@openai/agents`의 `getLogger(namespace)`를 사용하여 모듈 전용 네임스페이스 로거를 얻을 수 있습니다.
