...
직접 만든 `OpenAI` 클라이언트 인스턴스를 전달할 수도 있습니다. 그렇지 않으면 SDK가 기본 키를 사용하여 자동으로 생성합니다.
...
트레이싱 기능에 대해 더 알고 싶다면 [트레이싱](/openai-agents-js/ko/guides/tracing)을 확인하세요.
...
LLM 입력과 출력을 로그하지 않으려면:

```bash
export OPENAI_AGENTS_DONT_LOG_MODEL_DATA=1
```

도구 입력과 출력을 로그하지 않으려면:
