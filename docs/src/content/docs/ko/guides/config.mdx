...
이 페이지에서는 앱 시작 시 보통 한 번만 설정하는 **SDK 전반 기본값**을 다룹니다. 예를 들어 기본 OpenAI 클라이언트, 전송 방식, 트레이싱 내보내기 키, 디버그 로깅 동작 등이 있습니다. 이러한 설정은 기본적으로 프로세스 전체에 적용되므로, 에이전트별 또는 실행별 튜닝보다 전역 설정에 적합합니다.
...
- 실행별 트레이싱 구성 및 exporter 동작은 [트레이싱](/openai-agents-js/ko/guides/tracing)을 참고하세요

## OpenAI 클라이언트 및 전송 방식

### API 키 및 클라이언트

기본적으로 SDK 는 OpenAI 클라이언트를 생성해야 할 때 지연 방식으로 `OPENAI_API_KEY`를 확인합니다. 환경 변수를 설정할 수 없다면 `setDefaultOpenAIKey()`를 수동으로 호출하세요.
...
WebSocket 전송을 활성화하려면 `setOpenAIResponsesTransport('websocket')`를 사용하고, 다시 전환하려면 `setOpenAIResponsesTransport('http')`를 사용하세요. 프록시 또는 게이트웨이를 통해 websocket 트래픽을 라우팅한다면 `OPENAI_WEBSOCKET_BASE_URL`을 설정하세요(또는 `OpenAIProvider`에서 `websocketBaseURL`을 설정하세요).

이 프로세스 전체 기본값은 이후 기본 OpenAI provider 를 통해 확인되는 모델에만 영향을 줍니다. 구체적인 `Model` 인스턴스나 사용자 지정 `modelProvider`를 전달하는 경우에는 해당 위치에서 전송 방식을 설정하세요. [모델 가이드](/openai-agents-js/ko/guides/models#responses-websocket-transport)를 참고하세요.

## 관측 가능성 및 디버깅

### 트레이싱

지원되는 서버 런타임에서는 기본적으로 트레이싱이 활성화됩니다. 브라우저와 `NODE_ENV=test`에서는 기본적으로 비활성화됩니다.

기본적으로 트레이스 내보내기는 위 섹션과 동일한 OpenAI 키를 사용합니다.

`setTracingExportApiKey()`를 통해 별도의 키를 설정할 수 있습니다:
