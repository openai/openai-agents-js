...
기본적으로 SDK는 처음 임포트될 때 환경 변수 `OPENAI_API_KEY`를 읽습니다. 해당 변수를 설정할 수 없다면 `setDefaultOpenAIKey()` 를 수동으로 호출할 수 있습니다.
...
또한 직접 생성한 `OpenAI` 클라이언트 인스턴스를 전달할 수 있습니다. 그렇지 않으면 SDK가 기본 키를 사용해 자동으로 생성합니다.
...
마지막으로 Responses API와 Chat Completions API 중에서 전환할 수 있습니다.
