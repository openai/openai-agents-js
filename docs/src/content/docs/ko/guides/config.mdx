...
기본적으로 SDK는 처음 import될 때 `OPENAI_API_KEY` 환경 변수를 읽습니다. 이 변수를 설정할 수 없다면 `setDefaultOpenAIKey()`를 수동으로 호출할 수 있습니다.
...
일부 로그에는 user 데이터가 포함될 수 있습니다. 다음 환경 변수를 설정하여 비활성화하세요.
