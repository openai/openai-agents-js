...
- Cloudflare 전송은 내부적으로 `Upgrade: websocket` 헤더와 함께 `fetch()`를 사용하며, 소켓의 `open` 이벤트를 기다리지 않아 workerd API와 일치합니다
- 이 전송을 사용할 때 모든 `RealtimeSession` 기능(도구, 가드레일 등)이 평소처럼 동작합니다
