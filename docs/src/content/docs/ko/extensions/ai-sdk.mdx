...
<Aside type="caution">
  이 어댑터는 아직 베타입니다. 특히 소규모 모델 프로바이더와 함께 사용할 때
  문제가 발생할 수 있습니다. [GitHub
...
<Aside type="caution">
  ...
  특정한 이유로 v1 model providers를 계속 사용해야 하는 경우
...
메시지에 프로바이더 전용 옵션을 보내야 하는 경우 `providerMetadata`를 통해 전달하세요. 값은 하위 AI SDK 모델로 그대로 전달됩니다. 예를 들어, Agents SDK에서 다음 `providerData`
