...
<Aside type="caution">
  이 어댑터는 아직 베타입니다. 특히 규모가 작은 모델 프로바이더에서는 문제가
  발생할 수 있습니다. 문제가 있으면 [GitHub
  issues](https://github.com/openai/openai-agents-js/issues)로 보고해 주세요.
  빠르게 수정하겠습니다.
</Aside>

기본적으로 Agents SDK는 Responses API 또는 Chat Completions API를 통해 OpenAI 모델과 함께 작동합니다. 그러나 다른 모델을 사용하고 싶다면, 이 어댑터를 통해 [Vercel's AI SDK](https://sdk.vercel.ai/)에서 지원하는 다양한 모델을 Agents SDK와 함께 사용할 수 있습니다.
...
메시지와 함께 프로바이더별 옵션을 전송해야 하는 경우 `providerMetadata`를 사용하세요. 값은 하위 AI SDK 모델로 그대로 전달됩니다. 예를 들어, Agents SDK에서 아래의 `providerData`
...
는 AI SDK 통합을 사용할 때 다음과 같이 변경됩니다
