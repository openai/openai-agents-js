...
<Aside type="caution">
  이 어댑터는 아직 베타 버전입니다. 일부 모델 provider, 특히 규모가 작은
  provider에서 문제가 발생할 수 있습니다. 문제를 [GitHub
  이슈](https://github.com/openai/openai-agents-js/issues)로 제보해 주시면
  빠르게 수정하겠습니다.
</Aside>

기본적으로 Agents SDK는 Responses API 또는 Chat Completions API를 통해 OpenAI 모델과 함께 동작합니다. 하지만 다른 모델을 사용하려는 경우, [Vercel AI SDK](https://sdk.vercel.ai/)는 지원되는 다양한 모델을 제공하며 이 어댑터를 통해 Agents SDK에 연결할 수 있습니다.
...
<Aside type="caution">
  `specificationVersion` `v2` 또는 `v3`를 노출하는 AI SDK provider를 지원합니다.
  특정 이유로 이전 v1 provider 스타일을 계속 사용해야 한다면,
...
AI SDK 연동을 사용할 때 위와 같이 동작합니다.

## 최종 출력 텍스트 정규화

일부 provider는 구조화된 출력을 JSON 코드 펜스 같은 추가 래핑이 포함된 일반 텍스트로 반환합니다. Agents 런타임이 최종 출력을 검증하기 전에 provider별 정리가 필요하다면, 어댑터 생성 시 `transformOutputText`를 전달하세요.
...
두 헬퍼 모두 `StreamedRunResult`, 스트림 유사 소스, 또는 호환되는 래퍼 객체를 받아 스트리밍 친화적 헤더가 포함된 `Response`를 반환합니다.

도구 호출이나 reasoning 파트 같은 구조화된 청크가 UI에 필요하면 `createAiSdkUiMessageStreamResponse(...)`를 사용하세요. 일반 텍스트만 원하면 `createAiSdkTextStreamResponse(...)`를 사용하세요.
...
엔드 투 엔드 사용 방법은 이 리포지토리의 `examples/ai-sdk-ui` 앱을 참고하세요.
