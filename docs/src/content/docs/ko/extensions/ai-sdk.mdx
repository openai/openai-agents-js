...
<Aside type="caution">
  이 어댑터는 아직 베타 단계입니다. 특히 규모가 작은 일부 모델 프로바이더에서
...
<Aside type="caution">
  ...
  호환됩니다. 특별한 사유로 v1 model providers를 계속 사용해야 한다면
...
메시지에 프로바이더별 옵션을 전달해야 한다면 `providerMetadata`를 사용하세요. 값은 기본 AI SDK 모델로 그대로 전달됩니다. 예를 들어, Agents SDK에서 다음과 같은 `providerData`는
