...
Cloudflare Workers 和其他 workerd 运行时无法使用全局 `WebSocket` 构造函数发起出站 WebSocket 连接。为便于在这些环境中连接实时智能体，扩展包提供了一个专用传输，在内部通过基于 `fetch()` 的升级来完成连接。

<Aside type="caution">
  此适配器仍处于测试版。您可能会遇到边界情况或 bug。 请通过 [GitHub
