...
Cloudflare Workers 和其他 workerd 运行时无法使用全局 `WebSocket` 构造函数发起出站 WebSocket 连接。为便于在这些环境中连接 实时智能体，扩展包提供了一个专用传输机制，在内部通过基于 `fetch()` 的升级完成连接。

<Aside type="caution">
  该适配器仍处于测试阶段。您可能会遇到边缘情况或漏洞。 请通过 [GitHub
...
- Cloudflare 传输在底层使用带有 `Upgrade: websocket` 的 `fetch()`，并跳过等待套接字的 `open` 事件，与 workerd API 一致。
