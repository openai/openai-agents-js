...
Cloudflare Workers 和其他 workerd 运行时无法使用全局 `WebSocket` 构造函数发起向外的 WebSocket 连接。为简化在这些环境中连接 Realtime Agents，扩展包提供了一个专用传输，它在内部执行基于 `fetch()` 的升级。

<Aside type="caution">
  此适配器仍处于测试版。你可能会遇到边缘情况或错误。 请通过 [GitHub
  issues](https://github.com/openai/openai-agents-js/issues)
  报告任何问题，我们会尽快修复。对于在 Workers 中使用 Node.js 风格 API
  的场景，考虑启用 `nodejs_compat`。
</Aside>
...
- Cloudflare 传输在底层使用带有 `Upgrade: websocket` 的 `fetch()`，并跳过等待套接字的 `open` 事件，以匹配 workerd 的 API。
- 使用此传输时，所有 `RealtimeSession` 功能（工具、护栏等）都可照常工作。
