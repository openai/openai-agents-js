...
Cloudflare Workers 和其他 workerd 运行时无法使用全局 `WebSocket` 构造函数发起向外的 WebSocket 连接。为便于在这些环境中连接实时智能体（Realtime Agents），扩展包提供了一个专用传输机制，在内部通过基于 `fetch()` 的升级流程完成连接。

<Aside type="caution">
  该适配器仍处于测试阶段（beta）。您可能会遇到边缘情况或缺陷。 请通过 [GitHub
  issues](https://github.com/openai/openai-agents-js/issues)
  报告问题，我们会尽快修复。 若需在 Workers 中使用 Node.js 风格 API，可考虑启用
  `nodejs_compat`。
</Aside>
...
- Cloudflare 传输在底层使用带有 `Upgrade: websocket` 的 `fetch()`，并跳过等待套接字 `open` 事件的步骤，以匹配 workerd API。
