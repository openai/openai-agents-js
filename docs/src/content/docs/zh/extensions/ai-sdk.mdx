...
<Aside type="caution">
  此适配器仍处于测试版。您可能会在某些模型提供商上遇到问题，尤其是规模较小的提供商。请通过[GitHub
  issues](https://github.com/openai/openai-agents-js/issues)反馈任何问题，我们会尽快修复。
</Aside>

Agents SDK 默认通过 Responses API 或 Chat Completions API 与 OpenAI 模型协同工作。不过，如果您希望使用其他模型，[Vercel AI SDK](https://sdk.vercel.ai/)提供了一系列受支持的模型，可通过此适配器接入 Agents SDK。
...
<Aside type="caution">
  我们支持暴露 `specificationVersion` `v2` 或 `v3` 的 AI SDK
  提供商。如果您有明确原因需要继续使用较旧的 v1
  提供商风格，可以从[examples/ai-sdk-v1](https://github.com/openai/openai-agents-js/tree/main/examples/ai-sdk-v1)复制该模块并将其包含到您的项目中。
</Aside>
...
如果您需要随消息发送提供商特定选项，请通过 `providerMetadata` 传递。其值会直接转发到底层 AI SDK 模型。例如，在 Agents SDK 中如下 `providerData`
...
某些提供商会将 structured outputs 以带额外包裹的纯文本返回，例如 JSON 代码围栏。如果您需要在 Agents 运行时校验最终输出之前进行提供商特定清理，请在创建适配器时传入 `transformOutputText`：
...
对于非流式响应，`transformOutputText` 会在最终 assistant 文本上运行；对于流式响应，则会在最终 `response_done` 事件上运行。它不会修改增量 `output_text_delta` 事件。
...
- `@openai/agents-extensions/ai-sdk-ui` 会适配流式 Agents SDK 运行结果，使 AI SDK UI 路由可以返回标准流式 `Response`。
...
这两个辅助函数都接受 `StreamedRunResult`、类流式 source 或兼容的包装对象，并返回带有流式友好标头的 `Response`。

当您的 UI 需要结构化分块（如工具调用或推理片段）时，请使用 `createAiSdkUiMessageStreamResponse(...)`。当您只需要纯文本时，请使用 `createAiSdkTextStreamResponse(...)`。
