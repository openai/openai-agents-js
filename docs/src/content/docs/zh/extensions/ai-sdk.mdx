...
<Aside type="caution">
  此适配器仍处于测试阶段。您在使用部分模型提供方（尤其是较小的提供方）时可能会遇到问题。请通过
  [GitHub issues](https://github.com/openai/openai-agents-js/issues)
  报告问题，我们会尽快修复。
</Aside>

开箱即用，Agents SDK 可通过 Responses API 或 Chat Completions API 与 OpenAI 模型配合使用。不过，如果您想使用其他模型，可以通过此适配器将[Vercel 的 AI SDK](https://sdk.vercel.ai/)提供的一系列受支持模型引入 Agents SDK。
...
<Aside type="caution">
  我们目前支持 ai-sdk 的 model provider v2 模块，兼容 Vercel AI SDK
  v5。若您有特定原因需要继续使用 v1 model providers，可以从
...
如果需要随消息发送提供方特定的选项，请通过 `providerMetadata` 传递。这些值会直接转发给底层的 AI SDK 模型。例如，在 Agents SDK 中如下的 `providerData`
