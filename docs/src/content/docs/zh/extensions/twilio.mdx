...
2. **搭建一个可接收来自 Twilio 事件的 WebSocket 服务器。**
   ...
   让 Twilio 能访问您的本地服务器。您可以使用 `TwilioRealtimeTransportLayer`
   连接到 Twilio。
...
`RealtimeSession` 中您期望的任何事件与行为都将如常工作，包括工具调用、护栏等。阅读[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)了解如何将 `RealtimeSession` 用于语音智能体的更多信息。

## 提示与注意事项

1. **速度至关重要。**

   为了接收来自 Twilio 的所有必要事件与音频，应在拿到 WebSocket 连接引用后尽快创建
   `TwilioRealtimeTransportLayer` 实例，并立即调用 `session.connect()`。

2. **访问原始 Twilio 事件。**

   如果您想访问 Twilio 发送的原始事件，可以在 `RealtimeSession` 实例上监听
   `transport_event` 事件。来自 Twilio 的每个事件都有 `twilio_message` 类型，并带有包含原始事件数据的 `message` 属性。

3. **查看调试日志。**

   有时您可能需要更多信息来排查问题。使用环境变量 `DEBUG=openai-agents*` 将显示来自 Agents SDK 的所有调试日志。或者，您也可以使用
