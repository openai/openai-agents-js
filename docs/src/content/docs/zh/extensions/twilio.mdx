...
2. **设置一个可接收来自 Twilio 事件的 WebSocket 服务器。**
   ...
   如果您在本地开发，则需要配置一个本地隧道，例如使用 [`ngrok`](https://ngrok.io/) 或
   [Cloudflare Tunnel](https://developers.cloudflare.com/pages/how-to/preview-with-cloudflare-tunnel/)
   使本地服务器可供 Twilio 访问。您可以使用 `TwilioRealtimeTransportLayer`
   来连接 Twilio。
...
您期望从 `RealtimeSession` 获得的任何事件和行为都会按预期工作，包括工具调用、护栏等。阅读[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)，了解如何将 `RealtimeSession` 与语音智能体配合使用的更多信息。
...
2. **访问原始 Twilio 事件。**
   ...
   `transport_event` 事件。来自 Twilio 的每个事件的类型为 `twilio_message`，并带有包含原始事件数据的 `message` 属性。

3. **查看调试日志。**

   有时您可能会遇到需要更多信息的问题。使用环境变量 `DEBUG=openai-agents*` 将显示来自 Agents SDK 的所有调试日志。
   或者，您也可以只为 Twilio 适配器启用调试日志：`DEBUG=openai-agents:extensions:twilio*`。
