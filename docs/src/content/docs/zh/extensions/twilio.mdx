...
2. **设置一个可接收来自 Twilio 事件的 WebSocket 服务器。**
   ...
   如果您在本地开发，这将需要您配置一个本地隧道，如 [`ngrok`](https://ngrok.io/) 或
...
您对 `RealtimeSession` 期望的任何事件和行为都将按预期工作，包括工具调用、护栏等。阅读[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)，了解如何将 `RealtimeSession` 与语音智能体一起使用的更多信息。
...
2. **访问原始 Twilio 事件。**
   ...
   如果您想访问 Twilio 发送的原始事件，您可以监听 `RealtimeSession` 实例上的
   `transport_event` 事件。来自 Twilio 的每个事件的类型为 `twilio_message`，并包含一个
   `message` 属性，其中含有原始事件数据。

3. **查看调试日志。**

   有时您可能会遇到需要更多信息的问题。使用 `DEBUG=openai-agents*` 环境变量将显示来自 Agents SDK 的所有调试日志。或者，您也可以仅启用 Twilio 适配器的调试日志：`DEBUG=openai-agents:extensions:twilio*`。
