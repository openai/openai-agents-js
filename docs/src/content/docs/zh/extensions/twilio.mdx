...
2. **设置一个可以接收来自 Twilio 事件的 WebSocket 服务器。**
   ...
   如果您在本地开发，这将需要配置一个本地隧道，例如 [`ngrok`](https://ngrok.io/) 或
...
任何您对 `RealtimeSession` 期望的事件和行为都会如期工作，包括工具调用、护栏等。阅读[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)以了解更多在语音智能体中使用 `RealtimeSession` 的信息。
...
2. **访问原始 Twilio 事件。**
   ...
   `transport_event` 事件。来自 Twilio 的每个事件的类型为 `twilio_message`，并包含一个 `message` 属性保存原始事件数据。

3. **查看调试日志。**

   有时您可能会遇到需要更多信息的问题。使用环境变量 `DEBUG=openai-agents*` 将显示来自 Agents SDK 的所有调试日志。或者，您也可以仅为 Twilio 适配器启用调试日志：
