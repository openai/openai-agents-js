...
Twilio 提供了一个 [Media Streams API](https://www.twilio.com/docs/voice/media-streams)，可将电话通话中的原始音频发送到一个 WebSocket 服务器。该设置可用于将您的[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)连接到 Twilio。您可以使用默认的 Realtime Session 传输，在 `websocket` 模式下将来自 Twilio 的事件连接到您的 Realtime Session。不过，这需要您设置正确的音频格式，并根据需要调整中断时机，因为与基于网页的对话相比，电话通话自然会引入更多延迟。

为改进设置体验，我们创建了一个专用传输层，为您处理与 Twilio 的连接，包括处理中断和音频转发。
...
1. **确保您拥有一个 Twilio 账号和一个 Twilio 电话号码。**

2. **设置一个可以接收来自 Twilio 事件的 WebSocket 服务器。**

   如果您在本地开发，这将需要您配置一个本地隧道，例如使用 [`ngrok`](https://ngrok.io/) 或
...
您期望从 `RealtimeSession` 获得的任何事件和行为都将按预期工作，包括工具调用、护栏等。阅读[语音智能体概述](/openai-agents-js/zh/guides/voice-agents)以了解如何将 `RealtimeSession` 与语音智能体一起使用的更多信息。
...
2. **访问原始 Twilio 事件。**
   ...
   `transport_event` 事件。来自 Twilio 的每个事件都会有一个类型 `twilio_message`，以及一个包含原始事件数据的 `message` 属性。
