...
- [构建语音智能体](/openai-agents-js/zh/guides/voice-agents/build)：涵盖会话设置、工具、护栏、打断与历史记录管理。
- [传输机制](/openai-agents-js/zh/guides/voice-agents/transport)：介绍 WebRTC/WebSocket/SIP 的传输选择与自定义传输。

## 为什么选择语音到语音

通过使用语音到语音模型，我们可以在无需先转写为文本并在模型响应后再转换回音频的情况下，直接利用模型对音频进行实时处理的能力。
