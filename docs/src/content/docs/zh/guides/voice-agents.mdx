...
语音智能体使用 OpenAI 的语音到语音模型，提供实时语音对话。这些模型支持音频、文本与工具调用的流式传输，非常适合语音/电话客服、移动应用体验和语音聊天等场景。

Voice Agents SDK 提供了面向 [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime) 的 TypeScript 客户端。

<LinkCard
  title="快速开始"
  href="/openai-agents-js/zh/guides/voice-agents/quickstart"
  description="几分钟内构建您的第一个使用 OpenAI Agents SDK 的实时语音助手。"
/>
...
借助语音到语音模型，我们可以利用模型实时处理音频的能力，而无需在模型执行后进行转录并将文本重新转换为音频。
