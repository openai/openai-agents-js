...
5. 投入于[评测（evals）](https://platform.openai.com/docs/guides/evals)。这可帮助你训练智能体改进并更好地完成任务。

## 通过代码的编排

虽然通过 LLM 编排功能强大，但通过代码编排在速度、成本和性能方面更具确定性和可预测性。常见模式包括：

- 使用[structured outputs](https://platform.openai.com/docs/guides/structured-outputs)生成可由你的代码检查的格式良好的数据。例如，你可以让智能体将任务分类为若干类别，然后基于类别选择下一个智能体。
