...
- **Cloudflare Workers**：Agents SDK 可用于 Cloudflare Workers，但目前存在一些限制：
  ...
  - 需要在请求结束时手动刷新追踪数据。有关更多详情，[查看 追踪](/openai-agents-js/zh/guides/tracing#export-loop-lifecycle)。
  - 由于 Cloudflare Workers 对 `AsyncLocalStorage` 的支持有限，一些追踪可能不准确
...
- **浏览器**：
  - 浏览器中目前不支持追踪
- **v8 isolates**：
  - 如果使用带有合适浏览器 polyfill 的打包工具，应该可以为 v8 isolates 打包 SDK，但追踪将无法工作
