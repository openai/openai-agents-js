...
- **Cloudflare Workers**：Agents SDK 可用于 Cloudflare Workers，但目前存在一些限制：
  ...
  - 需要在请求结束时手动刷新追踪数据。详情请参见[追踪](/openai-agents-js/zh/guides/tracing#export-loop-lifecycle)。
  - 由于 Cloudflare Workers 对 `AsyncLocalStorage` 的支持有限，部分追踪可能不准确
  - 出站 WebSocket 连接必须使用基于 fetch 的升级方式（不能使用全局的 `WebSocket` 构造函数）。对于 Realtime，请使用 `@openai/agents-extensions` 中的 Cloudflare 传输（`CloudflareRealtimeTransportLayer`）。
- **浏览器**：
  - 浏览器当前不支持追踪
- **v8 隔离环境（v8 isolates）**：
  - 即使使用带有合适浏览器 polyfill 的打包器将 SDK 打包到 v8 隔离环境中，追踪也无法工作
  - v8 隔离环境尚未进行充分测试
