...
- **Cloudflare Workers**：Agents SDK 可用于 Cloudflare Workers，但目前存在一些限制：
  ...
  - 需要在请求结束时手动刷新追踪数据。有关详细信息，请[查看追踪指南](/openai-agents-js/zh/guides/tracing#export-loop-lifecycle)。
  - 由于 Cloudflare Workers 对 `AsyncLocalStorage` 的支持有限，部分追踪可能不准确
  - 出站 WebSocket 连接必须使用基于 fetch 的升级方式（而不是全局的 `WebSocket` 构造函数）。对于实时功能，请使用 `@openai/agents-extensions` 中的 Cloudflare 传输（`CloudflareRealtimeTransportLayer`）。
- **浏览器**：
  - 浏览器当前不支持追踪
- **v8 isolates**：
  - 如果使用带有适当浏览器 polyfill 的打包器，您应该可以为 v8 隔离环境打包该 SDK，但追踪将无法工作
  - v8 隔离环境尚未经过广泛测试
