...
- **Cloudflare Workers**：Agents SDK 可用于 Cloudflare Workers，但目前存在一些限制：
  ...
  - 需要在请求结束时手动刷新追踪数据。有关更多详情，请参阅[追踪](/openai-agents-js/zh/guides/tracing#export-loop-lifecycle)。
  - 由于 Cloudflare Workers 对 `AsyncLocalStorage` 的支持有限，部分追踪可能不够准确
  - 出站 WebSocket 连接必须使用基于 fetch 的升级（不能使用全局 `WebSocket` 构造函数）。对于 Realtime，请使用 `@openai/agents-extensions` 中的 Cloudflare 传输（`CloudflareRealtimeTransportLayer`）。
- **Browsers**：
  - 浏览器中当前不支持追踪
- **v8 isolates**：
