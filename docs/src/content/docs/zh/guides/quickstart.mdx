...
这将返回一个运行结果，其中包含最终输出以及该次运行期间执行的所有操作。
...
可以用类似方式定义其他智能体，将问题拆解为更小的部分，让智能体更专注于当前任务。这也允许您为不同问题在智能体上定义不同的模型。
...
为在多个智能体之间编排，您可以为某个智能体定义 `handoffs`。这将使智能体能够将对话交接给下一个智能体。交接会在运行过程中自动发生。
...
运行结束后，您可以通过查看运行结果中的 `finalAgent` 属性来确定哪个智能体生成了最终响应。
...
让我们把以上内容整合为一个完整示例。将以下内容放入 `index.js` 并运行。

<Code lang="typescript" code={quickstartExample} title="Quickstart" />

## 查看追踪

Agents SDK 会自动为您生成追踪。这使您可以回顾智能体的运行方式、调用了哪些工具、以及交接给了哪个智能体。
