...
3. 设置 OpenAI API key。如果还没有，请按照[这些说明](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)创建一个 OpenAI API key。
   ...
   详细请参见[SDK 配置](/openai-agents-js/zh/guides/config)。
...
您可以使用 `run` 方法来运行智能体。通过传入要启动的智能体以及要传递的输入即可触发一次运行。

这将返回一个结果，其中包含最终输出以及在该次运行期间执行的任何操作。
...
为了在多个智能体之间编排，您可以为智能体定义 `handoffs`。这将使智能体能够将对话传递给下一个智能体。这将在运行过程中自动发生。
...
在运行结束后，您可以通过查看结果上的 `finalAgent` 属性来了解哪个智能体生成了最终响应。
...
让我们把以上内容整合成一个完整示例。将其放入 `index.js` 并运行。

<Code lang="typescript" code={quickstartExample} title="Quickstart" />

## 查看追踪

Agents SDK 将自动为您生成追踪。您可以据此审查智能体的运行方式、调用了哪些工具、或交接给了哪个智能体。
