...
3. 设置一个 OpenAI API key。如果还没有，请按照[这些说明](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)创建一个 OpenAI API key。

   ```bash
   export OPENAI_API_KEY=sk-...
   ```

   或者，您可以调用 `setDefaultOpenAIKey('<api key>')` 以编程方式设置 key，并使用 `setTracingExportApiKey('<api key>')` 进行追踪。
...
您可以使用 `run` 方法来运行智能体。传入要启动的智能体以及要传递的输入即可触发一次运行。

这将返回一个结果，其中包含最终输出以及在该次运行中执行的任何操作。
...
可以以类似方式定义更多智能体，将问题拆解为更小的部分，使智能体更专注于手头任务。还可以通过在智能体上定义模型，为不同问题使用不同的模型。
...
为在多个智能体之间进行编排，您可以为某个智能体定义 `handoffs`。这将使该智能体能够将对话传递给下一个智能体。这将在运行过程中自动发生。
...
Agents SDK 会自动为您生成追踪。这使您可以审阅智能体的运行方式、调用了哪些工具或交接给了哪个智能体。
