...
您可以使用 `run` 方法来运行智能体。通过同时传入要启动的智能体和要传入的输入来触发一次运行。
...
可以类似地定义更多智能体，将问题拆分为更小的部分，使智能体更专注于当前任务。通过在智能体上定义模型，您还可以针对不同问题使用不同的模型。
...
Runner（Runner）负责处理各个智能体的执行、潜在的交接以及工具调用。
...
Agents SDK 会自动为您生成追踪。这使您能够审查智能体的运行方式、调用了哪些工具或交接给了哪个智能体。
