...
3. 设置 OpenAI API 密钥。如果还没有，请按照[这些说明](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)创建一个 OpenAI API 密钥。
   ...
   或者，您也可以调用 `setDefaultOpenAIKey('<api key>')` 以编程方式设置密钥，
   并使用 `setTracingExportApiKey('<api key>')` 用于追踪。
...
您可以使用 `run` 方法运行智能体。通过同时传入要启动的智能体和要传递的输入来触发一次运行。

这将返回一个运行结果，其中包含最终输出以及在该次运行期间执行的所有操作。
...
还可以以类似方式定义其他智能体，将问题拆解为更小的部分，让智能体更专注于当前任务。并且可以通过在智能体上定义模型，为不同问题使用不同的模型。
...
为了在多个智能体之间编排，您可以为智能体定义 `handoffs`。这将使智能体能够将对话传递给下一个智能体。在运行过程中这会自动发生。
...
Agents SDK 会自动为您生成追踪。您可以查看智能体的运行情况、调用了哪些工具或交接给了哪个智能体。
