...
某些传输层（如默认的 `OpenAIRealtimeWebRTC`）会自动为你处理音频输入和输出。对于 `OpenAIRealtimeWebSocket` 等其他传输机制，则需要你自行处理会话音频：
...
与常规智能体不同，交接在实时智能体中的行为略有差异。执行交接时，正在进行的会话将使用新的智能体配置进行更新。因此，智能体会自动访问正在进行的对话历史，且当前不会应用输入过滤器。

此外，这意味着在交接过程中不能更改 `voice` 或 `model`。你也只能连接到其他实时智能体。如果你需要使用不同的模型，例如 `gpt-5-mini` 这样的推理模型，你可以使用[通过工具进行委派](#delegation-through-tools)。
...
你只能在实时智能体中使用函数工具，这些工具会与实时会话在同一位置执行。这意味着如果你在浏览器中运行实时会话，你的工具也会在浏览器中执行。如果需要执行更敏感的操作，你可以在工具内部向后端服务器发起 HTTP 请求。

在工具执行期间，智能体将无法处理来自用户的新请求。改善体验的一种方式是让你的智能体在即将执行工具时进行提示，或说出特定短语以为工具执行争取时间。

### 访问对话历史

除了智能体调用某个工具时传入的参数，你还可以访问由实时会话跟踪的当前对话历史快照。如果你需要基于会话当前状态执行更复杂的操作，或计划使用[通过工具进行委派](#delegation-through-tools)，这会很有用。
...
护栏提供一种方式来监控智能体的发言是否违反一组规则，并立即切断响应。这些护栏检查基于智能体响应的转录进行，因此要求启用模型的文本输出（默认启用）。

你提供的护栏会在模型响应返回时异步运行，使你能够基于预定义的分类触发器（例如“提到特定禁词”）来切断响应。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供一个包含触发该护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符或在响应文本生成结束时运行一次。由于朗读文本通常更耗时，这意味着在大多数情况下，护栏应能在用户听到之前捕获违规。
...
实时会话会自动检测用户何时开始说话，并使用内置的 [Realtime API 的语音活动检测模式](https://platform.openai.com/docs/guides/realtime-vad)触发新的轮次。
...
使用内置的语音活动检测时，覆盖智能体说话会自动触发智能体根据所说内容进行检测并更新其上下文。它还会发出 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。

<Code lang="typescript" code={audioInterruptedExample} />

如果你想手动执行打断，例如在 UI 中提供“停止”按钮，你可以手动调用 `interrupt()`：

<Code lang="typescript" code={sessionInterruptExample} />

无论哪种方式，实时会话都会处理打断智能体的生成、截断其已对用户说过内容的认知，并更新历史。
...
你可以用它来向客户渲染历史或对其执行额外操作。由于该历史会在对话过程中持续变化，你可以监听 `history_updated` 事件。
...
通过将对话历史与工具调用相结合，你可以将对话委派给另一个后端智能体以执行更复杂的操作，然后将其结果返回给用户。
