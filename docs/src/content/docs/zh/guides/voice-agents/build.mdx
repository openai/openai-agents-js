...
你可以在构造时向 [`RealtimeSession`](/openai-agents-js/openai/agents-realtime/classes/realtimesession/) 传递其他选项，或在调用 `connect(...)` 时进行配置。
...
对于新增且在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中没有对应参数的参数，你可以使用 `providerData`。传入 `providerData` 的任何内容都会作为 `session` 对象的一部分直接传递。
...
| `groupId`                                     | `string`                          | 跨会话或后端运行聚合追踪。                           |
| `traceMetadata`                               | `Record<string, any>`             | 要附加到会话追踪的自定义元数据。                     |
...
| 选项     | 类型                                          | 目的                                    |
| -------- | --------------------------------------------- | --------------------------------------- |
| `apiKey` | `string \| (() => string \| Promise<string>)` | 此连接所用的 API 密钥（或延迟加载器）。 |
| `model`  | `OpenAIRealtimeModels \| string`              | 传输连接的可选模型覆盖。                |
| `url`    | `string`                                      | 可选的自定义 Realtime 端点 URL。        |
| `callId` | `string`                                      | 附加到已有的 SIP 发起的通话/会话。      |
...
与常规智能体不同，交接在 Realtime Agents 中的行为略有差异。执行交接时，正在进行的会话会更新为新的智能体配置。因此，智能体会自动访问正在进行的对话历史，并且当前不应用输入过滤器。

此外，这意味着在交接过程中不能更改 `voice` 或 `model`。你也只能连接到其他 Realtime Agents。如果你需要使用不同的模型，例如像 `gpt-5-mini` 这样的推理模型，你可以使用[通过工具的委托](#delegation-through-tools)。

### 工具

与常规智能体一样，Realtime Agents 可以调用工具来执行操作。Realtime 支持**函数工具**（本地执行）和**托管 MCP 工具**（由 Realtime API 远程执行）。你可以使用与常规智能体相同的 `tool()` 帮助器来定义函数工具。

<Code lang="typescript" code={defineToolExample} />

函数工具在与你的 `RealtimeSession` 相同的环境中运行。这意味着如果你在浏览器中运行会话，工具也会在浏览器中执行。如果你需要执行敏感操作，请在工具内向你的后端发起 HTTP 请求。

托管 MCP 工具可通过 `hostedMcpTool` 进行配置并远程执行。当 MCP 工具可用性发生变化时，会话会触发 `mcp_tools_changed` 事件。若要在 MCP 工具调用完成后阻止会话自动触发模型响应，请设置 `automaticallyTriggerResponseForMcpToolCalls: false`。

在工具执行期间，智能体无法处理来自用户的新请求。改善体验的一种方式是让你的智能体在即将执行工具时提前告知，或说出特定的短语，以争取时间执行工具。

如果函数工具应当在完成后不立即触发下一次模型响应，请从 `@openai/agents/realtime` 返回 `backgroundResult(output)`。这会将工具输出发送回会话，同时将是否触发响应的控制权保留在你这边。

函数工具超时选项（`timeoutMs`、`timeoutBehavior`、`timeoutErrorFunction`）在 Realtime 会话中工作方式相同。使用默认的 `error_as_result` 时，超时消息将作为工具输出发送。使用 `raise_exception` 时，会话会发出带有 [`ToolTimeoutError`](/openai-agents-js/openai/agents-core/classes/tooltimeouterror) 的 `error` 事件，并且不会为该次调用发送工具输出。

#### 访问对话历史

除了智能体调用特定工具时传入的参数外，你还可以访问由 Realtime Session 追踪的当前对话历史快照。如果你需要基于当前对话状态执行更复杂的操作，或计划[通过工具进行委托](#delegation-through-tools)，这将很有用。

<Code lang="typescript" code={toolHistoryExample} />

<Aside type="note">
  传入的历史记录是在工具调用时的快照。用户最后说的话的转录可能尚不可用。
</Aside>

#### 工具执行前审批

如果你以 `needsApproval: true` 定义你的工具，智能体会在执行工具之前触发 `tool_approval_requested` 事件。

通过监听此事件，你可以向用户显示 UI 以批准或拒绝工具调用。
...
护栏提供了一种方式来监控智能体的发言是否违反一组规则，并立即切断响应。这些护栏检查将基于智能体响应的转录执行，因此需要启用模型的文本输出（默认已启用）。

你提供的护栏会在模型响应返回时异步运行，允许你基于预定义的分类触发器（例如“提到特定的禁用词”）来切断响应。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供一个包含触发护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏会每 100 个字符或在响应文本末尾运行一次。由于朗读文本通常更耗时，这意味着在大多数情况下，护栏应能在用户听到之前捕获违规。
...
Realtime Session 会自动检测用户何时在说话，并使用 Realtime API 的内置[语音活动检测模式](https://platform.openai.com/docs/guides/realtime-vad)来触发新的轮次。
...
使用内置的语音活动检测时，用户打断智能体说话会自动触发智能体根据所说内容检测并更新其上下文。它还会发出 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。

<Code lang="typescript" code={audioInterruptedExample} />

如果你想执行手动打断，例如在 UI 中提供一个“停止”按钮，可以手动调用 `interrupt()`：

<Code lang="typescript" code={sessionInterruptExample} />

无论哪种方式，Realtime Session 都会处理对智能体生成的打断，截断其对向用户所说内容的认知，并更新历史记录。

如果你使用 WebRTC 连接智能体，它也会清除音频输出。如果你使用 WebSocket，则需要自行处理，停止已排队待播放的音频的播放。
...
你可以使用它向客户渲染历史或对其执行其他操作。由于在对话过程中历史会不断变化，你可以监听 `history_updated` 事件。
...
通过将对话历史与工具调用结合起来，你可以将对话委托给另一个后端智能体以执行更复杂的操作，然后将结果返回给用户。
