...
你可以在构造时向 [`RealtimeSession`](/openai-agents-js/openai/agents-realtime/classes/realtimesession/) 传入其他选项，或在调用 `connect(...)` 时配置会话。
...
对于新增且在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中尚无对应参数的配置，你可以使用 `providerData`。传入 `providerData` 的任何内容都会作为 `session` 对象的一部分直接传递。
...
与常规智能体不同，交接在实时智能体中的行为略有差异。执行交接时，正在进行的会话会更新为新的智能体配置。因此，智能体会自动访问当前的对话历史，并且当前不会应用输入过滤器。

此外，这意味着在交接中不能更改 `voice` 或 `model`。你也只能连接到其他实时智能体。如果你需要使用不同的模型，例如推理模型 `gpt-5-mini`，可以使用[通过工具进行委派](#delegation-through-tools)。

## 工具

与常规智能体一样，实时智能体可以调用工具以执行操作。你可以使用与常规智能体相同的 `tool()` 函数来定义工具。

<Code lang="typescript" code={defineToolExample} />

你只能在实时智能体中使用函数工具，这些工具会在与你的 Realtime 会话相同的位置执行。这意味着如果你的 Realtime 会话在浏览器中运行，你的工具也会在浏览器中执行。如果需要执行更敏感的操作，可以在工具内部向你的后端服务器发起 HTTP 请求。

在工具执行期间，智能体将无法处理来自用户的新请求。提升体验的一种方式是让智能体在即将执行工具时进行提示，或说出特定短语，为执行工具争取时间。

### 访问对话历史

除了智能体调用特定工具时传入的参数外，你还可以访问 Realtime 会话跟踪的当前对话历史快照。如果你需要基于对话当前状态执行更复杂的操作，或计划使用[用于委派的工具](#delegation-through-tools)，这会很有用。
...
护栏提供了一种方式来监控智能体的输出是否违反了一组规则，并在需要时立即切断响应。这些护栏检查基于智能体响应的转录文本执行，因此需要启用模型的文本输出（默认已启用）。

你提供的护栏会在模型返回响应时异步运行，允许你基于预定义的分类触发条件来切断响应，例如“提及特定禁用词”。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供一个包含触发该护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符或在响应文本生成结束时运行。由于朗读文本通常更耗时，这意味着在大多数情况下，护栏应能在用户听到之前捕获到违规内容。
...
修改轮次检测设置有助于校准不期望的打断并处理静音情况。查看[Realtime API 文档以了解不同设置的更多细节](https://platform.openai.com/docs/guides/realtime-vad)

## 打断

使用内置语音活动检测时，与智能体“抢话”会自动触发智能体根据用户所说更新其上下文。它还会触发一个 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。
...
无论哪种方式，Realtime 会话都会处理对智能体生成的打断、截断其对已向用户输出内容的认知，并更新历史。
...
如果你希望让用户以两种模态与智能体交互，或为对话提供额外上下文，这将很有用。
...
你可以用它向客户渲染历史，或对其执行其他操作。由于在对话过程中历史会不断变化，你可以监听 `history_updated` 事件。
...
通过将对话历史与一次工具调用相结合，你可以将对话委派给另一个后端智能体来执行更复杂的操作，然后将结果返回给用户。

<Code lang="typescript" code={delegationAgentExample} />

下面的代码将在服务器上执行。本示例通过 Next.js 的 server actions。
