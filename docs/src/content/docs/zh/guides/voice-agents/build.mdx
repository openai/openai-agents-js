...
您可以在构造期间向[`RealtimeSession`](/openai-agents-js/openai/agents-realtime/classes/realtimesession/)传入额外选项，或在调用 `connect(...)` 时进行配置。

<Code lang="typescript" code={configureSessionExample} />

这些传输层允许您传入与[session](https://platform.openai.com/docs/api-reference/realtime-client-events/session/update) 匹配的任意参数。

对于新增且在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中没有对应参数的配置，您可以使用 `providerData`。传入 `providerData` 的内容会直接作为 `session` 对象的一部分传递。
...
与常规智能体不同，交接在 Realtime Agents 中的行为略有差异。执行交接时，正在进行的会话会使用新的智能体配置进行更新。因此，智能体会自动访问持续的对话历史，且当前不会应用输入过滤器。

另外，这意味着在交接中不能更改 `voice` 或 `model`。并且只能连接到其他 Realtime Agents。如果您需要使用不同的模型，例如像 `gpt-5-mini` 这样的推理模型，您可以使用[通过工具进行委托](#delegation-through-tools)。

## 工具

与常规智能体一样，Realtime Agents 可以调用工具来执行操作。您可以使用与常规智能体相同的 `tool()` 函数来定义工具。

<Code lang="typescript" code={defineToolExample} />

在 Realtime Agents 中只能使用函数工具，并且这些工具会在与您的 Realtime Session 相同的位置执行。这意味着如果您在浏览器中运行 Realtime Session，工具也会在浏览器中执行。如果需要执行更敏感的操作，您可以在工具内向后端服务器发起 HTTP 请求。

在工具执行期间，智能体无法处理来自用户的新请求。提升体验的一种方式是让智能体在即将执行工具时进行提醒，或说出特定短语，为执行工具争取时间。

### 访问会话历史

除了智能体调用某个工具时传入的参数之外，您还可以访问由 Realtime Session 追踪的当前对话历史快照。如果您需要基于对话的当前状态执行更复杂的操作，或计划[将工具用于委托](#delegation-through-tools)，这会很有用。

<Code lang="typescript" code={toolHistoryExample} />

<Aside type="note">
  传入的历史是工具调用当时的快照。用户最后说的话的转录可能尚不可用。
</Aside>

### 工具执行前的审批

如果您在定义工具时设置了 `needsApproval: true`，智能体会在执行工具前发出 `tool_approval_requested` 事件。
...
当模型响应返回时，您提供的护栏将异步运行，允许您基于预定义的分类触发器（例如“提及特定禁用词”）来切断响应。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还提供一个包含触发护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符运行一次，或在响应文本生成结束时运行。由于朗读文本通常更耗时，这意味着在大多数情况下，护栏应在用户听到之前捕获到违规。
...
修改轮次检测设置有助于校准不必要的打断以及处理静默。查看[Realtime API 文档以了解不同设置的更多细节](https://platform.openai.com/docs/guides/realtime-vad)

## 打断

使用内置语音活动检测时，打断智能体说话会自动触发智能体基于所听到的内容进行检测并更新其上下文。同时会发出 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。
...
无论哪种方式，Realtime Session 都会处理对智能体生成的打断，截断其对已向用户说出内容的认知，并更新历史。

如果您使用 WebRTC 连接智能体，它还会清除音频输出。如果您使用 WebSocket，则需要自行处理，停止播放已经排队等待播放的音频。

## 文本输入

如果您想向智能体发送文本输入，可以使用 `RealtimeSession` 上的 `sendMessage` 方法。

这在您希望让用户以双模态与智能体交互，或为对话提供额外上下文时很有用。
...
您可以使用它向客户渲染历史或对其执行其他操作。由于该历史会在对话过程中不断变化，您可以监听 `history_updated` 事件。
...
通过将对话历史与一次工具调用相结合，您可以将对话委托给另一个后端智能体以执行更复杂的操作，然后将结果再传回给用户。
