...
3. `updateHistory()` 不能通过 Realtime 创建条目路径直接重放助手 `output_audio` 项。如果您需要 round-trip 包含助手音频的历史，请使用 `output_audio.transcript` 将这些项转换为文本消息，或在调用 `updateHistory()` 前移除音频项。
4. 因中断而被截断的响应不会保留最终转录。
5. 对输入音频的转录最好只作为用户所说内容的大致参考，而不是模型如何理解音频的精确副本。
