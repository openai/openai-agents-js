...
你可以在构造期间向 [`RealtimeSession`](/openai-agents-js/openai/agents-realtime/classes/realtimesession/) 传递额外选项，或在调用 `connect(...)` 时进行配置。
...
对于全新的、在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中没有对应参数的配置，你可以使用 `providerData`。传入 `providerData` 的内容会直接作为 `session` 对象的一部分传递。
...
与常规智能体不同，交接在实时智能体中表现略有差异。执行交接时，正在进行的会话会用新的智能体配置进行更新。因此，智能体会自动访问持续的对话历史，且当前不会应用输入过滤器。

此外，这意味着在交接中不能更改 `voice` 或 `model`。你也只能连接到其他实时智能体。如果你需要使用不同的模型，例如像 `gpt-5-mini` 这样的推理模型，可以使用[通过工具进行委派](#delegation-through-tools)。
...
你只能在实时智能体中使用函数工具，这些工具会在与你的 Realtime Session 相同的位置执行。这意味着如果你在浏览器中运行 Realtime Session，你的工具也会在浏览器中执行。如果需要执行更敏感的操作，你可以在工具内向后端服务器发起 HTTP 请求。

当工具执行时，智能体将无法处理来自用户的新请求。改进体验的一种方式是让你的智能体在即将执行工具时进行告知，或说出特定语句为执行工具争取时间。

### 访问对话历史

除了智能体调用某个工具时使用的参数外，你还可以访问由 Realtime Session 追踪的当前对话历史快照。如果你需要基于对话的当前状态执行更复杂的操作，或计划[使用工具进行委派](#delegation-through-tools)，这将很有用。

<Code lang="typescript" code={toolHistoryExample} />

<Aside type="note">
  传入的历史记录是在工具调用时的历史快照。用户最后一句话的转录可能尚不可用。
</Aside>
...
通过监听此事件，你可以向用户展示 UI 来批准或拒绝工具调用。
...
护栏提供一种方式，用于监控智能体的发言是否违反了一系列规则，并立即切断响应。这些护栏检查基于智能体响应的转录执行，因此需要启用模型的文本输出（默认已启用）。

你提供的护栏会在模型响应返回时异步运行，使你可以基于预定义的分类触发器（例如“提到了某个被禁词”）来切断响应。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供包含触发护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符或在响应文本生成结束时运行。由于朗读文本通常需要更长时间，这意味着在大多数情况下护栏应在用户听到之前捕捉到违规内容。
...
Realtime Session 会自动检测用户何时在说话，并使用 Realtime API 内置的[语音活动检测模式](https://platform.openai.com/docs/guides/realtime-vad)触发新的回合。

你可以通过向会话传入 `turnDetection` 对象来更改语音活动检测模式。

<Code lang="typescript" code={turnDetectionExample} />

修改回合检测设置有助于校准不必要的打断和处理静默。查看[Realtime API 文档以了解不同设置的更多详情](https://platform.openai.com/docs/guides/realtime-vad)
...
如果你想执行手动中断，例如在 UI 中提供一个“停止”按钮，你可以手动调用 `interrupt()`：

<Code lang="typescript" code={sessionInterruptExample} />

无论哪种方式，Realtime Session 都会处理对智能体生成过程的中断，截断其对已对用户所说内容的认知，并更新历史。

如果你使用 WebRTC 连接到智能体，它还会清除音频输出。如果你使用 WebSocket，则需要自行处理，停止播放已排队的音频。
...
你可以使用它向客户渲染历史，或对其执行其他操作。由于在对话过程中该历史会不断变化，你可以监听 `history_updated` 事件。
...
将对话历史与工具调用相结合，你可以将对话委派给另一个后端智能体来执行更复杂的操作，然后将其结果返回给用户。

<Code lang="typescript" code={delegationAgentExample} />

以下代码将在服务器上执行。本示例通过 Next.js 的 server actions 实现。
