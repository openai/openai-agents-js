...
对于新增且在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中没有对应参数的项，你可以使用 `providerData`。传入 `providerData` 的内容会作为 `session` 对象的一部分被直接传递。
...
与常规智能体不同，交接在 Realtime Agents 中的行为略有差异。当执行交接时，正在进行的会话会更新为新的智能体配置。因此，智能体会自动访问正在进行的对话历史，并且当前不会应用输入过滤器。

此外，这意味着在交接时不能更改 `voice` 或 `model`。你也只能连接到其他 Realtime Agents。如果需要使用不同的模型，例如像 `gpt-5-mini` 这样的推理模型，你可以使用[通过工具进行委托](#delegation-through-tools)。
...
你只能在 Realtime Agents 中使用函数工具，并且这些工具会在与你的 Realtime Session 相同的位置执行。这意味着如果你在浏览器中运行 Realtime Session，你的工具也会在浏览器中执行。如果需要执行更敏感的操作，可以在工具内向你的后端服务器发起 HTTP 请求。

在工具执行期间，智能体无法处理来自用户的新请求。改善体验的一种方法是让智能体在即将执行工具时进行提示，或说出特定语句以为执行工具争取时间。

### 访问对话历史

除了智能体调用特定工具时传入的参数外，你还可以访问由 Realtime Session 追踪的当前对话历史快照。如果你需要基于对话的当前状态执行更复杂的操作，或计划[使用工具进行委托](#delegation-through-tools)，这会很有用。

<Code lang="typescript" code={toolHistoryExample} />

<Aside type="note">
  传入的历史是工具调用时刻的快照。用户最后一句话的转录可能尚不可用。
</Aside>

### 工具执行前的审批

如果你在定义工具时使用了 `needsApproval: true`，智能体会在执行工具之前发出 `tool_approval_requested` 事件。
...
你提供的护栏会在模型响应返回时异步运行，允许你基于预定义的分类触发条件来切断响应，例如“提到了特定的禁用词”。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供一个包含触发护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符或在响应文本结束时运行一次。由于朗读文本通常更耗时，这意味着大多数情况下护栏会在用户听到违规内容之前将其拦截。
...
修改回合检测设置有助于校准不必要的打断以及处理静默。查看[Realtime API 文档以了解不同设置的更多细节](https://platform.openai.com/docs/guides/realtime-vad)

## 打断

使用内置语音活动检测时，用户说话会自动触发智能体基于所说内容检测并更新其上下文。同时会触发 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。
...
无论哪种方式，Realtime Session 都会处理打断智能体生成、截断其对用户所说内容的知识，并更新历史。

如果你使用 WebRTC 连接到智能体，它还会清空音频输出。如果你使用 WebSocket，则需要自行处理，停止播放已排队等待播放的音频。
...
你可以用它来向客户渲染历史，或对其执行其他操作。由于在整个对话过程中该历史会不断变化，你可以监听 `history_updated` 事件。
...
结合对话历史与工具调用，你可以把对话委托给另一个后端智能体以执行更复杂的操作，然后将其结果返回给用户。

<Code lang="typescript" code={delegationAgentExample} />

下面的代码将会在服务器上执行。本示例通过 Next.js 中的 server actions 实现。
