...
对于新增且在 [RealtimeSessionConfig](/openai-agents-js/openai/agents-realtime/type-aliases/realtimesessionconfig/) 中没有对应参数的项，你可以使用 `providerData`。传入 `providerData` 的任何内容都会作为 `session` 对象的一部分直接传递。

可以在构造时设置的其他 `RealtimeSession` 选项：

| 选项                                          | 类型                              | 目的                                                 |
| --------------------------------------------- | --------------------------------- | ---------------------------------------------------- |
| `context`                                     | `TContext`                        | 额外的本地上下文，与会话上下文合并。                 |
...
| `tracingDisabled`                             | `boolean`                         | 为本次会话禁用追踪。                                 |
| `groupId`                                     | `string`                          | 在多个会话或后端运行中对追踪进行分组。               |
...
与常规智能体类似，你可以使用交接将智能体拆分为多个智能体并在其间编排，以提升智能体性能并更好地限定问题范围。

<Code lang="typescript" code={multiAgentsExample} />

与常规智能体不同，交接在实时智能体中的行为略有差异。执行交接时，正在进行的会话将更新为新的智能体配置。因此，智能体会自动访问正在进行的对话历史，且当前不会应用输入过滤器。

此外，这意味着在交接中不能更改 `voice` 或 `model`。你也只能连接到其他实时智能体。如果你需要使用不同的模型，例如推理模型 `gpt-5-mini`，可以使用[通过工具进行委托](#delegation-through-tools)。

## 工具

与常规智能体一样，实时智能体可以调用工具来执行操作。Realtime 支持**函数工具**（本地执行）和**远程 MCP 服务器工具**（由 Realtime API 远程执行）。你可以使用与常规智能体相同的 `tool()` 辅助函数来定义函数工具。
...
在工具执行期间，智能体将无法处理来自用户的新请求。改进体验的一种方式是让你的智能体在即将执行工具时进行公告，或说一些特定短语来为执行工具争取时间。

如果函数工具应在完成后不立即触发下一次模型响应，请从 `@openai/agents/realtime` 返回 `backgroundResult(output)`。这会将工具输出发送回会话，同时将是否触发响应的控制权保留在你这边。

函数工具超时选项（`timeoutMs`、`timeoutBehavior`、`timeoutErrorFunction`）在 Realtime 会话中的行为相同。使用默认的 `error_as_result` 时，超时消息将作为工具输出发送。使用 `raise_exception` 时，会话会触发带有 [`ToolTimeoutError`](/openai-agents-js/openai/agents-core/classes/tooltimeouterror) 的 `error` 事件，并且不会为该次调用发送工具输出。

### 访问对话历史

除了访问智能体调用特定工具时传入的参数外，你还可以访问 Realtime Session 跟踪的当前对话历史快照。如果你需要基于当前对话状态执行更复杂的操作，或计划使用[工具进行委托](#delegation-through-tools)，这会很有用。

<Code lang="typescript" code={toolHistoryExample} />

<Aside type="note">
  传入的历史是在工具调用时刻的快照。用户刚刚说的话的转录可能尚不可用。
</Aside>

### 工具执行前审批

如果你在定义工具时设置了 `needsApproval: true`，智能体会在执行工具前触发 `tool_approval_requested` 事件。
...
护栏提供了一种监控智能体发言是否违反一组规则并立即切断响应的方式。这些护栏检查会基于智能体响应的转录进行，因此要求启用模型的文本输出（默认已启用）。

你提供的护栏会在模型响应返回时异步运行，使你能够基于预定义的分类触发来切断响应，例如“提及特定的禁用词”。

当护栏被触发时，会话会发出 `guardrail_tripped` 事件。该事件还会提供一个包含触发护栏的 `itemId` 的 `details` 对象。

<Code lang="typescript" code={guardrailsExample} />

默认情况下，护栏每 100 个字符或在响应文本结束时运行一次。由于朗读文本通常需要更长时间，这意味着在大多数情况下，护栏会在用户听到之前捕获到违规。
...
Realtime Session 将自动检测用户何时在说话，并使用 Realtime API 内置的[语音活动检测模式](https://platform.openai.com/docs/guides/realtime-vad)触发新的轮次。

你可以通过向会话传入 `turnDetection` 对象来更改语音活动检测模式。

<Code lang="typescript" code={turnDetectionExample} />

修改轮次检测设置有助于校准不必要的打断并处理静默。参见[Realtime API 文档以了解不同设置的更多细节](https://platform.openai.com/docs/guides/realtime-vad)

## 中断

使用内置的语音活动检测时，打断智能体说话会自动触发智能体根据所说内容检测并更新其上下文。同时会触发 `audio_interrupted` 事件。这可用于立即停止所有音频播放（仅适用于 WebSocket 连接）。
...
无论哪种方式，Realtime Session 都会处理对智能体生成的中断、截断其对已向用户播报内容的认知，并更新历史。

如果你使用 WebRTC 连接到智能体，它还会清除音频输出。如果你使用 WebSocket，则需要自行处理，停止已排队待播放的音频的播放。
