...
使用任意录音/回放库来处理原始 PCM16 音频字节。

### SIP 连接

使用 `OpenAIRealtimeSIP` 传输将来自 Twilio 等提供商的 SIP 呼叫桥接过来。该传输会让 Realtime 会话与您的电话供应商发出的 SIP 事件保持同步。

1. 通过 `OpenAIRealtimeSIP.buildInitialConfig()` 生成初始会话配置来接受来电。这可确保 SIP 邀请与 Realtime 会话共享相同的默认值。
...
Cloudflare Workers 与其他 workerd 运行时无法使用全局 `WebSocket` 构造函数打开出站 WebSocket。请使用扩展包中的 Cloudflare 传输，其在内部通过 `fetch()` 完成升级。

<Code lang="typescript" code={cloudflareTransportExample} />

### 自定义传输机制

如果您想使用不同的语音到语音 API，或拥有自定义的传输机制，可以实现 `RealtimeTransportLayer` 接口并触发 `RealtimeTransportEventTypes` 事件，来创建自己的传输。

## 更直接地与 Realtime API 交互

如果您想使用 OpenAI Realtime API，同时需要更直接的控制，有两种方式：
...
如果您不需要自动工具执行、护栏等功能，也可以将传输层作为一个只管理连接和中断的“轻薄”客户端来使用。
