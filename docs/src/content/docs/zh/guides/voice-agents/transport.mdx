...
如需使用自定义媒体流或音频元素，请在创建会话时传入一个 `OpenAIRealtimeWebRTC` 实例。

<Code lang="typescript" code={customWebRTCTransportExample} />

### 通过 WebSocket 连接

在创建会话时传入 `transport: 'websocket'` 或一个 `OpenAIRealtimeWebSocket` 实例，以使用 WebSocket 连接替代 WebRTC。它非常适合服务器端用例，例如使用 Twilio 构建电话智能体。
...
如果你想使用不同的语音到语音 API，或拥有自定义传输机制，你可以实现 `RealtimeTransportLayer` 接口并触发 `RealtimeTransportEventTypes` 事件，以创建自己的传输层。
...
如果你仍希望受益于 `RealtimeSession` 的全部能力，可以通过 `session.transport` 访问传输层。

传输层会在 `*` 事件下发出它收到的每一个事件，你也可以使用 `sendEvent()` 方法发送原始事件。

<Code lang="typescript" code={transportEventsExample} />

### 选项 2 — 仅使用传输层

如果你不需要自动工具执行、护栏等功能，你也可以将传输层作为一个“轻量”客户端，只管理连接和中断。
