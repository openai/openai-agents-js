...
若要使用自定义媒体流或音频元素，请在创建会话时提供一个 `OpenAIRealtimeWebRTC` 实例。
...
Cloudflare Workers 和其他 workerd 运行时无法使用全局 `WebSocket` 构造函数发起出站 WebSocket。请使用扩展包中的 Cloudflare 传输，其在内部通过 `fetch()` 执行升级。

<Code lang="typescript" code={cloudflareTransportExample} />

### 自定义传输机制

如果你想使用不同的语音到语音 API，或拥有自定义传输机制，你可以实现 `RealtimeTransportLayer` 接口并触发 `RealtimeTransportEventTypes` 事件，来创建自己的传输层。

## 更直接的 Realtime API 交互

如果你希望使用 OpenAI Realtime API，同时对 Realtime API 拥有更直接的访问方式，你有两个选项：
...
如果你不需要自动工具执行、护栏等功能，也可以仅将传输层用作一个只管理连接和打断的“薄”客户端。
