...
在创建会话时传入 `transport: 'websocket'`，或传入一个 `OpenAIRealtimeWebSocket` 实例，以使用 WebSocket 连接替代 WebRTC。这非常适合服务器端场景，例如使用 Twilio 构建电话智能体。
...
使用 `OpenAIRealtimeSIP` 传输，将来自 Twilio 等提供商的 SIP 呼叫桥接进来。该传输会使 Realtime 会话与您的电话提供商发出的 SIP 事件保持同步。

1. 通过 `OpenAIRealtimeSIP.buildInitialConfig()` 生成初始会话配置以接受来电。这可确保 SIP 邀请与 Realtime 会话共享相同的默认值。
2. 附加一个使用 `OpenAIRealtimeSIP` 传输的 `RealtimeSession`，并使用提供商 webhook 下发的 `callId` 进行连接。
...
如果您希望使用不同的语音到语音 API，或拥有自定义传输机制，可以实现 `RealtimeTransportLayer` 接口并发出 `RealtimeTransportEventTypes` 事件，从而创建自己的传输。
...
如果仍希望受益于 `RealtimeSession` 的全部功能，可以通过 `session.transport` 访问您的传输层。
...
如果您不需要自动工具执行、护栏等功能，也可以将传输层作为一个仅管理连接和中断的“轻量”客户端来使用。
