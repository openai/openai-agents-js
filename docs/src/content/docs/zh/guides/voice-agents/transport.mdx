...
如需使用自有的媒体流或音频元素，请在创建会话时提供一个 `OpenAIRealtimeWebRTC` 实例。

<Code lang="typescript" code={customWebRTCTransportExample} />

### WebSocket 连接

在创建会话时传入 `transport: 'websocket'` 或一个 `OpenAIRealtimeWebSocket` 实例，以使用 WebSocket 连接替代 WebRTC。此方式非常适合服务端用例，例如使用 Twilio 构建电话智能体。
...
通过使用 `OpenAIRealtimeSIP` 传输桥接来自 Twilio 等提供商的 SIP 呼叫。该传输会使 Realtime 会话与您的电话提供商发出的 SIP 事件保持同步。

1. 使用 `OpenAIRealtimeSIP.buildInitialConfig()` 生成初始会话配置以接受来电。这可确保 SIP 邀请和 Realtime 会话共享相同的默认值。
...
如果您想使用不同的语音到语音 API，或自有的传输机制，可以实现 `RealtimeTransportLayer` 接口并发出 `RealtimeTransportEventTypes` 事件，来创建自定义实现。

## 更直接的 Realtime API 交互

如果您想使用 OpenAI Realtime API，同时获得对 Realtime API 更直接的访问，有两种方式：

### 选项 1——访问传输层

如果您仍希望享受 `RealtimeSession` 的全部能力，可以通过 `session.transport` 访问传输层。
