...
0. **创建项目**
   ...
   在本快速开始中，我们将创建一个可在浏览器中使用的语音智能体。若想从一个新项目开始，可以尝试使用[`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
1. **安装 Agents SDK**
   ...
   或者，你也可以安装 `@openai/agents-realtime` 来获取独立的浏览器包。

2. **生成客户端临时令牌**

   由于此应用将在用户的浏览器中运行，我们需要一种安全的方式通过 Realtime API 连接到模型。为此，我们可以使用在你的后端服务器上生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试时，你也可以使用 `curl` 和常规的 OpenAI API key 来生成密钥。
   ...
   响应将在顶层包含一个名为 "value" 的字符串，其以 "ek\_" 前缀开头。稍后你可以使用此临时密钥建立 WebRTC 连接。请注意，此密钥仅在短时间内有效，需要定期重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会持续在一个 `RealtimeSession` 中运行并监听，它负责随着时间推进管理与模型的对话与连接。该会话还将处理音频处理、打断以及稍后会介绍的许多其他生命周期功能。
...
5. **连接到会话**
   ...
   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置你的麦克风与扬声器以进行音频输入和输出。如果你在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 将自动使用 WebSocket 作为连接。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解更多不同的传输层。
...
7. **启动并开始对话**
   ...
   启动你的 Web 服务器并访问包含新 Realtime Agent 代码的页面。你应会看到麦克风访问权限请求。授予权限后，你就可以开始与智能体对话了。
...
从这里开始，你可以设计并构建自己的语音智能体。语音智能体与常规智能体具备许多相同的功能，同时也有一些独特特性。
