...
0. **创建项目**
   ...
   在本快速上手中，我们将创建一个可在浏览器中使用的语音智能体。若想从一个全新项目开始，可以尝试 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
2. **生成客户端临时令牌**
   ...
   由于该应用将在用户的浏览器中运行，我们需要一种安全的方式通过 Realtime API 连接模型。为此可以使用在后端服务器上生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试场景下，你也可以使用 `curl` 搭配常规的 OpenAI API key 来生成密钥。
   ...
   响应的顶层会包含一个以 "ek\_" 为前缀的 "value" 字符串。稍后你可以使用该临时密钥建立 WebRTC 连接。注意该密钥仅在短时间内有效，需要定期重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会持续在一个 `RealtimeSession` 中运行和监听，该会话负责长时间的对话与模型连接。该会话还会处理音频处理、打断，以及我们稍后将介绍的许多其他生命周期功能。
   ...
   `RealtimeSession` 构造函数的第一个参数是 `agent`。该智能体将成为用户最先可以交互的智能体。
...
5. **连接到会话**
   ...
   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置你的麦克风和扬声器用于音频输入和输出。如果你在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 将自动使用 WebSocket 作为连接。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解不同的传输层。
...
从这里开始，你可以设计并构建自己的语音智能体。语音智能体包含与常规智能体相同的许多功能，同时也具有一些独特特性。
