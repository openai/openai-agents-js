...
1. **安装 Agents SDK**
   ...
   或者，你也可以安装`@openai/agents-realtime`，它是一个独立的浏览器包。

2. **生成客户端临时令牌**

   由于该应用将在用户的浏览器中运行，我们需要一种安全的方式通过 Realtime API 连接到模型。为此，我们可以使用在你的后端服务器上生成的[ephemeral client key](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试期间，你也可以使用`curl`和你的常规 OpenAI API key 来生成一个密钥。
   ...
   响应将在顶层包含一个以“ek\_”为前缀的 "value" 字符串。你可以稍后使用这个临时密钥建立 WebRTC 连接。请注意，该密钥只在短时间内有效，需要定期重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会持续在一个`RealtimeSession`中运行并监听，该会话会随着时间的推移处理与模型的对话与连接。该会话还会处理音频处理、打断以及我们稍后将涉及的许多其他生命周期功能。
   ...
   `RealtimeSession` 构造函数将一个 `agent` 作为第一个参数。该智能体将是你的用户首先能够交互的对象。
...
5. **连接到会话**
   ...
   这将在浏览器中使用 WebRTC 连接到 Realtime API，并自动配置你的麦克风和扬声器用于音频输入与输出。如果你在后端服务器（如 Node.js）上运行`RealtimeSession`，SDK 将自动使用 WebSocket 作为连接。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解更多不同的传输层。
...
从这里开始，你可以设计并构建你自己的语音智能体。语音智能体包含许多与常规智能体相同的特性，同时也有一些独特功能。
