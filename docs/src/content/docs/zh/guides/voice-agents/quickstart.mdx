...
0. **创建项目**
   ...
   在本快速上手中，我们将创建一个可在浏览器中使用的语音智能体。若要从新项目开始，可尝试使用[`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
2. **生成客户端临时令牌**
   ...
   由于该应用将在用户的浏览器中运行，我们需要一种安全方式通过 Realtime API 连接到模型。为此，可使用在后端服务器上生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试环境中，您也可以使用 `curl` 搭配常规 OpenAI API 密钥来生成密钥。
   ...
   响应会在顶层包含一个以 "ek\_" 为前缀的 "value" 字符串。您可以使用该临时密钥稍后建立 WebRTC 连接。请注意，该密钥仅在短时间内有效，需要重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体在 `RealtimeSession` 中持续运行并监听，该会话会随时间处理对话并维持与模型的连接。它还会处理音频处理、打断以及稍后将介绍的许多其他生命周期功能。
...
5. **连接会话**
   ...
   要连接会话，需要传入之前生成的客户端临时令牌。

   ```typescript
   await session.connect({ apiKey: 'ek_...(put your own key here)' });
   ```

   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置麦克风与扬声器进行音频输入与输出。若您在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 会自动使用 WebSocket 作为连接。您可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解不同的传输层。
...
7. **启动并开始对话**
   ...
   启动您的 Web 服务器并访问包含新 Realtime Agent 代码的页面。您将看到麦克风访问请求。授权后即可开始与智能体对话。
...
从这里开始，您可以设计并构建自己的语音智能体。语音智能体包含许多与常规智能体相同的功能，同时也有一些独特能力。
