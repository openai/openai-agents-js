...
0. **创建项目**
   ...
   在本次快速上手中，我们将创建一个可在浏览器中使用的语音智能体。若要从零开始新项目，可以尝试 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
1. **安装 Agents SDK**（需要 Zod v4）
   ...
   或者，你也可以安装 `@openai/agents-realtime` 作为独立的浏览器包。

2. **生成客户端临时令牌**

   由于此应用将在用户的浏览器中运行，我们需要一种安全方式通过 Realtime API 连接到模型。为此可以使用应由后端服务器生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。出于测试目的，你也可以使用 `curl` 和常规 OpenAI API 密钥生成一个密钥。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会在 `RealtimeSession` 中持续运行并监听，它会随时间处理与模型的对话与连接。该会话还将处理音频处理、打断，以及我们稍后将介绍的许多其他生命周期功能。
   ...
   `RealtimeSession` 构造函数将 `agent` 作为第一个参数。该智能体将成为用户最先可以交互的智能体。
...
5. **连接到会话**
   ...
   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置你的麦克风和扬声器进行音频输入与输出。如果你在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 将自动使用 WebSocket 作为连接。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解更多不同的传输层。
...
从这里开始，你可以设计并构建你自己的语音智能体。语音智能体与常规智能体共享大量功能，同时也具备一些独有特性。
...
- 了解如何为语音智能体添加：
  ...
  - [处理音频中断](/openai-agents-js/zh/guides/voice-agents/build#interruptions)
  - [管理会话历史](/openai-agents-js/zh/guides/voice-agents/build#conversation-history-management)
