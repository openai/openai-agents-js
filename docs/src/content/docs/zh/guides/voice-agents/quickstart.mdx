...
0. **创建项目**
   ...
   在本快速上手中，我们将创建一个可在浏览器中使用的语音智能体。如果你想从一个新项目开始，可以尝试 [`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
2. **生成客户端临时令牌**
   ...
   由于该应用将在用户的浏览器中运行，我们需要一种安全的方式通过 Realtime API 连接到模型。为此可以使用一种[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)，应当在你的后端服务器上生成。用于测试时，你也可以使用 `curl` 和常规的 OpenAI API 密钥来生成一个密钥。
   ...
   响应会在顶层包含一个以 "ek\_" 前缀开头的 "value" 字符串。你可以使用此临时密钥稍后建立 WebRTC 连接。请注意，该密钥仅在短时间内有效，需要重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会在一个持续运行并监听的 `RealtimeSession` 中工作，它会随时间处理与模型的对话与连接。该会话还将处理音频处理、打断，以及我们稍后会介绍的大量其他生命周期功能。
...
5. **连接到会话**
   ...
   这将在浏览器中使用 WebRTC 连接到 Realtime API，并自动配置你的麦克风和扬声器以进行音频输入和输出。如果你在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 会自动使用 WebSocket 进行连接。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解更多不同的传输层。
...
7. **启动引擎并开始对话**
   ...
   启动你的 Web 服务器并访问包含新 Realtime 智能体代码的页面。你应该会看到麦克风访问请求。授予权限后，你就可以开始与智能体对话了。
...
从这里开始，你可以设计并构建你自己的语音智能体。语音智能体包含与常规智能体相同的许多功能，同时也有其独特的特性。
