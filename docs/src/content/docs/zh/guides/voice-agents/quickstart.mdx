...
0. **创建项目**
   ...
   在本快速上手中，我们将创建一个可在浏览器中使用的语音智能体。若要从新项目开始，可以试试[`Next.js`](https://nextjs.org/docs/getting-started/installation) 或者 [`Vite`](https://vite.dev/guide/installation.html)。
...
1. **安装 Agents SDK**
   ...
   另外，你也可以安装 `@openai/agents-realtime` 以使用独立的浏览器包。

2. **生成客户端临时令牌**

   由于该应用将在用户的浏览器中运行，我们需要一种安全方式通过 Realtime API 连接到模型。为此可以使用应在后端服务器上生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试时，你也可以使用 `curl` 和常规的 OpenAI API 密钥来生成密钥。
   ...
   响应会在顶层包含一个以 "ek\_" 前缀开头的 "value" 字符串。你可以使用这个临时密钥稍后建立 WebRTC 连接。请注意，该密钥只在短时间内有效，需要定期重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会在一个持续运行并监听的 `RealtimeSession` 中工作，它随着时间处理与模型的对话和连接。该会话还会处理音频处理、打断以及我们稍后会介绍的许多其他生命周期功能。
   ...
   `RealtimeSession` 构造函数将 `agent` 作为第一个参数。该智能体将是你的用户最先能够交互的对象。
...
5. **连接到会话**
   ...
   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置你的麦克风和扬声器用于音频输入和输出。如果你在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 将自动使用 WebSocket 作为连接方式。你可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解更多不同的传输层。
...
7. **启动并开始对话**
   ...
   启动你的 Web 服务器并导航到包含新 Realtime Agent 代码的页面。你应该会看到请求麦克风访问的提示。授予访问权限后，你就可以开始与智能体对话了。
...
从这里开始，你可以设计并构建你自己的语音智能体。语音智能体包含许多与常规智能体相同的功能，但也有其独特特性。
