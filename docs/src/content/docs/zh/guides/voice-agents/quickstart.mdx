...
0. **创建项目**
   ...
   在本快速开始中，我们将创建一个可在浏览器中使用的语音智能体。若要从零开始新项目，可以尝试使用[`Next.js`](https://nextjs.org/docs/getting-started/installation) 或 [`Vite`](https://vite.dev/guide/installation.html)。
...
1. **安装 Agents SDK**
   ...
   或者，您也可以安装 `@openai/agents-realtime` 来使用独立的浏览器包。

2. **生成客户端临时令牌**

   由于此应用将在用户的浏览器中运行，我们需要一种安全的方式通过 Realtime API 连接到模型。为此可以使用在后端服务器上生成的[临时客户端密钥](https://platform.openai.com/docs/guides/realtime#creating-an-ephemeral-token)。在测试场景下，您也可以使用 `curl` 和常规的 OpenAI API key 生成一个密钥。
   ...
   响应会在顶层包含一个以 "ek\_" 为前缀的 "value" 字符串。稍后您可以使用这个临时密钥来建立 WebRTC 连接。请注意，此密钥只在短时间内有效，需要定期重新生成。
...
4. **创建会话**
   ...
   与常规智能体不同，语音智能体会在一个持续运行并监听的 `RealtimeSession` 中工作，该会话会随时间管理与模型的对话和连接。此会话还会处理音频处理、打断等稍后将介绍的许多生命周期功能。
...
5. **连接到会话**
   ...
   这将在浏览器中通过 WebRTC 连接到 Realtime API，并自动配置麦克风与扬声器用于音频输入和输出。如果您在后端服务器（如 Node.js）上运行 `RealtimeSession`，SDK 将自动使用 WebSocket 作为连接方式。您可以在[传输机制](/openai-agents-js/zh/guides/voice-agents/transport)指南中了解不同传输层的更多信息。
...
7. **启动并开始对话**
   ...
   启动您的 Web 服务器并访问包含新 Realtime Agent 代码的页面。您应该会看到请求麦克风访问权限的提示。授权后即可开始与智能体对话。
...
从这里开始，您可以设计并构建自己的语音智能体。语音智能体包含与常规智能体相同的许多特性，同时也拥有一些独特能力。
