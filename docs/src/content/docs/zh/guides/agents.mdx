...
| `prompt`                          | no       | OpenAI Responses API 的提示配置。参见 [Prompt](/openai-agents-js/zh/guides/models#prompt)。                            |
...
| `mcpServers`                      | no       | 为智能体提供工具的 MCP 服务器。参见 [MCP 指南](/openai-agents-js/zh/guides/mcp)。                                      |
...
智能体对其上下文类型是**泛型**的——即 `Agent<TContext, TOutput>`。该上下文是你创建并传递给 `Runner.run()` 的依赖注入对象。它会被转发到每个工具、护栏、交接等，对于存储状态或提供共享服务（数据库连接、用户元数据、功能开关等）很有用。
...
组合智能体的方式很多。在生产应用中我们经常看到两种模式：

1. **Manager (agents as tools)**——一个中心智能体掌控对话，并调用以工具形式暴露的专业智能体。
...
在这种模式下，manager 从不移交控制权——LLM 使用工具，manager 总结最终答案。详见 [工具指南](/openai-agents-js/zh/guides/tools#agents-as-tools)。

<Code lang="typescript" code={agentsAsTools} title="Agents as tools" />

#### 交接

在交接中，分诊智能体负责路由请求，但一旦发生交接，专家智能体将拥有对话的控制权，直到其产生最终输出。这样可以保持提示简短，并让你可以独立地推理每个智能体。了解更多参见[交接指南](/openai-agents-js/zh/guides/handoffs)。
...
`instructions` 可以是**函数**而不是字符串。该函数接收当前的 `RunContext` 和 Agent 实例，并可以返回字符串或 `Promise<string>`。
...
护栏允许你验证或转换用户输入和智能体输出。它们通过 `inputGuardrails` 和 `outputGuardrails` 数组进行配置。详情参见[护栏指南](/openai-agents-js/zh/guides/guardrails)。
...
提供工具并不保证 LLM 会调用它。你可以通过 `modelSettings.toolChoice` **强制**使用工具：
...
在工具调用后，SDK 会自动将 `toolChoice` 重置回 `'auto'`。这可防止模型陷入反复尝试调用工具的无限循环。你可以通过 `resetToolChoice` 标志或配置 `toolUseBehavior` 来覆盖此行为：
...
注意：`toolUseBehavior` 仅适用于**函数工具**。托管工具总是会返回给模型进行处理。
