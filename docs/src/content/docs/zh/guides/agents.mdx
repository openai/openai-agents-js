...
智能体是 OpenAI Agents SDK 的主要构建块。一个**Agent** 是经过如下配置的 Large Language Model (LLM)：

- **Instructions**——告诉模型“它是谁”和“应如何回应”的系统提示。
...
| `modelSettings` | no   | 调参项（temperature、top_p 等）。如果所需属性不在顶层，可以放在 `providerData` 下。        |
...
智能体对其上下文类型是**泛型**的——即 `Agent<TContext, TOutput>`。_上下文_ 是一个依赖注入对象，由你创建并传给 `Runner.run()`。它会被转发到每个工具、护栏、交接等位置，可用于存储状态或提供共享服务（数据库连接、用户元数据、功能开关等）。
...
默认情况下，智能体返回**纯文本**（`string`）。如果你希望模型返回结构化对象，可以指定 `outputType` 属性。SDK 支持：
...
将智能体组合在一起的方法很多。在生产应用中我们常见的两种模式是：

1. **管理者（智能体作为工具）**——一个中心智能体掌控对话，并调用以工具形式暴露的专业化智能体。
2. **交接**——初始智能体在识别出用户请求后，将整个对话交给某个专家智能体。

这些方法是互补的。管理者为你提供一个统一位置来实施护栏或速率限制；而交接则让每个智能体专注于单一任务，而不必持续掌控对话。

### 管理者（智能体作为工具）

在此模式下，管理者不会移交控制权——LLM 使用工具，最终由管理者汇总答案。更多内容参见[工具](/openai-agents-js/zh/guides/tools#agents-as-tools)。

<Code lang="typescript" code={agentsAsTools} title="作为工具的智能体" />

### 交接

通过交接，分诊智能体负责路由请求，但一旦发生交接，专家智能体将掌控对话直至产出最终结果。这样可以保持提示简短，并让你能独立地推理每个智能体。了解更多请参见[交接](/openai-agents-js/zh/guides/handoffs)。
...
`instructions` 可以是**函数**而不是字符串。该函数会接收当前的 `RunContext` 以及 Agent 实例，并可返回字符串或 `Promise<string>`。
...
对于高级用例，你可以通过监听事件来观察智能体生命周期。可用项请参考此处列出的智能体钩子事件名称：[here](https://github.com/openai/openai-agents-js/blob/main/packages/agents-core/src/lifecycle.ts)。
...
护栏允许你验证或转换用户输入和智能体输出。通过 `inputGuardrails` 与 `outputGuardrails` 数组进行配置。详情参见[护栏](/openai-agents-js/zh/guides/guardrails)。

---

## 克隆/复制智能体

需要现有智能体的一个略微修改版本？使用 `clone()` 方法，它会返回一个全新的 `Agent` 实例。
...
2. `'required'`——LLM **必须**调用一个工具（可自行选择）。
3. `'none'`——LLM **不得**调用工具。
...
在一次工具调用后，SDK 会自动将 `tool_choice` 重置为 `'auto'`。这可防止模型反复尝试调用工具而陷入无限循环。你可以通过 `resetToolChoice` 标志或配置 `toolUseBehavior` 来覆盖该行为：
