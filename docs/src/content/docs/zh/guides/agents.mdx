...
| `modelSettings`      | no       | 调优参数（temperature、top_p 等）。如果你需要的属性不在顶层，可以放在 `providerData` 下。  |
...
智能体在其上下文类型上是**泛型**的，即 `Agent<TContext, TOutput>`。*上下文*是你创建并传递给 `Runner.run()` 的依赖注入对象。它会被转发到每个工具、护栏、交接等，对于存储状态或提供共享服务（数据库连接、用户元数据、功能开关等）非常有用。
...
将智能体组合在一起有很多方式。在生产应用中我们经常看到两种模式：

1. **管理者（将智能体作为工具）**——一个中心智能体拥有对话，并调用作为工具暴露的专业智能体。
2. **交接**——初始智能体在识别出用户请求后，将整个对话交给某个专家智能体。

这些方法是互补的。管理者让你可以在一个地方实施护栏或速率限制，而交接让每个智能体专注于单一任务，而无需保留对话控制权。

### 管理者（将智能体作为工具）

在这种模式下，管理者从不移交控制权——LLM 使用工具，而管理者总结最终答案。更多内容请参阅[工具](/openai-agents-js/zh/guides/tools#agents-as-tools)。

<Code lang="typescript" code={agentsAsTools} title="作为工具的智能体" />

### 交接

通过交接，分诊智能体路由请求，但一旦交接发生，专家智能体将拥有对话，直到其产生最终输出为止。这样可以保持提示简洁，并让你可以独立地推理每个智能体。详细了解请参阅[交接](/openai-agents-js/zh/guides/handoffs)。
...
对于进阶用例，你可以通过监听事件来观察智能体生命周期。要了解可用内容，请参考[此处](https://github.com/openai/openai-agents-js/blob/main/packages/agents-core/src/lifecycle.ts)列出的智能体钩子事件名称。
...
提供工具并不保证 LLM 会调用其中之一。你可以通过 `modelSettings.tool_choice` **强制**使用工具：
...
在一次工具调用后，SDK 会自动将 `tool_choice` 重置回 `'auto'`。这可防止模型进入反复尝试调用工具的无限循环。你可以通过 `resetToolChoice` 标志或配置 `toolUseBehavior` 来覆盖此行为：

- `'run_llm_again'`（默认）——使用工具结果再次运行 LLM。
- `'stop_on_first_tool'`——将第一个工具结果视为最终答案。
- `{ stopAtToolNames: ['my_tool'] }`——当调用任意列出的工具时停止。
...
注意：`toolUseBehavior` 仅适用于**函数工具（function tools）**。托管工具（Hosted tool）始终返回给模型进行处理。
...
- 深入探索[工具](/openai-agents-js/zh/guides/tools)、[护栏](/openai-agents-js/zh/guides/guardrails)和[模型](/openai-agents-js/zh/guides/models)。
