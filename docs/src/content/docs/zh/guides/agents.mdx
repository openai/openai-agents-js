...
智能体是 OpenAI Agents SDK 的主要构建块。一个**Agent** 是一个经配置的 Large Language Model (LLM)，包括：
...
| `modelSettings` | no   | 调参项（temperature、top_p 等）。如果你需要的属性不在顶层，可以放在 `providerData` 下。    |
...
智能体在其上下文类型上是**泛型**的——即 `Agent<TContext, TOutput>`。该上下文是一个依赖注入对象，由你创建并传给 `Runner.run()`。它会被转发到每个工具、护栏、交接等，适合用于存储状态或提供共享服务（数据库连接、用户元数据、功能开关等）。
...
将智能体组合在一起有许多方式。我们在生产应用中常见的两种模式是：

1. **管理者（将智能体作为工具）**——一个中心智能体拥有对话，并调用通过工具暴露的专长智能体。
2. **交接**——初始智能体在识别出用户请求后，将整个对话交给某个专家智能体。

这些方法是互补的。管理者让你可以在单一位置实施护栏或限流，而交接则让每个智能体聚焦于单一任务，而不必持续掌控对话。

### 管理者（将智能体作为工具）

在这种模式下，管理者从不移交控制权——LLM 使用工具，由管理者总结最终答案。更多内容见[工具](/openai-agents-js/zh/guides/tools#agents-as-tools)。

<Code lang="typescript" code={agentsAsTools} title="Agents as tools" />

### 交接

使用交接时，分诊智能体负责路由请求，但一旦发生交接，专家智能体就拥有对话，直到产生最终输出。这能让提示保持简短，并让你可以独立推理每个智能体。了解更多见[交接](/openai-agents-js/zh/guides/handoffs)。
...
对于高级用例，你可以通过监听事件来观察 Agent 生命周期。要了解可用内容，请参阅[此处](https://github.com/openai/openai-agents-js/blob/main/packages/agents-core/src/lifecycle.ts)列出的 agent 钩子事件名称。
...
护栏允许你验证或转换用户输入与智能体输出。通过 `inputGuardrails` 与 `outputGuardrails` 数组进行配置。详见[护栏](/openai-agents-js/zh/guides/guardrails)。
...
在一次工具调用后，SDK 会自动将 `tool_choice` 重置为 `'auto'`。这可防止模型陷入反复尝试调用工具的无限循环。你可以通过 `resetToolChoice` 标志或配置
