...
智能体是 OpenAI Agents SDK 的主要构建块。一个**智能体**是经过配置的 Large Language Model (LLM)，包含：
...
| `modelSettings`      | no       | 调优参数（temperature、top_p 等）。如果所需属性不在顶层，可将它们放在 `providerData` 下。  |
...
智能体对其上下文类型是**泛型**的，即 `Agent<TContext, TOutput>`。上下文是一个依赖注入对象，由你创建并传给 `Runner.run()`。它会被转发到每个工具、护栏、交接等，适用于存储状态或提供共享服务（数据库连接、用户元数据、功能开关等）。
...
将智能体组合在一起有很多方法。我们在生产应用中常见的两种模式是：

1. **管理器（智能体作为工具）**——中心智能体拥有对话权，并调用作为工具暴露的专业化智能体。
2. **交接**——初始智能体在识别到用户请求后，将整个对话委派给专家智能体。

这些方法是互补的。管理器让你在一个地方实施护栏或速率限制，而交接则让每个智能体专注于单一任务，而无需保留对话控制权。

### 管理器（智能体作为工具）

在这种模式下，管理器不会移交控制权——LLM 使用工具，管理器汇总最终答案。详见[工具](/openai-agents-js/zh/guides/tools#agents-as-tools)。

<Code lang="typescript" code={agentsAsTools} title="Agents as tools" />

### 交接

在交接中，分诊智能体负责路由请求，但一旦发生交接，专家智能体将拥有对话权直到产生最终输出。这样可以保持提示简短，并使你能够独立地推理每个智能体。了解更多，请参见[交接](/openai-agents-js/zh/guides/handoffs)。
...
对于高级用例，你可以通过监听事件来观察智能体生命周期。要了解可用内容，请参阅列出的智能体钩子事件名称[这里](https://github.com/openai/openai-agents-js/blob/main/packages/agents-core/src/lifecycle.ts)。
...
在一次工具调用后，SDK 会自动将 `tool_choice` 重置为 `'auto'`。这可防止模型进入反复尝试调用工具的无限循环。你可以通过 `resetToolChoice` 标志或配置 `toolUseBehavior` 来覆盖此行为：

- `'run_llm_again'`（默认）——使用工具结果再次运行 LLM。
- `'stop_on_first_tool'`——将第一个工具结果视为最终答案。
- `{ stopAtToolNames: ['my_tool'] }`——调用到任一列出的工具时停止。
