...
本项目遵循一种稍作修改的语义化版本规范，形式为 `0.Y.Z`。前导的 `0` 表示该 SDK 仍在快速演进中。各部分的递增规则如下：

## 次版本（`Y`）

对于未标记为 beta 的任何公共接口出现的**破坏性变更**，我们会提升次版本号 `Y`。例如，从 `0.0.x` 升级到 `0.1.x` 可能包含破坏性变更。
...
主包 `@openai/agents` 由多个可独立使用的子包组成。目前这些包的版本是联动的，即如果某个包的版本提升，其他包也会随之提升。随着我们迈向 `1.0.0`，此策略可能会有所调整。

## 更新日志

我们会为每个子包生成更新日志，以帮助了解变更内容。由于变更可能发生在某个子包中，你可能需要查看对应的更新日志以获取详细信息。
