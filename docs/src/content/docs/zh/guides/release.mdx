...
本项目采用经过轻微修改的语义化版本控制，形式为 `0.Y.Z`。前导的 `0` 表示 SDK 仍在快速演进。各组件递增规则如下：

## 次版本（`Y`）

对于未标记为 beta 的任意公共接口发生的**不兼容变更**，我们会增加次版本号 `Y`。例如，从 `0.0.x` 到 `0.1.x` 可能包含破坏性变更。

如果你不希望引入不兼容变更，建议在项目中固定到 `0.0.x` 版本。

## 修订版本（`Z`）

对于非破坏性变更，我们会增加修订版本号 `Z`：
...
主包 `@openai/agents` 由多个可独立使用的子包组成。目前这些包的版本是联动的，即某个包版本提升时，其他包也会提升。随着我们向 `1.0.0` 迁移，这一策略可能会发生变化。

## 更新日志

我们为每个子包生成更新日志，帮助理解变更内容。由于变更可能发生在某个子包中，你可能需要查看对应的更新日志以了解详情。
