...
本项目采用略作修改的语义化版本，形式为 `0.Y.Z`。前导的 `0` 表示 SDK 仍在快速演进中。各组件的递增规则如下：

## 次版本（`Y`）

对于未标记为 beta 的任何公共接口出现的**破坏性变更**，我们会增加次版本号 `Y`。例如，从 `0.0.x` 到 `0.1.x` 可能包含破坏性变更。
...
我们为每个子包生成更新日志，帮助了解变更内容。由于变更可能发生在某个子包中，你可能需要查看相应包的更新日志以了解详细信息。
