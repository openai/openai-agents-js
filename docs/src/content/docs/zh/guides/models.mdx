...
日常使用中，通常只需要与模型**名称**交互，偶尔会使用到 `ModelSettings`。
...
当初始化 `Agent` 时未指定模型，将使用默认模型。当前默认是为了兼容性和低延迟而选择的 [`gpt-4.1`](https://platform.openai.com/docs/models/gpt-4.1)。如果您有权限，建议为更高质量将智能体设置为 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2)，同时保留显式的 `modelSettings`。

如果您想切换到其他模型（如 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2)），有两种方式为智能体配置。

首先，如果希望对所有未设置自定义模型的智能体统一使用某个特定模型，请在运行智能体前设置环境变量 `OPENAI_DEFAULT_MODEL`。
...
当以这种方式使用任一 GPT-5 推理模型（[`gpt-5`](https://platform.openai.com/docs/models/gpt-5)、[`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini) 或 [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano)）时，SDK 会默认应用合理的 `modelSettings`：将 `reasoning.effort` 和 `verbosity` 都设为 `"low"`。若要为默认模型调整推理强度，请传入自定义的 `modelSettings`：
...
为了更低的延迟，使用 [`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini) 或 [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano) 并设置 `reasoning.effort="minimal"`，通常会比默认设置更快返回。不过，Responses API 中的一些内置工具（如文件搜索与图像生成）不支持 `"minimal"` 推理强度，因此本 Agents SDK 将默认值设为 `"low"`。

### 非 GPT-5 模型

如果传入非 GPT-5 的模型名称且未提供自定义 `modelSettings`，SDK 将回退到与任意模型兼容的通用 `modelSettings`。
...
如果需要自定义网络设置，您也可以通过 `setDefaultOpenAIClient(client)` 注入自有的 `OpenAI` 客户端。

---

## ModelSettings

`ModelSettings` 与 OpenAI 的参数保持一致，但不依赖特定提供方。
...
`Runner` 级别的设置会覆盖与每个智能体级别冲突的设置。

---

## Prompt

可以通过 `prompt` 参数为智能体进行配置，指向一个在服务器端存储的 prompt 配置，用于控制智能体行为。目前，此选项仅在使用 OpenAI [Responses API](https://platform.openai.com/docs/api-reference/responses) 时受支持。

| 字段        | 类型     | 说明                                                                                  |
| ----------- | -------- | ------------------------------------------------------------------------------------- |
| `promptId`  | `string` | prompt 的唯一标识符。                                                                 |
| `version`   | `string` | 你希望使用的 prompt 版本。                                                            |
| `variables` | `object` | 要替换进 prompt 的键/值变量对。值可以是字符串，或诸如文本、图像、文件等内容输入类型。 |

<Code lang="typescript" code={promptIdExample} title="使用 prompt 的智能体" />

任何额外的智能体配置（如 tools 或 instructions）都会覆盖您在已存储 prompt 中配置的值。

---

## 自定义模型提供方

实现自定义提供方很简单——实现 `ModelProvider` 和 `Model`，并将该提供方传入 `Runner` 构造函数：
...
使用 OpenAI 提供方时，可以通过提供 API 密钥选择性启用自动追踪导出：
...
- 通过[工具](/openai-agents-js/zh/guides/tools)为模型增添能力。
