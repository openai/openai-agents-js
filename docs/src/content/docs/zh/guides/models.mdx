...
在日常工作中，您通常只需要与模型**名称**交互，偶尔使用 `ModelSettings`。
...
当您在初始化 `Agent` 时未指定模型时，将使用默认模型。当前默认值为 [`gpt-4.1`](https://platform.openai.com/docs/models/gpt-4.1)，兼具兼容性与低延迟。如果您有权限，建议将智能体设置为 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2) 以获得更高质量，同时保持显式的 `modelSettings`。

如果您想切换到其他模型（如 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2)），有两种方式可以配置智能体。

首先，如果您希望对所有未设置自定义模型的智能体一致地使用某个特定模型，请在运行智能体之前设置 `OPENAI_DEFAULT_MODEL` 环境变量。
...
其次，您可以为某个 `Runner` 实例设置默认模型。如果您未为某个智能体设置模型，将使用该 `Runner` 的默认模型。
...
为降低延迟，使用 [`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini) 或 [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano) 并设置 `reasoning.effort="minimal"` 往往会比默认设置返回更快。然而，Responses API 中的一些内置工具（如文件搜索和图像生成）不支持 `"minimal"` 推理力度，因此本 Agents SDK 默认使用 `"low"`。

### 非 GPT-5 模型

如果您传入非 GPT-5 的模型名称且未提供自定义 `modelSettings`，SDK 将回退到适用于任意模型的通用 `modelSettings`。
...
`ModelSettings` 反映了 OpenAI 的参数，但对提供方保持无关。
...
| `presencePenalty`   | `number`                                   | 鼓励新增 token。                                                          |
...
`Runner` 级别的设置会覆盖任何冲突的每智能体设置。

---

## 提示词

智能体可通过 `prompt` 参数进行配置，指示应使用的服务器存储提示词配置，以控制智能体行为。目前，此选项仅在使用 OpenAI [Responses API](https://platform.openai.com/docs/api-reference/responses) 时受支持。

| 字段        | 类型     | 说明                                                                      |
| ----------- | -------- | ------------------------------------------------------------------------- |
| `promptId`  | `string` | 提示词的唯一标识符。                                                      |
| `version`   | `string` | 您希望使用的提示词版本。                                                  |
| `variables` | `object` | 代入提示词的键/值变量对。值可以是字符串或文本、图像、文件等内容输入类型。 |

<Code lang="typescript" code={promptIdExample} title="带提示词的智能体" />

任何额外的智能体配置，如 tools 或 instructions，都会覆盖您在已存储提示词中可能配置的值。

---

## 自定义模型提供方

实现您自己的提供方很简单——实现 `ModelProvider` 和 `Model`，并将该提供方传入 `Runner` 构造函数：
