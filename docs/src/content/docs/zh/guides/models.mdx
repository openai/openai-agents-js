...
当你在初始化 `Agent` 时未指定模型，将会使用默认模型。当前默认是为兼容性与低延迟而选择的 [`gpt-4.1`](https://platform.openai.com/docs/models/gpt-4.1)。如果你有访问权限，我们建议将智能体设置为 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2) 以获得更高质量，同时保持显式的 `modelSettings`。

如果你想切换到其他模型（如 [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2)），有两种方式配置你的智能体。

首先，如果你希望对所有未设置自定义模型的智能体一致使用某一模型，请在运行智能体前设置 `OPENAI_DEFAULT_MODEL` 环境变量。
...
为了降低延迟，使用 [`gpt-5-mini`](https://platform.openai.com/docs/models/gpt-5-mini) 或 [`gpt-5-nano`](https://platform.openai.com/docs/models/gpt-5-nano) 且将 `reasoning.effort="minimal"`，通常会比默认设置更快返回结果。但需要注意，Responses API 中的一些内置工具（如文件搜索与图像生成）不支持 `"minimal"` 的推理强度，因此本 Agents SDK 默认使用 `"low"`。
...
默认的 `ModelProvider` 会使用 OpenAI API 来解析模型名称。它支持两个不同的端点：
...
如果你需要自定义网络设置，你也可以通过 `setDefaultOpenAIClient(client)` 注入你自己的 `OpenAI` 客户端。
...
| `store`             | `boolean`                                  | 将响应持久化，便于检索 / RAG 工作流。                                      |
| `reasoning.effort`  | `'minimal' \| 'low' \| 'medium' \| 'high'` | 适用于 gpt-5 等模型的推理强度。                                            |
| `text.verbosity`    | `'low' \| 'medium' \| 'high'`              | 适用于 gpt-5 等模型的文本冗长程度。                                        |

可在任一层级附加设置：

<Code lang="typescript" code={modelSettingsExample} title="模型设置" />

`Runner` 级别的设置会覆盖任何与之冲突的智能体级设置。

---

## 提示词

可以通过 `prompt` 参数为智能体进行配置，指向一个存储在服务器上的提示词配置，用于控制智能体行为。目前，这一选项仅在使用 OpenAI 的
...
| `variables` | `object` | 需要替换进提示词的键值对变量。值可以是字符串，或诸如文本、图像、文件等内容输入类型。 |
...
当使用 OpenAI 提供方时，你可以通过提供 API 密钥来选择加入自动追踪导出：
