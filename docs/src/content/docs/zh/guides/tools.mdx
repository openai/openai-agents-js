...
工具让智能体能够**采取行动**——获取数据、调用外部 API、执行代码，甚至进行计算机操作。JavaScript/TypeScript SDK 支持四种类别：

1. **托管工具**——与模型一起在 OpenAI 服务器上运行。（Web 搜索、文件搜索、计算机操作、Code Interpreter、图像生成）
2. **函数工具**——用 JSON schema 包装任意本地函数，便于 LLM 调用。
...
精确的参数集合与 OpenAI Responses API 保持一致——高级选项如 `rankingOptions` 或语义过滤等，请参阅官方文档。
...
如果你需要模型在输入不完整或无效时进行“猜测”，可以在使用原始 JSON schema 时关闭 strict 模式：
...
有时你希望一个智能体“辅助”另一个智能体，而不是完全接管对话。使用 `agent.asTool()`：
...
当你将智能体作为工具运行时，Agents SDK 会使用默认设置创建一个 runner，并在函数执行中用它来运行该智能体。如果你想提供任何 `runConfig` 或 `runOptions` 的属性，可以将它们传给 `asTool()` 方法，以自定义 runner 的行为。
...
完整示例参见 [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts)。此外，如果你在寻找关于 MCP 服务器工具集成的综合指南，请参阅 [MCP 集成](/openai-agents-js/zh/guides/mcp) 获取详情。

---

## 工具使用行为

关于控制模型何时以及如何必须使用工具（`tool_choice`、`toolUseBehavior` 等），参见[智能体](/openai-agents-js/zh/guides/agents#forcing-tool-use)。
...
- 查阅 TypeDoc 对 [`tool()`](/openai-agents-js/openai/agents/functions/tool) 以及各类托管工具类型的参考文档。
