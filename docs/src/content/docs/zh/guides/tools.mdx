...
1. **托管工具（Hosted tool）**——在 OpenAI 服务器上的模型旁运行。（Web 搜索、文件搜索、计算机操作、Code Interpreter、图像生成）
2. **函数工具（function tools）**——用 JSON Schema 包装任意本地函数，使 LLM 可以调用。
...
具体的参数集合与 OpenAI Responses API 保持一致——参见官方文档了解 `rankingOptions` 或语义过滤等高级选项。

---

## 2. 函数工具

你可以用 `tool()` 帮助器将**任意**函数变成工具。
...
| `parameters`    | 是   | Zod schema 或原始 JSON schema 对象。使用 Zod 参数将自动启用 **strict** 模式。                                                                                                               |
| `strict`        | 否   | 当为 `true`（默认）时，如果参数验证失败，SDK 会返回模型错误。将其设为 `false` 可进行模糊匹配。                                                                                              |
| `execute`       | 是   | `(args, context) => string                                                                                               \| Promise<string>`——你的业务逻辑。第二个可选参数为 `RunContext`。 |
| `errorFunction` | 否   | 自定义处理器 `(context, error) => string`，用于将内部错误转换为可见的字符串。                                                                                                               |
...
当你将智能体作为工具运行时，Agents SDK 会使用默认设置创建一个 runner，并在函数执行内用它来运行该智能体。如果你想提供任何 `runConfig` 或 `runOptions` 的属性，可以将它们传给 `asTool()` 方法以自定义 runner 的行为。
...
完整示例参见 [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts)。另外，如果你在寻找关于 MCP 服务器工具集成的完整指南，请参阅 [MCP 集成](/openai-agents-js/zh/guides/mcp) 了解详情。

---

## 工具使用行为

参见[智能体](/openai-agents-js/zh/guides/agents#forcing-tool-use)以控制模型在何时以及如何必须使用工具（`tool_choice`、`toolUseBehavior` 等）。

---

## 最佳实践

- **简短、明确的描述**——说明工具做什么、何时使用。
...
- 查阅 TypeDoc 参考文档了解 [`tool()`](/openai-agents-js/openai/agents/functions/tool) 以及各类托管工具类型。
