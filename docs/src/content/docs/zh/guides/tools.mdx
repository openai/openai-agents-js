...
2. **函数工具**——使用 JSON schema 封装任何本地函数，供 LLM 调用。
...
| Apply patch             | `'apply_patch'`      | 将 V4A 差异应用到本地文件。      |
...
确切的参数集合与 OpenAI Responses API 对齐——有关 `rankingOptions` 或语义过滤等高级选项，请参阅官方文档。
...
| `strict`        | 否   | 当为 `true`（默认）时，如果参数验证不通过，SDK 将返回模型错误。设置为 `false` 可进行模糊匹配。                                                                                                |
...
当你将智能体作为工具运行时，Agents SDK 会用默认设置创建一个 runner，并在函数执行中用它来运行该智能体。如果你想提供任何 `runConfig` 或 `runOptions` 的属性，可以将它们传递给 `asTool()` 方法以自定义 runner 的行为。

### 来自智能体工具的流式事件

智能体工具可以将所有嵌套运行事件流式传回你的应用。根据你构建工具的方式选择合适的钩子风格：
...
- `onStream` 是最简单的“全捕获”方式，适合在内联声明工具时使用（`tools: [agent.asTool({ onStream })]`）。如果你不需要逐事件路由，请使用它。
- `on(eventName, handler)` 允许你选择性订阅（或使用 `'*'`），当你需要更细粒度的处理或希望在创建后附加监听器时最合适。
- 如果你提供了 `onStream` 或任意 `on(...)` 处理器，作为工具的智能体将自动以流式模式运行；否则保持非流式路径。
- 处理器是并行调用的，因此缓慢的 `onStream` 回调不会阻塞 `on(...)` 处理器（反之亦然）。
- 当工具通过模型的工具调用触发时会提供 `toolCallId`；直接 `invoke()` 调用或提供方差异可能会省略它。
...
完整示例参见 [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts)。此外，如果你在寻找关于 MCP 服务器工具集成的全面指南，请参阅 [MCP 集成](/openai-agents-js/zh/guides/mcp) 了解详情。
...
- **每个工具只承担一个职责**——小而可组合的工具有助于更好的模型推理。
...
- 深入查看 TypeDoc 参考文档：[`tool()`](/openai-agents-js/openai/agents/functions/tool) 以及各类托管工具类型。
