...
工具让智能体**执行操作**——获取数据、调用外部 API、执行代码，甚至进行计算机操作。JavaScript/TypeScript SDK 支持四大类别：

1. **托管工具**——与模型一起在 OpenAI 服务器上运行。（Web 搜索、文件搜索、计算机操作、Code Interpreter、图像生成）
2. **函数工具**——用 JSON schema 包装任意本地函数，使 LLM 可以调用。
...
具体的参数集与 OpenAI Responses API 保持一致——参阅官方文档以了解如 `rankingOptions` 或语义过滤等高级选项。
...
| `description`   | 是   | 面向人类、清晰易懂的描述，展示给 LLM。                                                                                                                                                        |
| `parameters`    | 是   | 可以是 Zod schema 或原始 JSON schema 对象。使用 Zod 参数会自动启用 **strict** 模式。                                                                                                          |
| `strict`        | 否   | 当为 `true`（默认）时，如果参数验证失败，SDK 会返回模型错误。将其设为 `false` 可启用模糊匹配。                                                                                                |
...
如果你需要模型对无效或不完整的输入进行“猜测”，在使用原始 JSON schema 时可以禁用严格模式：

<Code
  lang="typescript"
  code={nonStrictSchemaTools}
  title="非严格的 JSON schema 工具"
/>
...
当你将智能体作为工具运行时，Agents SDK 会使用默认设置创建一个 runner，并在函数执行内用它运行该智能体。如果你想提供任何 `runConfig` 或 `runOptions` 的属性，可以将它们传给 `asTool()` 方法以自定义 runner 的行为。
...
- 事件类型与 `RunStreamEvent['type']` 对应：`raw_model_stream_event`、`run_item_stream_event`、`agent_updated_stream_event`。
- `onStream` 是最简单的“全捕获”，当你内联声明工具时（`tools: [agent.asTool({ onStream })]`）很适用。如果不需要按事件路由，请使用它。
- `on(eventName, handler)` 允许你选择性订阅（或使用 `'*'`），当你需要更细粒度的处理或想在创建后附加监听器时最合适。
- 如果你提供了 `onStream` 或任何 `on(...)` 处理器，智能体作为工具会自动以流式模式运行；否则保持非流式路径。
- 处理器并行调用，因此缓慢的 `onStream` 回调不会阻塞 `on(...)` 处理器（反之亦然）。
- 当工具通过模型 tool call 调用时会提供 `toolCallId`；直接 `invoke()` 调用或个别提供方的差异可能省略它。

---

## 4. MCP 服务器

你可以通过 [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 服务器暴露工具，并将其连接到一个智能体。
例如，你可以使用 `MCPServerStdio` 启动并连接到 stdio MCP 服务器：

<Code lang="typescript" code={mcpLocalServer} title="本地 MCP 服务器" />

完整示例参见 [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts)。另外，如果你在寻找关于 MCP 服务器工具集成的完整指南，请参考 [MCP 集成](/openai-agents-js/zh/guides/mcp) 了解详情。
...
- 查阅 [`tool()`](/openai-agents-js/openai/agents/functions/tool) 以及各种托管工具类型的 TypeDoc 参考。
