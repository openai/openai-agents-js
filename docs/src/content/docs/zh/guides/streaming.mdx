...
Agents SDK 可以增量传递模型及其他执行步骤的输出。流式传输可保持 UI 响应，并避免在更新用户前等待完整最终结果。
...
启用流式传输后，返回的 `stream` 实现了 `AsyncIterable` 接口。每个产出的事件都是一个对象，用于描述运行中的发生情况。该流会产出三种事件类型之一，分别描述智能体执行的不同部分。多数应用只需要模型的文本，因此流还提供了便捷方法。
...
你可以使用 `for await` 循环在事件到达时逐个检查。实用信息包括底层模型事件、任何智能体切换，以及 SDK 特定的运行信息：
...
参见[流式 code examples](https://github.com/openai/openai-agents-js/tree/main/examples/agent-patterns/streamed.ts)，该脚本完整打印纯文本流和原始事件流。
...
流式传输与会暂停执行的交接兼容（例如当某个工具需要审批时）。流对象上的 `interruption` 字段会暴露中断，你可以对每个中断调用 `state.approve()` 或 `state.reject()` 以继续执行。再次以 `{ stream: true }` 执行即可恢复流式输出。
...
- 初始的 `{ stream: true }` 选项仅适用于提供该选项的那次调用。如果使用 `RunState` 重新运行，必须再次指定该选项。
- 如果你的应用只关心文本结果，优先使用 `toTextStream()`，以避免处理单独的事件对象。

借助流式传输与事件系统，你可以将智能体集成到聊天界面、终端应用或任何用户受益于增量更新的场景中。
