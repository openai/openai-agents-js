...
Agents SDK 可以按增量方式输出模型及其他执行步骤的结果。流式传输可保持 UI 响应灵敏，避免在更新用户界面前等待整个最终结果。

## 启用流式传输

向 `Runner.run()` 传入 `{ stream: true }` 选项，以获取一个流对象而非完整结果：
...
启用流式传输后，返回的 `stream` 实现了 `AsyncIterable` 接口。每个产出的事件都是描述此次运行中发生情况的对象。流会产出三种事件类型之一，分别描述智能体执行的不同部分。大多数应用只需要模型的文本，因此流提供了便捷方法。
...
Promise `stream.completed` 会在此次运行及所有挂起的回调完成后解析。如果你需要确保没有更多输出，请务必等待它。
...
参见[流式示例](https://github.com/openai/openai-agents-js/tree/main/examples/agent-patterns/streamed.ts)，其中提供了一个完整脚本，同时打印纯文本流和原始事件流。
...
流式传输可与会暂停执行的交接兼容（例如当某个工具需要审批时）。流对象上的 `interruption` 字段会暴露这些中断，你可以通过为每个中断调用 `state.approve()` 或 `state.reject()` 来继续执行。使用 `{ stream: true }` 再次执行将恢复流式输出。
...
- 初始的 `{ stream: true }` 选项只适用于提供该选项的调用。如果你使用 `RunState` 再次运行，必须重新指定该选项。
