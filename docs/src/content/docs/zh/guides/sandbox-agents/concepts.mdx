...
<Aside type="caution" title="Beta 功能">
  ...
  可能会发生变化，未来预计还会加入更多高级功能。
</Aside>

现代智能体在能够操作文件系统中的真实文件时效果最佳。**沙盒智能体**可以使用专用工具和 shell 命令搜索和处理大型文档集、编辑文件、生成产物并运行命令。沙盒为模型提供一个持久化工作区，智能体可在其中代您完成工作。Agents SDK 中的沙盒智能体可帮助您运行与沙盒环境配对的智能体，让您能够轻松地将所需文件放入文件系统，并大规模编排沙盒以启动、停止和恢复任务。
...
<figure class="sandbox-harness-image">
  <img src={harnessWithCompute.src} alt="带计算环境的沙盒智能体框架" />
</figure>
...
在本页中，"沙盒会话"是指由沙盒客户端管理的实时执行环境。具体边界取决于客户端：Unix 本地会话在主机上的本地工作区中运行，而 Docker 和托管客户端可提供更强的环境隔离。这与[会话](/openai-agents-js/zh/guides/sessions)中介绍的 SDK 对话式 `Session` 接口不同。
...
>
  ...
  <div>
    ...
    <span>准备 instructions 并绑定功能工具</span>
  </div>
  <span aria-hidden="true">→</span>
  <div>
    <strong>沙盒会话</strong>
...
1. 使用 `SandboxAgent`、`Manifest` 和各项功能定义智能体与初始工作区内容。
2. 向 `run()` 或 `Runner` 提供 `sandbox` 运行选项，通过注入、恢复或创建沙盒会话来执行运行。
3. 稍后从运行器管理的 `RunState`、显式沙盒 `sessionState` 或保存的工作区快照继续运行。

如果只偶尔需要将 shell 访问作为一个工具，请先使用[工具](/openai-agents-js/zh/guides/tools)指南中的托管 shell。当工作区隔离、沙盒客户端选择或沙盒会话恢复行为属于整体设计的一部分时，请使用沙盒智能体。
...
2. 确定本次运行实际使用的工作区输入。
3. 让各项功能处理生成的清单。
4. 按固定顺序构建最终 instructions：SDK 的默认沙盒提示；如果您显式覆盖，则改用 `baseInstructions`；然后是 `instructions`、功能的 instructions 片段、任何远程挂载策略文本，最后是渲染后的文件系统树。
5. 将功能工具绑定到实时沙盒会话，并通过常规的 `run()` 和 `Runner` API 运行准备好的智能体。
...
<Code lang="typescript" code={pathGrantsExample} title="授权访问共享本地来源" />
...
<div
  ...
  aria-label="在 SDK 管理的生命周期中，运行器负责创建和关闭沙盒；在开发者管理的生命周期中，由应用代码负责创建和清理。"
>
  <section>
    ...
    <ol>
      ...
      <li>
        ...
        <span class="sandbox-lifecycle-step">运行器关闭其管理的资源。</span>
      </li>
    </ol>
  </section>
  <section>
...
- [Agents as tools](/openai-agents-js/zh/guides/tools#4-agents-as-tools)：将多个沙盒智能体公开为工具，通常是在每次调用 `asTool(...)` 时传入沙盒运行配置，让每个工具拥有自己的沙盒边界。
- [MCP 集成](/openai-agents-js/zh/guides/mcp)和常规函数工具：沙盒功能可以与 `mcpServers` 和常规工具共存。
