...
护栏与您的智能体*并行*运行，允许您对用户输入或智能体输出执行检查与校验。比如，您可以在调用昂贵模型之前运行一个轻量模型作为护栏。如果护栏检测到恶意使用，它可以触发错误并阻止昂贵模型运行。
...
> **注意**
> 输入护栏面向用户输入，因此只在该智能体是工作流中的*第一个*智能体时运行。护栏在智能体上进行配置，因为不同智能体通常需要不同护栏。
