...
“上下文”是一个被频繁复用的术语。通常有两大类你可能关心的上下文：
...
当调用 LLM 时，它只能看到会话历史。要提供额外信息，你可以：

1. 将其添加到智能体的 `instructions`（也称为系统或开发者消息）。可以是静态字符串，或接收上下文并返回字符串的函数。
2. 在调用 `Runner.run()` 时将其包含在 `input` 中。这与 instructions 的方式类似，但允许你将消息放在[指挥链的更低位置](https://cdn.openai.com/spec/model-spec-2024-05-08.html#follow-the-chain-of-command)。
3. 通过函数工具暴露出来，使 LLM 可按需获取数据。
