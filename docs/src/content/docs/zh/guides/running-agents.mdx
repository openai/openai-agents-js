...
当你在 Runner 中使用 run 方法时，需要传入一个起始智能体和输入。输入可以是一个字符串（被视为用户消息），或一组输入项，这些输入项即 OpenAI Responses API 中的项目。
...
在应用启动时创建一个 `Runner` 并在多个请求间复用。该实例会存储全局配置，例如模型提供方和追踪选项。只有在需要完全不同的设置时才创建另一个 `Runner`。对于简单脚本，你也可以调用 `run()`，它内部使用默认 runner。

## 运行参数

传给 `run()` 方法的输入包含用于开始运行的初始智能体、运行的输入以及一组选项。
...
| ---------- | ------- | ---------------------------------------------------------------------------------------------------------------------- |
| `stream`   | `false` | 若为 `true`，调用将返回 `StreamedRunResult` 并在事件从模型抵达时发出事件。                                             |
...
流式传输允许你在 LLM 运行时额外接收流式事件。流开始后，`StreamedRunResult` 将包含关于运行的完整信息，包括所有新生成的输出。你可以使用 `for await` 循环迭代流式事件。详见[流式传输](/openai-agents-js/zh/guides/streaming)。
...
| `handoffInputFilter`        | `HandoffInputFilter`  | 在执行交接时变更输入项（如果交接本身未定义该过滤器）。 |
...
每次调用 `runner.run()`（或 `run()` 工具）代表你的应用层对话中的一个回合。你可以选择向终端用户展示多少 `RunResult`——有时仅展示 `finalOutput`，有时展示每个生成的项目。

<Code lang="typescript" code={chatLoopExample} title="携带对话历史的示例" />

参见[聊天示例](https://github.com/openai/openai-agents-js/tree/main/examples/basic/chat.ts)获取交互版本。

### 服务器托管的会话

你可以让 OpenAI Responses API 为你持久化对话历史，而不是在每个回合都发送你本地的完整记录。当你协调长对话或多个服务时，这很有用。详见[会话状态指南](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)。
...
#### 2. 使用 `previousResponseId` 从上个回合继续

如果你只想从 Responses API 入手，可以将每次请求串联上一次响应返回的 ID。这样无需创建完整的会话资源也能在回合间保持上下文。
...
下面是一个处理 `GuardrailExecutionError` 的示例代码。由于输入护栏只在第一次用户输入时运行，示例会使用原始输入和上下文重新启动运行。它还展示了如何复用保存的状态来重试输出护栏，而无需再次调用模型：
...
- 输入护栏只在运行的第一次用户输入时触发，因此你必须用相同的输入/上下文开启一个新的运行来重试——传入保存的 `state` 不会重新触发输入护栏。
