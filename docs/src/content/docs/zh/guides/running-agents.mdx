...
当你在 Runner 中使用 run 方法时，你需要传入一个起始智能体和输入。输入可以是字符串（被视为用户消息），也可以是输入项列表，即 OpenAI Responses API 中的各项。
...
在应用启动时创建一个 `Runner` 并在请求之间复用。该实例存储全局配置，例如模型提供方和追踪选项。只有在需要完全不同的配置时才创建另一个 `Runner`。对于简单脚本，你也可以调用内部使用默认 runner 的 `run()`。

## 运行参数

传递给 `run()` 方法的输入包括用于启动运行的初始智能体、运行的输入以及一组选项。

输入可以是字符串（被视为用户消息）、[输入项](/openai-agents-js/openai/agents-core/type-aliases/agentinputitem)列表，或者在构建[人机协作](/openai-agents-js/zh/guides/human-in-the-loop)智能体时传入的 [`RunState`](/openai-agents-js/openai/agents-core/classes/runstate) 对象。
...
流式传输允许你在 LLM 运行时接收流事件。流开始后，`StreamedRunResult` 将包含关于此次运行的完整信息，包括所有新生成的输出。你可以使用 `for await` 循环迭代这些流事件。更多信息参见[流式传输](/openai-agents-js/zh/guides/streaming)。
...
| `handoffInputFilter`        | `HandoffInputFilter`     | 执行交接时修改输入项（若交接本身未定义过滤器）。          |
...
| `traceMetadata`             | `Record<string, string>` | 附加到每个跨度上的任意元数据。                            |
...
每次调用 `runner.run()`（或 `run()` 工具）代表你应用层对话中的一个**轮次**。你可以自行决定向终端用户展示多少 `RunResult`——有时仅展示 `finalOutput`，有时展示每个生成项。
...
你可以让 OpenAI Responses API 为你持久化对话历史，而不必在每一轮发送完整的本地抄本。当你协调长对话或多个服务时，这很有用。详情参见[会话状态指南](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)。

OpenAI 提供两种复用服务器端状态的方式：

#### 1. 整个对话使用 `conversationId`

你可以使用 [Conversations API](https://platform.openai.com/docs/api-reference/conversations/create) 创建一次对话，然后在每一轮重用其 ID。SDK 会自动只包含新生成的项目。

<Code lang="typescript" code={conversationIdExample} title="复用服务器端对话" />

#### 2. 使用 `previousResponseId` 从上一次轮次继续

如果你无论如何都只想从 Responses API 开始，你可以将每个请求与上一次响应返回的 ID 串联起来。这样可以在不创建完整对话资源的情况下在轮次之间保持上下文。
...
使用 `callModelInputFilter` 在调用模型*之前*编辑模型输入。该钩子会接收当前智能体、上下文以及合并后的输入项（包含会话历史（如存在））。返回更新后的 `input` 数组和可选的 `instructions`，以便脱敏、丢弃旧消息或注入额外的系统提示。

可在每次运行中通过 `runner.run(..., { callModelInputFilter })` 设置，或在 `Runner` 配置（`RunConfig` 中的 `callModelInputFilter`）中设置默认值。
...
以下是处理 `GuardrailExecutionError` 的代码示例。由于输入护栏只在首次用户输入时运行，该示例会用原始输入和上下文重新开始运行。它还展示了如何重用已保存的状态，在不再次调用模型的情况下重试输出护栏：
...
- 输入护栏仅在一次运行的首次用户输入时运行，因此你必须使用相同的输入/上下文重新开始一次全新的运行进行重试——传入已保存的 `state` 不会重新触发输入护栏。
- 输出护栏在模型响应之后运行，因此你可以复用来自 `GuardrailExecutionError` 的已保存 `state`，在不再次调用模型的情况下重新运行输出护栏。
...
- 添加用于生产化的[护栏](/openai-agents-js/zh/guides/guardrails)或[追踪](/openai-agents-js/zh/guides/tracing)。
