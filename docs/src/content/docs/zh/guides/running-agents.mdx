...
运行智能体后，你会收到一个[执行结果](/openai-agents-js/zh/guides/results)对象，包含最终输出以及完整的运行历史。

## 智能体循环

在 Runner 中使用 run 方法时，你需要传入一个起始智能体和输入。输入可以是字符串（被视为用户消息），也可以是输入项列表，这些项与 OpenAI Responses API 的项目一致。
...
<Aside type="note">
  将 LLM 输出视为“最终输出”的规则是：输出了期望类型的文本，并且没有工具调用。
</Aside>

### Runner 生命周期

在应用启动时创建一个 `Runner` 并在请求之间复用。该实例存储全局配置，例如模型提供方和追踪配置。仅当你需要完全不同的配置时才创建另一个 `Runner`。对于简单脚本，你也可以调用 `run()`，它会在内部使用默认 runner。

## 运行参数

`run()` 方法的输入包括用于启动运行的初始智能体、运行输入以及一组选项。
...
流式传输允许你在 LLM 运行时接收额外的流事件。一旦流开始，`StreamedRunResult` 将包含关于此次运行的完整信息，包括所有新生成的输出。你可以使用 `for await` 循环迭代这些流事件。详见[流式传输](/openai-agents-js/zh/guides/streaming)。
...
| `handoffInputFilter`        | `HandoffInputFilter`  | 在执行交接时变更输入项（如果交接本身尚未定义）。                |
...
每次调用 `runner.run()`（或 `run()` 工具）代表你在应用层对话中的一个**轮次**。你可以选择向终端用户展示多少 `RunResult`——有时只展示 `finalOutput`，有时展示每个生成的项目。
...
你可以让 OpenAI Responses API 为你持久化对话历史，而不是在每个轮次发送整个本地记录。这在协调长对话或多个服务时很有用。详情见[会话状态指南](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)。

OpenAI 提供两种重用服务器端状态的方式：

#### 1. 整个会话使用 `conversationId`

你可以使用 [Conversations API](https://platform.openai.com/docs/api-reference/conversations/create) 创建一次会话，然后在每个轮次重用其 ID。SDK 会自动仅包含新生成的项目。

<Code lang="typescript" code={conversationIdExample} title="重用服务器会话" />

#### 2. 使用 `previousResponseId` 从上一个轮次继续

如果你无论如何都只想从 Responses API 开始，可以使用上一次响应返回的 ID 串联每个请求。这样无需创建完整的会话资源，也能在轮次之间保持上下文。

<Code
  lang="typescript"
  code={previousResponseIdExample}
  title="使用 previousResponseId 进行串联"
/>
...
上述错误均继承自基础 `AgentsError` 类，该类可能提供 `state` 属性以访问当前运行状态。
...
- 添加[护栏](/openai-agents-js/zh/guides/guardrails)或[追踪](/openai-agents-js/zh/guides/tracing)以达到生产级就绪。
