...
当你使用 Runner 的 run 方法时，需要传入一个起始智能体和输入。输入可以是字符串（视为用户消息），也可以是输入项列表，这些输入项与 OpenAI Responses API 的项相同。
...
<Aside type="note">
  判断 LLM
  输出是否为“最终输出”的规则是：它产生了期望类型的文本输出，且没有工具调用。
</Aside>

### Runner 生命周期

在应用启动时创建一个 `Runner` 并在各个请求间复用。该实例存储全局配置，如模型提供方和追踪选项。只有在需要完全不同的设置时才创建另一个 `Runner`。对于简单脚本，你也可以直接调用 `run()`，它会在内部使用默认 runner。

## 运行参数

`run()` 方法的输入包括用于启动的初始智能体、运行输入以及一组选项。
...
| ---------- | ------- | -------------------------------------------------------------------------------------------------------------------- |
| `stream`   | `false` | 若为 `true`，调用返回 `StreamedRunResult`，并在事件从模型到达时逐步发出。                                            |
...
流式传输允许你在 LLM 运行过程中实时接收流式事件。流开始后，`StreamedRunResult` 将包含关于本次运行的完整信息，包括所有新产生的输出。你可以使用 `for await` 循环来迭代这些流式事件。详见[流式传输](/openai-agents-js/zh/guides/streaming)。
...
| `handoffInputFilter`        | `HandoffInputFilter`  | 执行交接时用于变更输入项（若交接本身未定义该过滤器）。        |
...
每次调用 `runner.run()`（或 `run()` 工具）都代表你的应用层会话中的一轮。你可以自行决定向终端用户展示多少 `RunResult`——有时只展示 `finalOutput`，有时展示每个生成的项。

<Code lang="typescript" code={chatLoopExample} title="延续会话历史示例" />
...
- [`ToolCallError`](/openai-agents-js/openai/agents-core/classes/toolcallerror) —— 任意函数工具调用失败。
- [`UserError`](/openai-agents-js/openai/agents-core/classes/usererror) —— 基于配置或用户输入抛出的任何错误。

以上错误均继承自基础类 `AgentsError`，它可能提供 `state` 属性以访问当前运行状态。
