...
默认情况下，SDK 在首次导入时读取环境变量 `OPENAI_API_KEY`。如果无法设置该变量，可手动调用 `setDefaultOpenAIKey()`。
...
可通过 `setTracingExportApiKey()` 设置单独的密钥：
...
SDK 使用 [`debug`](https://www.npmjs.com/package/debug) 包进行调试日志记录。将环境变量 `DEBUG` 设置为 `openai-agents*` 可查看详尽日志。

```bash
export DEBUG=openai-agents*
```

你可以使用来自 `@openai/agents` 的 `getLogger(namespace)` 获取用于自有模块的命名空间日志记录器。
