...
您也可以传入自定义的 `OpenAI` 客户端实例。否则，SDK 会使用默认的 key 自动创建一个客户端。
...
追踪默认启用，并使用上文中的 OpenAI key。

可以通过 `setTracingExportApiKey()` 设置单独的 key：
...
<Code lang="typescript" code={getLoggerExample} title="获取 logger" />
