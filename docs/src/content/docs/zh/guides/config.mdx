...
本页介绍通常在应用启动时一次性设置的**SDK 全局默认项**，例如默认 OpenAI 客户端、传输方式、追踪导出 key 以及调试日志行为。这些设置默认在整个进程范围内生效，因此这里适合做通用配置，而不是针对单个智能体或单次运行做调优。

如果你需要改为配置特定的 `Agent`、`Runner` 或 `run()` 调用，请参阅：

- [运行智能体](/openai-agents-js/guides/running-agents)：了解 `Runner` 和按运行配置选项。
- [模型](/openai-agents-js/guides/models)：了解智能体级别和 runner 级别的模型设置。
- [追踪](/openai-agents-js/guides/tracing)：了解特定运行的追踪配置和导出器行为。
...
这个进程级默认值只会影响后续通过默认 OpenAI provider 解析的模型。如果你传入的是具体的 `Model` 实例或自定义 `modelProvider`，请改为在对应位置配置传输。参见[模型指南](/openai-agents-js/guides/models#responses-websocket-transport)。
...
如果你想进一步了解追踪功能，请查看[追踪指南](/openai-agents-js/guides/tracing)。
