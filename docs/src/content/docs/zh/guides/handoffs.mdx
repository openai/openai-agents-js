...
交接让一个智能体将对话的一部分委派给另一个智能体。当不同智能体在特定领域各有所长时，这很有用。比如在客服应用中，您可以有负责预订、退款或常见问题的智能体。

交接在 LLM 中表现为工具。如果将对话交接给名为 `Refund Agent` 的智能体，工具名称将是 `transfer_to_refund_agent`。
...
有时你希望 LLM 在触发交接时提供数据。定义一个输入模式并在 `handoff()` 中使用它。
...
当你的提示词提到交接时，LLM 的响应更稳定。SDK 通过 `RECOMMENDED_PROMPT_PREFIX` 暴露了一个推荐前缀。
