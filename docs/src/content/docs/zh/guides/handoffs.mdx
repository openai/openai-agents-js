...
交接让一个智能体将对话的一部分委托给另一个智能体。当不同智能体专注于特定领域时，这很有用。例如在客服应用中，你可能有负责预订、退款或常见问题的智能体。

交接会作为工具呈现给 LLM。如果你将会话交接给名为 `Refund Agent` 的智能体，则工具名为 `transfer_to_refund_agent`。
...
- `onHandoff` – 发生交接时的回调。接收 `RunContext` 以及可选的解析后输入。
- `inputType` – 交接所期望的输入模式。
- `inputFilter` – 筛选传递给下一个智能体的历史。
...
常用辅助函数位于 `@openai/agents-core/extensions` 中。

<Code lang="typescript" code={inputFilterExample} title="输入筛选器" />

## 推荐提示

当提示中提到交接时，LLM 的响应更可靠。SDK 通过 `RECOMMENDED_PROMPT_PREFIX` 提供了推荐前缀。
