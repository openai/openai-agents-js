...
交接让一个智能体可以将对话的一部分委托给另一个智能体。当不同智能体擅长不同领域时，这很有用。比如在客服应用中，你可能有处理预订、退款或常见问题的智能体。

对于 LLM，交接被表示为工具。如果你将对话交接给名为 `Refund Agent` 的智能体，工具名称将是 `transfer_to_refund_agent`。
...
- `onHandoff` – 交接发生时的回调。接收 `RunContext` 以及可选的已解析输入。
- `inputType` – 交接所需的输入模式。
...
默认情况下，交接会接收整个对话历史。若要修改传递给下一个智能体的内容，请提供 `inputFilter`。
...
当你的提示提到交接时，LLM 的响应更稳定。SDK 通过 `RECOMMENDED_PROMPT_PREFIX` 提供了一个推荐的前缀。
