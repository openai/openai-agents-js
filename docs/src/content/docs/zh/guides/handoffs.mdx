...
交接让一个智能体将对话的一部分委托给另一个智能体。当不同智能体专注于特定领域时，这非常有用。比如在客服应用中，您可以让不同智能体分别处理预订、退款或常见问题。

交接在 LLM 中以工具的形式呈现。如果您把对话交接给名为 `Refund Agent` 的智能体，则工具名将是 `transfer_to_refund_agent`。

## 创建交接

每个智能体都接受一个 `handoffs` 选项。它可以包含其他 `Agent` 实例，或由 `handoff()` 帮助函数返回的 `Handoff` 对象。

如果传入的是普通的 `Agent` 实例，它们的 `handoffDescription`（如果提供）会被附加到默认工具描述之后。使用它来明确模型应何时选择该交接。
...
- `onHandoff` – 发生交接时的回调。接收一个 `RunContext` 和可选的解析后输入。
- `inputType` – 交接的预期输入模式。
...
默认情况下，交接会接收完整的对话历史。若需修改传递给下一个智能体的内容，请提供 `inputFilter`。
...
当提示中提到交接时，LLM 的响应会更为稳定。SDK 通过 `RECOMMENDED_PROMPT_PREFIX` 提供了一个推荐前缀。

<Code lang="typescript" code={recommendedPromptExample} title="推荐的提示词" />
