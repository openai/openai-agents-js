...
Update your voice agent's behavior in real-time during active calls using the session update API. This allows you to modify instructions and prompts without ending the current conversation.

```typescript
// Update agent configuration during an active session
const updateResponse = await fetch('/update-agent', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    instructions:
      'You are now a technical support specialist. Help users with technical issues.',
    prompt: 'How can I assist you with your technical question today?',
  }),
});
```

The transport layer's `updateSessionConfig()` method allows you to:

- **Modify system instructions** to change the agent's role or behavior
- **Update conversation prompts** to guide the interaction in new directions
- **Adapt to caller needs** based on conversation context or external systems

This is particularly useful for:

- **Escalation scenarios** where the agent needs to switch roles (e.g., from sales to technical support)
- **Context-aware responses** based on caller history or CRM data
- **A/B testing** different agent personalities or response styles during live calls
...
1. **Clone the repository and checkout the Plivo support branch:**
   ...
   ```bash
   git clone https://github.com/anshagrawal-plivo/openai-agents-js.git
   cd openai-agents-js
...
6. **Create a tunnel to your local server:**
