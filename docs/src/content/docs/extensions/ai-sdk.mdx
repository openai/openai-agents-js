...
import aiSdkPdfFileInputExample from '../../../../../examples/docs/ai-sdk/pdfFileInput.ts?raw';
...
When the filename or URL path ends in `.pdf`, the adapter infers `application/pdf`. Otherwise, set `providerData: { mediaType: 'application/pdf' }` explicitly. OpenAI file IDs are not supported through the AI SDK adapter; pass file data or a public URL, or use an OpenAI Responses model directly. Support for reading PDFs still depends on the selected AI SDK provider and model.

<Code
  lang="typescript"
  code={aiSdkPdfFileInputExample}
  title="Send a PDF through an AI SDK model"
/>
