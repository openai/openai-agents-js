...
Agents SDK は標準で Responses API または Chat Completions API を通じて OpenAI モデルで動作します。別のモデルを使用したい場合、[Vercel の AI SDK](https://sdk.vercel.ai/) がサポートする幅広いモデルを、このアダプターを通じて Agents SDK に取り込めます。
...
<Aside type="caution">
  現在、Vercel AI SDK v5 と互換性のある AI SDK のモデルプロバイダー v2
  モジュールをサポートしています。特別な理由で v1
...
メッセージにプロバイダー固有のオプションを送る必要がある場合は、`providerMetadata` に渡してください。値は基盤の AI SDK モデルにそのまま転送されます。たとえば、Agents SDK で次の `providerData`
