...
Agents SDK は標準で Responses API または Chat Completions API を通じて OpenAI モデルで動作します。ですが、別のモデルを使いたい場合は、[Vercel の AI SDK](https://sdk.vercel.ai/) がサポートするさまざまなモデルを、このアダプターを介して Agents SDK に取り込むことができます。
...
<Aside type="caution">
  ...
  を引き続き使用する必要がある場合は、[examples/ai-sdk-v1](https://github.com/openai/openai-agents-js/tree/main/examples/ai-sdk-v1)
...
メッセージにプロバイダー固有のオプションを送る必要がある場合は、`providerMetadata` に渡します。値は基盤となる AI SDK のモデルにそのまま転送されます。例えば、Agents SDK で次の `providerData`
