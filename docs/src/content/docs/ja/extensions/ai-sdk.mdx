...
Agents SDK は標準で Responses API または Chat Completions API を通じて OpenAI のモデルと動作します。別のモデルを使用したい場合は、[Vercel の AI SDK](https://sdk.vercel.ai/) がサポートする多数のモデルを提供しており、このアダプターを通じて Agents SDK に組み込むことができます。
...
<Aside type="caution">
  ...
  と互換）をサポートしています。特別な理由で v1 の model provider
  を使い続ける場合は、
...
メッセージにプロバイダー固有のオプションを送る必要がある場合は、`providerMetadata` を使って渡します。値は基盤となる AI SDK のモデルにそのまま転送されます。例えば、Agents SDK の次の `providerData`
