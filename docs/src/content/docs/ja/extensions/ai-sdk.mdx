...
Agents SDK は標準で OpenAI モデルと Responses API または Chat Completions API を通じて動作します。別のモデルを使用したい場合は、[Vercel の AI SDK](https://sdk.vercel.ai/) がサポートするさまざまなモデルをこのアダプター経由で Agents SDK に取り込めます。
...
<Aside type="caution">
  現在、Vercel AI SDK v5 と互換性のある ai-sdk の model provider v2
  モジュールをサポートしています。特別な理由で v1 の model provider
  を使い続けたい場合は、
