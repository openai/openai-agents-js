...
音声エージェントは OpenAI の speech-to-speech モデルを使用して、リアルタイムの音声チャットを提供します。これらのモデルは、ストリーミング オーディオ、テキスト、ツール呼び出しをサポートし、音声/電話でのカスタマーサポート、モバイルアプリでの体験、音声チャットなどのアプリケーションに最適です。
...
- ストリーミング イベントに対するコールバック
- テキストと音声のエージェントの両方で同じコンポーネントを再利用

音声対音声モデルを使うことで、モデルが音声をリアルタイムに処理する能力を活用でき、モデルの実行後にテキストへ書き起こしてから再度音声に変換し直す必要がありません。
