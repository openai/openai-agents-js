...
音声エージェントは OpenAI の音声対音声モデルを使用して、リアルタイムの音声チャットを提供します。これらのモデルは音声、テキスト、ツール呼び出しのストリーミングに対応し、音声／電話のカスタマーサポート、モバイルアプリでの体験、ボイスチャットなどの用途に最適です。
...
<LinkCard
  ...
  description="OpenAI Agents SDK を使って、リアルタイムの音声アシスタントを数分で構築します。"
/>
...
- ストリーミングイベントのコールバック
- 同じコンポーネントをテキストと音声のエージェントの両方で再利用

音声対音声モデルを使用することで、モデルが動作した後にテキストへ文字起こしして音声に再変換することなく、モデルの音声処理能力をリアルタイムに活用できます。
