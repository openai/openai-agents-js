...
ツールはエージェントに **行動させる** 手段です。データ取得、外部 API 呼び出し、コード実行、さらにはコンピュータの使用まで可能にします。JavaScript/TypeScript SDK は次の 4 つのカテゴリーをサポートします:

1. **組み込みツール（Hosted）** – OpenAI の サーバー 上でモデルと並行して動作します（Web 検索、ファイル検索、コンピュータ操作、Code Interpreter、画像生成）
2. **関数ツール** – 任意のローカル関数を JSON スキーマでラップし、LLM が呼び出せるようにします
3. **ツールとしてのエージェント** – 1 つのエージェント全体を呼び出し可能なツールとして公開します
4. **ローカル MCP サーバー** – 手元のマシンで動作する Model Context Protocol サーバーを接続します
...
| ファイル / 検索  | `'file_search'`      | OpenAI 上の ベクトルストア をクエリ |
...
正確なパラメーターは OpenAI Responses API に一致します。`rankingOptions` やセマンティックフィルタのような高度なオプションは公式ドキュメントを参照してください。
...
| `description`   | はい   | LLM に提示される明確で人間に読みやすい説明                                                                                                                                                                  |
| `parameters`    | はい   | Zod スキーマまたは 元 の JSON スキーマオブジェクトのいずれか。Zod の parameters を使うと自動的に **strict** モードが有効                                                                                    |
...
無効または部分的な入力をモデルに推測させたい場合は、元 の JSON スキーマを使う際に strict モードを無効化できます:
...
会話を完全にハンドオフせず、別のエージェントを支援役として使いたい場合は `agent.asTool()` を使います:
...
エージェントをツールとして実行する場合、Agents SDK はデフォルト設定でランナーを作成し、関数実行内でそのランナーによりエージェントを実行します。`runConfig` や `runOptions` のプロパティを指定したい場合は、`asTool()` メソッドに渡してランナーの動作をカスタマイズできます。
...
完全な例は [`filesystem-example.ts`](https://github.com/openai/openai-agents-js/tree/main/examples/mcp/filesystem-example.ts) を参照してください。MCP サーバーツール連携の包括的ガイドを探している場合は、[MCP 連携](/openai-agents-js/ja/guides/mcp) を参照してください。
...
- [`tool()`](/openai-agents-js/openai/agents/functions/tool) および各種 組み込みツール（Hosted） タイプの TypeDoc リファレンスを読む
