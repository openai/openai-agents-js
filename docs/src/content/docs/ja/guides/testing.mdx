...
| `RealtimeAgent` の関数ツールをテストする | `function_call` を発行し、`sendFunctionCallOutput` を期待する | [Test a Realtime agent tool workflow](#test-a-realtime-agent-tool-workflow) |
...
<Code
  ...
  title="Realtime エージェントの関数ツールのテスト"
/>
...
- [リアルタイムエージェントの概要](/openai-agents-js/ja/guides/voice-agents)では、`RealtimeSession` を紹介しています。
