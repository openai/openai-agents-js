...
デフォルトで SDK は、最初にインポートされた時に環境変数 `OPENAI_API_KEY` を読み取ります。環境変数を設定できない場合は、手動で `setDefaultOpenAIKey()` を呼び出せます。
...
トレーシングはデフォルトで有効になっており、上記の OpenAI キーを使用します。
...
`@openai/agents` の `getLogger(namespace)` を使って、独自モジュール向けの名前空間付きロガーを取得できます。
