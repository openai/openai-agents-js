...
既定では、SDK は初回のインポート時に環境変数 `OPENAI_API_KEY` を読み込みます。環境変数を設定できない場合は、`setDefaultOpenAIKey()` を手動で呼び出せます。
...
SDK はデバッグログに [`debug`](https://www.npmjs.com/package/debug) パッケージを使用します。冗長なログを表示するには、環境変数 `DEBUG` を `openai-agents*` に設定します。
