...
デフォルトでは SDK は初回インポート時に `OPENAI_API_KEY` 環境変数を読み込みます。変数を設定できない場合は `setDefaultOpenAIKey()` を手動で呼び出せます。
...
`@openai/agents` の `getLogger(namespace)` を使って、独自モジュール用に名前空間付きロガーを取得できます。
