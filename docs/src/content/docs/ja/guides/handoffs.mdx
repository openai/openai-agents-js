...
ハンドオフは、あるエージェントが会話の一部を別のエージェントに委譲できるようにします。これは、異なるエージェントが特定の分野を専門としている場合に便利です。たとえばカスタマーサポートアプリでは、予約、返金、または FAQ を扱うエージェントを用意できます。
...
- `onHandoff` – ハンドオフ発生時のコールバック。`RunContext` と、必要に応じてパース済みの入力を受け取る
- `inputType` – ハンドオフの期待される入力スキーマ
- `inputFilter` – 次のエージェントに渡す履歴をフィルター
...
プロンプトでハンドオフに言及すると、LLMs はより安定して応答します。SDK は `RECOMMENDED_PROMPT_PREFIX` を通じて推奨のプレフィックスを提供しています。
