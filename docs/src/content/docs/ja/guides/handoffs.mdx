...
ハンドオフは、エージェントが会話の一部を別のエージェントに委譲できるようにします。これは、異なるエージェントが特定の領域に特化している場合に有用です。例えばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントがあるかもしれません。
...
- `onHandoff` – ハンドオフ発生時のコールバック。`RunContext` と、オプションでパース済み入力を受け取ります
...
プロンプトにハンドオフが言及されていると、LLM はより安定して反応します。SDK は `RECOMMENDED_PROMPT_PREFIX` を通じて推奨のプレフィックスを提供します。
