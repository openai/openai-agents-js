...
ハンドオフは、あるエージェントが会話の一部を別のエージェントに委譲するしくみです。これは、異なるエージェントが特定の領域を専門化している場合に有用です。たとえばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントを用意できます。

ハンドオフは LLM にツールとして表現されます。`Refund Agent` というエージェントに引き継ぐ場合、ツール名は `transfer_to_refund_agent` になります。
...
- `onHandoff` – ハンドオフ時のコールバック。`RunContext` と、オプションで解析済み入力を受け取ります
- `inputType` – ハンドオフの期待入力スキーマ
...
プロンプトにハンドオフへの言及があると、LLM はより確実に応答します。SDK は `RECOMMENDED_PROMPT_PREFIX` を通じて推奨のプレフィックスを提供します。
