...
ハンドオフは、あるエージェントが会話の一部を別のエージェントに委譲できるようにします。これは、異なるエージェントが特定分野に特化している場合に有用です。たとえばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントを用意できます。
...
プレーンな `Agent` インスタンスを渡した場合、（指定されていれば）その `handoffDescription` が既定のツール説明に追加されます。モデルがそのハンドオフを選ぶべき状況を明確にするために使用します。
