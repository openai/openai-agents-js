...
ハンドオフを使うと、あるエージェントが会話の一部を別のエージェントに委譲できます。異なるエージェントが特定分野を専門化している場合に便利です。例えばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントを用意できます。

ハンドオフは LLM からはツールとして表現されます。`Refund Agent` というエージェントに引き継ぐ場合、ツール名は `transfer_to_refund_agent` になります。
...
既定ではハンドオフは会話履歴全体を受け取ります。次のエージェントに渡す内容を変更するには、`inputFilter` を指定します。
...
プロンプトでハンドオフに言及すると LLM の応答がより安定します。SDK は `RECOMMENDED_PROMPT_PREFIX` として推奨の接頭辞を提供しています。
