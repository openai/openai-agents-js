...
ハンドオフを使うと、あるエージェントが会話の一部を別のエージェントへ委譲できます。これは、異なるエージェントが特定分野を専門としている場合に有用です。たとえばカスタマーサポートアプリでは、予約、返金、FAQ を担当するエージェントを用意できます。

ハンドオフは LLM からはツールとして表現されます。`Refund Agent` というエージェントへ引き継ぐ場合、ツール名は `transfer_to_refund_agent` になります。

## ハンドオフの作成

すべてのエージェントは `handoffs` オプションを受け取ります。ここには、他の `Agent` インスタンスや、`handoff()` ヘルパーが返す `Handoff` オブジェクトを含められます。
...
ハンドオフの呼び出し時に、LLM にデータ提供を求めたい場合があります。入力スキーマを定義して `handoff()` で使用します。
...
プロンプトでハンドオフに言及すると、LLM はより安定して応答します。SDK は `RECOMMENDED_PROMPT_PREFIX` 経由で推奨の接頭辞を提供します。
