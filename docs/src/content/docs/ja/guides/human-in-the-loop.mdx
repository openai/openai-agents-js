...
これらのコールバックが決定を返すと、人間の応答を待つために一時停止することなく実行が続行されます。Realtime セッション API については、[音声エージェントの構築](/openai-agents-js/ja/guides/voice-agents/build#approval-before-tool-execution)の承認フローを参照してください。
