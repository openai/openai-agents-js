...
3. OpenAI API キーを設定します。まだ持っていない場合は、OpenAI API キーの作成方法については[こちらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)をご覧ください。
...
`run` メソッドでエージェントを実行できます。開始したいエージェントと、渡したい入力を指定して実行します。

この呼び出しは、最終出力と、その実行中に行われたあらゆるアクションを含む execution result を返します。
...
追加のエージェントを同様に定義して、課題を小さな部分に分割し、エージェントを目の前のタスクにより集中させられます。これは、エージェントでモデルを定義することで、異なる問題に異なるモデルを使うことも可能にします。
...
複数のエージェント間をオーケストレーションするために、エージェントに `handoffs` を定義できます。これにより、会話を次のエージェントへ引き継げます。これは実行の過程で自動的に行われます。
...
Runner は個々のエージェントの実行、潜在的なハンドオフ、およびツール実行の処理を担当します。
