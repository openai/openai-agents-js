...
3. OpenAI API キーを設定します。まだお持ちでない場合は、OpenAI API キーを作成するために[これらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)に従ってください
...
`run` メソッドでエージェントを実行できます。開始したいエージェントと渡したい入力を両方渡すことで実行をトリガーします
...
追加のエージェントを同様に定義して、問題を小さな部分に分解し、エージェントを目の前のタスクにより集中させることができます。また、エージェントにモデルを定義することで、問題ごとに異なるモデルを使用することもできます
...
Runner は個々のエージェントの実行、必要に応じたハンドオフ、ツール実行の処理を担当します
...
すべてを 1 つの完全なサンプルにまとめましょう。これを `index.js` に配置して実行します

<Code lang="typescript" code={quickstartExample} title="クイックスタート" />

## トレースの表示

Agents SDK は自動的にトレースを生成します。これにより、エージェントの動作、呼び出されたツール、引き継いだエージェントを確認できます
