...
3. OpenAI API key を設定します。まだお持ちでない場合は、 OpenAI API key の作成手順については[こちらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)をご覧ください
...
`run` メソッドでエージェントを実行できます。開始したいエージェントと渡したい入力の両方を渡すと、実行がトリガーされます

これにより、その実行中に行われた最終出力と任意のアクションを含む execution result が返されます
...
追加のエージェントを同様に定義して、問題を小さな部分に分割し、エージェントを目の前のタスクにより集中させることができます。また、エージェントにモデルを定義することで、問題ごとに異なるモデルを使用できます
...
実行後、 result の `finalAgent` プロパティを見ると、どのエージェントが最終的な応答を生成したかがわかります
