...
1. プロジェクトを作成し、 npm を初期化します。一度だけ実施すれば大丈夫です
...
3. OpenAI API キーを設定します。まだお持ちでない場合は、 OpenAI API キーの作成方法については[こちらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)をご覧ください
...
`run` メソッドを使ってエージェントを実行できます。開始したいエージェントと、渡したい入力の両方を指定して実行をトリガーします。
...
追加のエージェントを同様に定義して、問題を小さな部分に分解し、目の前のタスクに対してエージェントの集中度を高められます。さらに、エージェントにモデルを定義することで、異なる問題に対して異なるモデルを使い分けることもできます。
...
複数のエージェント間でオーケストレーションするために、エージェントに `handoffs` を定義できます。これにより、エージェントが会話を次のエージェントへ引き継げるようになります。これは実行の過程で自動的に行われます。
...
Runner は、個々のエージェントの実行、必要に応じたハンドオフ、およびツールの実行を処理します。
