...
3. OpenAI API key を設定します。まだ持っていない場合は、OpenAI API key の作成方法については[こちらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)を参照してください。

   ```bash
   export OPENAI_API_KEY=sk-...
   ```

   あるいは `setDefaultOpenAIKey('<api key>')` を呼び出してプログラムから key を設定し、
...
`run` メソッドでエージェントを実行できます。開始したいエージェントと、渡したい入力を同時に渡して実行します。

これにより、その実行中に行われた最終出力と実行されたアクションを含む execution result が返されます。
...
情報の検索やアクションの実行に使えるツールをエージェントに与えられます。
...
追加のエージェントを同様に定義して、問題を小さな部分に分割し、目の前のタスクに集中させられます。エージェントごとにモデルを定義することで、課題ごとに異なるモデルを使うことも可能です。
...
複数のエージェント間をオーケストレーションするために、エージェントに `handoffs` を定義できます。これにより、会話を次のエージェントへ自動的に引き継げます。これは実行の過程で自動的に行われます。
...
実行後、result の `finalAgent` プロパティを見ることで、どのエージェントが最終応答を生成したかを確認できます。

## エージェントのオーケストレーションの実行

Runner は各エージェントの実行、必要に応じたハンドオフ、ツール実行の処理を担当します。
...
Agents SDK は自動でトレースを生成します。これにより、エージェントの動作、呼び出したツール、ハンドオフ先のエージェントを確認できます。

エージェント実行中に何が起きたか確認するには、
[OpenAI Dashboard の Trace viewer](https://platform.openai.com/traces) に移動します。

## 次のステップ

より複雑なエージェント フローの作成方法を学びましょう。
