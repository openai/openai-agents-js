...
1. プロジェクトを作成し、npm を初期化します。これは一度だけ必要です
...
`run` メソッドでエージェントを実行できます。開始するエージェントと渡したい入力の両方を指定して実行します

これは、その実行中に行われた最終出力とすべてのアクションを含む実行結果を返します
...
追加のエージェントを同様に定義して、課題を小さな部分に分け、エージェントが目の前のタスクにより集中できるようにします。これはまた、エージェントにモデルを定義することで、課題ごとに異なるモデルを使うことも可能にします
...
Runner は個々のエージェントの実行、発生し得るハンドオフ、ツールの実行を処理します
...
エージェントの実行中に何が起きたかを確認するには、[OpenAI ダッシュボードの Trace viewer](https://platform.openai.com/traces)にアクセスしてください
