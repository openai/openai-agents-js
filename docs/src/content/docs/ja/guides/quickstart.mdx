...
3. OpenAI API キーを設定します。まだ持っていない場合は、OpenAI API キーの作成方法については[こちらの手順](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)を参照してください

   ```bash
   export OPENAI_API_KEY=sk-...
   ```

   代わりに `setDefaultOpenAIKey('<api key>')` を呼び出してプログラムからキーを設定し、
...
これにより、その実行で行われたすべてのアクションと最終出力を含む実行結果が返されます
...
情報の参照やアクションの実行のために、エージェントに tools を与えることができます
...
追加のエージェントを同様に定義して、問題を小さな部分に分割し、エージェントが目の前のタスクにより集中できるようにします。また、エージェントごとにモデルを定義することで、問題に応じて異なるモデルを使い分けることもできます
...
複数のエージェント間でオーケストレーションするために、エージェントに `handoffs` を定義できます。これにより、実行の過程で会話を次のエージェントに自動的に引き継げるようになります
...
実行後、結果の `finalAgent` プロパティを見ると、最終的な応答を生成したエージェントが分かります

## エージェントオーケストレーションの実行

Runner は、各エージェントの実行、ハンドオフ、ツール実行の処理を担当します
...
すべてをひとつの完全な例にまとめましょう。これを `index.js` に配置して実行します

<Code lang="typescript" code={quickstartExample} title="Quickstart" />

## トレースの確認

Agents SDK はトレースを自動生成します。これにより、エージェントの動作、呼び出されたツール、どのエージェントにハンドオフしたかを確認できます
