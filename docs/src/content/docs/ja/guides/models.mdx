...
`Agent` の初期化時にモデルを指定しない場合、既定のモデルが使用されます。現在の既定は互換性のため [`gpt-4.1`](https://platform.openai.com/docs/models/gpt-4.1) です。利用可能であれば、より高品質のために [`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2) に設定することを推奨します。

[`gpt-5.2`](https://platform.openai.com/docs/models/gpt-5.2) など他のモデルに切り替えたい場合、エージェントの設定方法は 2 つあります。
