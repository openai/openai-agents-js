...
Agents SDK は、モデルや他の実行ステップからの出力を段階的に配信できます。ストリーミングにより UI は応答性を保ち、エンドユーザーの更新前に最終結果全体を待つ必要がなくなります。
...
ストリーミングが有効な場合、返される `stream` は `AsyncIterable` インターフェースを実装します。各イベントは、実行中に何が起きたかを説明するオブジェクトとして出力されます。ストリームは、エージェントの実行の異なる部分を表す 3 種類のイベントのいずれかを出力します。多くのアプリケーションはモデルのテキストだけを必要とするため、ストリームには補助機能が用意されています。

### テキスト出力の取得

`stream.toTextStream()` を呼び出して、出力されたテキストのストリームを取得します。`compatibleWithNodeStreams` が `true` の場合、戻り値は通常の Node.js `Readable` です。これをそのまま `process.stdout` や他の出力先にパイプできます。
...
`stream.completed` の Promise は、実行とすべての保留中のコールバックが完了した時点で解決します。出力がもうないことを確実にするには、必ず待機してください。
...
ストリーミングは、実行を一時停止するハンドオフ（たとえばツールの承認が必要な場合）と両立します。ストリームオブジェクトの `interruption` フィールドで割り込みを取得でき、各割り込みに対して `state.approve()` または `state.reject()` を呼び出すことで実行を継続できます。`{ stream: true }` で再実行すると、ストリーミング出力が再開します。
...
- アプリケーションがテキスト結果だけを必要とするなら、個々のイベントオブジェクトを扱わずに済むよう `toTextStream()` を優先してください
