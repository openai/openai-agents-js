/* global document, navigator, RTCPeerConnection, MediaStream, WebSocket, location, window, setTimeout, clearTimeout */
...
function end(call) {
  ...
  stop.disabled = true;
  mute.disabled = true;
  call.microphone?.getTracks().forEach((track) => {
    track.enabled = false;
  });
...
start.addEventListener('click', async () => {
  ...
  active = call;
  try {
    call.microphone = await navigator.mediaDevices.getUserMedia({
      audio: true,
    });
    if (active !== call) {
      cleanup(call);
    ...
    call.timeout = setTimeout(() => {
      if (active === call) end(call);
    }, 45000);
    call.peer = new RTCPeerConnection();
    call.microphone
      .getTracks()
      .forEach((track) => call.peer.addTrack(track, call.microphone));
    call.peer.addEventListener('track', (event) => {
      ...
      audio.play().catch(() => {
        if (active !== call || call.closing) return;
        status.textContent =
          'Select play in the audio controls to hear the assistant.';
      });
    });
    ...
    call.peer.addEventListener('connectionstatechange', () => {
      if (
        active === call &&
        ['failed', 'disconnected'].includes(call.peer.connectionState)
      ) {
        end(call);
      }
    });
    ...
    call.socket.addEventListener('close', () => {
      ...
      if (!call.finalized)
        status.textContent =
          call.error ?? 'Disconnected without confirmed final session usage.';
      cleanup(call);
    });
