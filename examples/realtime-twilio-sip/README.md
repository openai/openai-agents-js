...
To deliberately log caller and assistant text and audio transcripts, set `TWILIO_SIP_LOG_TRANSCRIPTS=1` before starting the server:

```bash
TWILIO_SIP_LOG_TRANSCRIPTS=1 pnpm -F realtime-twilio-sip start
```

Use this option only with test conversations whose content you intend to store in logs. Unset the variable and restart the server to disable transcript logging. Only the exact value `1` enables this option; changing the general log level does not enable it. Raw provider errors and exception stacks remain omitted from the example's logs even when transcript logging is enabled. This setting controls only this example's transcript logging, not SDK tracing or logging configured separately for dependencies.
