# Tool Integrations

These examples demonstrate the hosted tools provided by the Agents SDK.

## Examples

- `computer-use.ts` – Uses the computer tool with Playwright to automate a local browser.

  ```bash
  pnpm examples:tools-computer-use
  ```

- `file-search.ts` – Shows how to run a vector search with `fileSearchTool`.

  ```bash
  pnpm examples:tools-file-search
  ```

- `tool-search.ts` – Demonstrates deferred namespace loading plus a top-level deferred `tool_search` example with selective loading.

  ```bash
  pnpm examples:tools-tool-search
  ```

- `programmatic-tool-calling.ts` – Uses hosted JavaScript to run read-only inventory tools concurrently and reduce their results into a replenishment plan.

  ```bash
  pnpm examples:tools-programmatic-tool-calling
  ```

- `codex.ts` – Runs the Codex CLI via the codex tool wrapper with streaming event logs and skill usage. Requires `CODEX_API_KEY` or `OPENAI_API_KEY` (Codex falls back to this unless `CODEX_API_KEY` is set). You can set `CODEX_PATH` to override the Codex CLI path.

  ```bash
  pnpm examples:tools-codex
  ```

- `codex-same-thread.ts` – Reuses a Codex thread across multiple agent turns via `useRunContextThreadId`.

  ```bash
  pnpm examples:tools-codex-same-thread
  ```

- `web-search.ts` – Demonstrates `webSearchTool` for general web queries.

  ```bash
  pnpm examples:tools-web-search
  ```

- `code-interpreter.ts` – Demonstrates `codeInterpreterTool` for code execution.

  ```bash
  pnpm -F tools start:code-interpreter
  ```

- `image-generation.ts` – Demonstrates `imageGenerationTool` for image generation.

  ```bash
  pnpm -F tools start:image-generation
  ```

- `local-shell.ts` – Demonstrates `shellTool` with a local shell environment.

  ```bash
  pnpm examples:tools-shell
  ```

- `container-shell-skill-ref.ts` – Demonstrates `shellTool` with a generated `csv-workbench` skill mounted in `container_auto`, then reuses the same environment via `container_reference`.

  ```bash
  pnpm examples:tools-container-shell
  ```

- `container-shell-inline-skill.ts` – Demonstrates `shellTool` with an inline `csv-workbench` skill embedded directly in `container_auto`, then reuses the same environment via `container_reference`.

  ```bash
  pnpm examples:tools-container-shell-inline
  ```

- `apply-patch.ts` – Uses the `applyPatchTool` helper to apply file edits generated by the model.

  ```bash
  pnpm examples:tools-apply-patch
  ```
