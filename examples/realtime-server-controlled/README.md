...
The browser disables Start until the previous session's cleanup request succeeds or confirms that no session is accessible to the current principal. If that request fails, the browser keeps the application session ID and enables Stop for another attempt. An authentication rejection triggers one authentication refresh and Close retry. An expired demo login may renew as a different principal; the browser then accepts a not-found response without transferring ownership of the old call. The server retains failed provider hangups and retries them in the existing one-minute cleanup sweep, including calls rejected during setup and calls owned by expired demo principals. During shutdown, the server makes one additional hangup attempt after a failure and reports any remaining failure without logging provider payloads. Cleanup state is in memory only; this example does not guarantee cleanup after process termination.

The browser observes the WebRTC peer connection after signaling completes. A `failed` connection starts the same cleanup path as Stop. A `disconnected` connection gets 10 seconds to recover before cleanup starts; recovery or Stop cancels that timer. The example does not restart ICE or reconnect an existing call automatically.
