...
- Default-deny explicit, ambient environment, workload identity, and managed identity credentials for mounts whose helper runs with the same principal inside a model-controlled sandbox. Credentialless helpers must disable ambient or metadata authentication and use anonymous access where supported. Prefer external or provider-native mounts; require a runtime-only exact effective-path acknowledgement on a trusted `Manifest` before enabling ambient or metadata authentication, reject credential files whose effective paths resolve to serializable manifest entries, and validate dynamic mount mutations before provider or filesystem side effects.
- Strip mount credentials and credential-config paths from serialized state. For resumable external or provider-native mounts, compare the credential-free persisted and current trusted mount topologies before restoring credentials from the current manifest; never accept exposure policy from persisted data. Before reconnecting an explicit session state, also prove that the original unredacted live authority matches the current trusted manifest; redacted or changed authority requires fresh creation. Never reconnect serialized sessions that contain same-principal in-container mounts; create a fresh sandbox from current trusted configuration instead. Reuse a live mounted session only when its runtime-recorded credential authority and topology still match the current provider-normalized trusted manifest; otherwise close it and create a fresh session.
- Serialize manifest mutations per session and materialize the same immutable mount and credential-file candidates that passed validation. Record live mount authority only after that mount succeeds. Treat failed unmounts during replacement as ambiguous privileged transitions. If a privileged transition fails after provider effects may have started, tombstone the session before attempting force termination and reject every later I/O, persistence, or reuse path even when cleanup fails.
