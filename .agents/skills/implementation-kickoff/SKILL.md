...
After clean review, run `$changeset-validation` when packages or changesets changed and `$code-change-verification` whenever its SDK eligibility rules apply. Lightweight review does not waive an otherwise required SDK stack. Repo-meta work uses its applicable skill and focused checks. Do not create the branch or commit while required review, verification, or evidence remains incomplete.
