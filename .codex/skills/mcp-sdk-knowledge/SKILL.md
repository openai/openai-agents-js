---
name: mcp-sdk-knowledge
description: Use when working with the MCP TypeScript SDK (`modelcontextprotocol/typescript-sdk`) and you need authoritative repository docs. Prefer the DeepWiki MCP server to retrieve repo docs or answer questions; if it is not configured, guide the user to enable `deepwiki`.
---
...
Use the DeepWiki MCP server to read documentation for the MCP TypeScript SDK repository or to ask questions about it. Base your answers on the returned content instead of guessing.

## Workflow

### 1) Check whether the DeepWiki MCP server is available

If DeepWiki MCP tools are available (for example, `read_wiki_structure`, `read_wiki_contents`, `ask_question`), use them.
...
- For structure: call `read_wiki_structure`.
- For content: call `read_wiki_contents` for the sections you need.
