  ...
  try {
    ...
    expect(warnSpy.mock.calls).toEqual(
      Array.from({ length: expectedCount }, () => [
        REDACTED_TOOL_NAME_COLLISION_WARNING,
      ]),
    );
    return result;
  } finally {
...
export function registerRunStateCoreTests(): void {
  describe('RunState', () => {
    ...
    it('does not serialize runtime-only agent-tool metadata', async () => {
      ...
      const restored = await RunState.fromString(agent, state.toString());
      expect(
        new RunResult(restored as any).agentToolInvocation,
      ).toBeUndefined();
    });
    ...
    it('prefers override-context rejection messages when merge conflicts occur', async () => {
      ...
      expect(restored._context).toBe(overrideContext);
      expect(
        restored._context.getRejectionMessage('secure_tool', 'call-1'),
    ...
    it('tracks pending agent tool runs using tool name and call id', async () => {
      ...
      const restored = await RunState.fromString(agent, state.toString());
      expect(restored.getPendingAgentToolRun('toolA', 'call-1')).toBe(
        'state-A',
      );
      expect(restored.getPendingAgentToolRun('toolB', 'call-1')).toBe(
        'state-B',
      );
    });
    ...
    it('keeps pending agent tool aliases on one canonical state entry', async () => {
      ...
      expect(restored.hasPendingAgentToolRun(canonicalKey, 'call-1')).toBe(
        false,
      );
      expect(restored.hasPendingAgentToolRun('crm.lookup', 'call-1')).toBe(
        false,
      );
    ...
    it.each([
      ...
      'rejects a pending agent tool alias with a $name',
      async ({ mutate }) => {
        ...
        state._lastProcessedResponse = {
          ...
          functions: [
            {
              toolCall: createToolCall('crm', 'call-1'),
              tool: crmLookup as any,
            },
            {
              toolCall: createToolCall('crm', 'call-2'),
              tool: crmLookup as any,
            },
            {
    ...
    it('restores duplicate-name run item ownership by identity', async () => {
      ...
      expect(
        (restored._lastProcessedResponse?.newItems[0] as ToolApprovalItem)
          .agent,
      ).toBe(childB);
    });
...
export function registerRunStateMigrationTests(): void {
  describe('RunState', () => {
    ...
    it.each([
      ...
      async (_name, approvalRecord, expectedApproval, expectedMessage) => {
        ...
        const publicApproval =
          restored._context.toJSON().approvals['crm.lookup'];
    ...
    it('keeps ambiguous legacy function fallback separate from an exact non-function override', async () => {
      ...
      expect(
        durable.context.legacyFunctionApprovals['crm.lookup'].rejected,
      ).toBe(true);
    ...
    it('keeps schema 1.15 approval ownership when merging a canonical override context', async () => {
      ...
      for (const decision of decisions) {
        ...
        expect(
          restored._context.getRejectionMessage(namespacedKey, callId),
        ).toBe(decision.message);
        expect(
          restored._context.isToolApproved({
            toolName: namespacedKey,
...
export function registerRunStateToolSearchTests(): void {
  describe('RunState', () => {
    ...
    it('accepts schema versions with tool_search support during deserialization', async () => {
      ...
      for (const $schemaVersion of ['1.8', '1.10', '1.11'] as const) {
        ...
        expect(restored._generatedItems[0]).toBeInstanceOf(
          RunToolSearchCallItem,
        );
    ...
    it('rehydrates duplicate-name client tool_search calls by agent identity', async () => {
      ...
      expect(restored.getToolSearchRuntimeTools(betaAgent)).toEqual([
        betaLookup,
      ]);
    });
    ...
    it('uses the collision-filtered tool winner during tool-search rehydration', async () => {
      ...
      const restored = await expectToolNameCollisionWarnings(1, () =>
        RunState.fromString(agent, state.toString()),
      );
    ...
    it('rejects reused function call ids before tool-search rehydration callbacks', async () => {
      ...
      state._lastProcessedResponse = {
        newItems: [],
        functions: [
          { toolCall: configuredCall, tool: configuredLookup as any },
        ],
        handoffs: [],
        computerActions: [],
        shellActions: [],
    ...
    it('rejects a malformed serialized runtime identity before rehydration callbacks', async () => {
      ...
      await expect(
        RunState.fromString(agent, state.toString()),
      ).rejects.toThrow('function without a valid routed identity');
      expect(execute).not.toHaveBeenCalled();
    });
...
export function registerRunStateDeserializeTests(): void {
  describe('deserialize helpers', () => {
    ...
    it('rejects a schema 1.16 handoff without exact identity instead of rebinding a same-name target', async () => {
      ...
      const missingTarget = JSON.parse(serializedJson);
      delete missingTarget.lastProcessedResponse.handoffs[0].targetAgent;

      const unknownTarget = JSON.parse(serializedJson);
      unknownTarget.lastProcessedResponse.handoffs[0].targetAgent.identity =
        'missing-target-identity';

      await expectToolNameCollisionWarnings(3, () =>
        Promise.all([
          expect(RunState.fromString(agent, serializedJson)).rejects.toThrow(
            'Handoff transfer not found',
          ),
          expect(
          ...
          expect(
            RunState.fromString(agent, JSON.stringify(unknownTarget)),
    ...
    it('deserializeProcessedResponse restores namespaced function tools', async () => {
      ...
      state._lastProcessedResponse = {
        newItems: [],
        functions: [
          { toolCall: functionCall, tool: billingNamespace[0] as any },
        ],
        handoffs: [],
        computerActions: [],
        shellActions: [],
    ...
    it('deserializeProcessedResponse restores GA computer tool aliases', async () => {
      ...
      const json = state.toJSON();
      json.lastProcessedResponse!.computerActions[0]!.computer.name =
        'computer';
    ...
    it('deserializeProcessedResponse restores shell actions', async () => {
      ...
      const restored = await RunState.fromString(agent, state.toString());
      expect(restored._lastProcessedResponse?.shellActions[0]?.shell).toBe(
        shell,
      );
    });
