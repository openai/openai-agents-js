...
describe('client tool-search MCP authentication', () => {
  it.each([
    { custom: false, stream: false },
    { custom: false, stream: true },
    { custom: true, stream: false },
    { custom: true, stream: true },
  ])(
    'keeps authentication in runtime tools (custom: $custom, stream: $stream)',
    async ({ custom, stream }) => {
      ...
      const agent = new Agent({
        ...
        model,
        tools: custom
          ? [attachClientToolSearchExecutor(clientSearch, discover)]
          : [clientSearch, mcp],
      });
      const runner = new Runner({ tracingDisabled: true });
      const session = new MemorySession();
      const result = stream
        ? await runner.run(agent, 'find records', { session, stream: true })
        : await runner.run(agent, 'find records', { session });
      const events: unknown[] = [];
      if ('completed' in result) {
        for await (const event of result) {
          if (event.type === 'run_item_stream_event') {
            events.push(event.item.rawItem);
          }
        }
        await result.completed;
      ...
      if (output?.type !== 'tool_search_output') {
        throw new Error('Expected a tool-search output');
      }
      expect(output.tools[0]).not.toHaveProperty('authorization');
      expect(output.tools[0]).not.toHaveProperty('headers');
      expect(output.tools[0]).not.toHaveProperty('on_approval');
      for (const sink of [
        result.history,
      ...
      expect(mcp.providerData.on_approval).toBe(onApproval);
      expect(mcp.providerData.authorization).toBe(authorization);
      expect(mcp.providerData.headers).toEqual({ 'X-Api-Key': header });
      ...
      if (custom) {
        expect(restored.getToolSearchRuntimeTools(agent)).toContain(mcp);
        expect(discover).toHaveBeenCalledTimes(2);
      }
      // Flat history carries definitions, so a new run supplies current credentials.
      const replayAgent = new Agent({
        name: agent.name,
        model,
        tools: [clientSearch, mcp],
      });
      const resumed = await runner.run(replayAgent, result.history);
      expect(resumed.finalOutput).toBe('DONE');
      expect(model.calls[2].request.tools).toContainEqual(
        expect.objectContaining({
          providerData: expect.objectContaining({
            authorization,
            headers: { 'X-Api-Key': header },
      ...
      expect(JSON.stringify(model.calls[2].request.input)).not.toContain(
        authorization,
      );
      expect(JSON.stringify(model.calls[2].request.input)).not.toContain(
        header,
