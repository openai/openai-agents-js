...
describe('BlaxelSandboxClient', () => {
  ...
  test('rejects named sandbox conflict fallback when the sandbox is still non-resumable', async () => {
    createSandboxMock.mockRejectedValue(sandboxAlreadyExistsError());
    getMock
    ...
    await expect(client.create(new Manifest())).rejects.toThrow(
      'Blaxel sandbox shared-sandbox cannot be safely reused because its status is FAILED.',
    );
  ...
  test('cleans S3 credential files after timed-out secret writes finish late', async () => {
    let resolveSecretWrite: (() => void) | undefined;
    ...
    expect(
      commands.some(
        (command) =>
          command.includes('rm -f --') &&
          command.includes(secretPath as string),
      ),
    ).toBe(true);
    expect(rmMock).not.toHaveBeenCalledWith(secretPath);

    resolveSecretWrite?.();
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(rmMock.mock.calls.some(([path]) => path === secretPath)).toBe(true);
  });
