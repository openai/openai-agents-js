...
The declaration check preserves released binding spaces and declaration kinds, enum and literal members, public class constructibility, and the names, optional/readonly shape, and callable-versus-property kind of public named members on SDK-owned classes and object types. The selected Runloop facade list separately requires its named entries, including `platform()`, to remain present. Explicitly selected direct callable type aliases also preserve their generic parameters, callable parameters, and return type. Explicit `selectedPublicObjectProperties` entries also protect one direct inline object property of a selected function parameter. Each entry must list every immediate field and its primitive type, optionality, and readonly status, including before the object appears in the released baseline. This policy currently covers `webSearchTool` image settings through both public package roots. Optional field additions require complete policy enrollment; released field removals, shape/type changes, and required additions remain incompatible even if the candidate policy is updated. All declaration conditions, source checks, both installed-package profiles, and promotion validate these selections. Selected object properties support only a single non-generic function signature and inline objects with identifier-named number, boolean, or string fields; unsupported selected forms fail explicitly. Selection identities are registered in checker code independently of the candidate JSON policy. Removing a registered selection, omitting the selection policy, or adding an unregistered identity fails before package acquisition. Adding a new selection requires registering its identity in checker code as well as providing its complete JSON policy. Readonly checks use effective TypeScript symbols for both the selected object and its immediate fields, including mapped utility types. The checker does not recursively enroll other properties.

The contract intentionally does not implement a separate compatibility system for other methods, constructor or callable parameters and returns, unselected property types, index signatures, generics, overloads, or unselected callable type aliases. Existing TypeScript build checks and release review remain responsible for those deeper signature changes.
...
Promotion first rejects incompatibility with the previous baseline. It then records every subpath and export condition from the integrity-pinned published packages, so newly released paths become protected automatically. Promotion preserves selection policies and requires complete selected-object coverage before writing the new baseline.
